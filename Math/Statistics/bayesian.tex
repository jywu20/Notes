...
A generative model models $p(x, y | \theta)$,
and not $p(y | x, \theta)$ (the latter being a regression model, i.e. a discriminative model).
In theory, we can derive a generative model from a discriminative model via 
\begin{equation}
...
\end{equation}
Note that the equations make sense in frequentist statistics,
as long as we take $\theta$ to be fixed (but unknown) parameters.
