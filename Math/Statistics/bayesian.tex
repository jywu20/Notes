...
(see discussions in \prettyref{sec:frequentist-comparison}),
...
\subsection{Comparison with the frequentist school}\label{sec:frequentist-comparison}
...
\footnote{
    ...
    like assuming that there is a noise term (an objective-ish concept).
    ...
    and that's all we need in Bayesian statistics.
}
...
\begin{equation}
    ...
    \label{eq:bayesian-frequentist}
\end{equation}
and thus 
\begin{equation}
    p(B | A, \theta) = \frac{P(A | B, \theta)}{P(A | \theta)} P (B | \theta) 
    \Rightarrow p_\theta(B | A) = \frac{p_\theta(A | B)}{p_\theta(A)} p_\theta(B),
\end{equation}
...
$\theta$ appearing in the condition is just another way to say ``fixing $\theta$ to a constant''.
...
At the start of this note, we have already noticed that MAP can be seen as MLE (a frequentist concept) with a regularization term.
So $p(\theta)$ and $p(\dots | \theta)$ can all be understood within the frequentist framework.
A natural question then is, what's $p(\theta | \dots)$ in the frequentist framework?
It is generally hard to answer (except as a loss function for optimization in MAP-as-MLE-with-regularization), but asymptotically,
we have theorems like the Bernstein-von Mises theorem showing 
that a posteriori probabilities in Bayesian statistics are asymptotically correct from a frequentist point of view.

One particular benefit of the observation \eqref{eq:bayesian-frequentist}
is that it enables us to incorporate the action of making hypotheses when building a model into probabilistic inference.
Suppose I stipulate that $p(A | B, \theta)$ has to take a particular form.
We can understand this as 
\begin{equation}
    p_H(A | B, \theta) \coloneqq p(A | B, \theta, H).
\end{equation}
Note that we can even let $H$ participate in \eqref{eq:naive-update}:
as long as its prior is set to $1$, it never changes in Bayesian update, a direct consequence of \eqref{eq:bayesian}.%
\footnote{
    One may note that different $H$ lead to different forms of $\theta$.
    One can, however, replace $\theta$ by $(\theta_1, \theta_2, \dots)$,
    where $\theta_i$ is the form of parameters assumed by $H_i$.
    Now we further assume that all probabilistic distributions involving $\theta, H_i$ only depend on $\theta_i$,
    and hence $\grad_\theta$ can be replaced by $\grad_{\theta_i}$.
}
Other quantities like $p(A | \theta)$ can be similarly defined as 
$p_H(A | \theta)$ or $p(A | \theta, H)$.
Now from \eqref{eq:bayesian-frequentist}, we are confident that we can treat $p_H(A | \theta)$ as $p(A | \theta)$,
and this explains why we can replace $p(\theta | \mathcal{D})$ and $p(\theta)$ in \eqref{eq:bayesian}
by $p_H(\theta | \mathcal{D})$ and $p_H(\theta)$,
which justifies stipulating forms for them.
This also gives us a way to estimate how likely $H$ is,
in the same way we calculate the a posteriori distribution of $\theta$,
which can be seen as a way to check how reasonable $H$ is.
