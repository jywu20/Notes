...
这就是我们将有左逆的态射称为section的原因；我们也可以从这里看到为什么截面是单射——每个$x$都被映射到了不同的$y$上，但是$f X$未必能够覆盖整个$Y$。相应的，$Y$称为**bundle** over $X$。（注意：不同的$f$可能会以不同的方式将$Y$分割为纤维，再一次，我们看到所有的信息其实都在态射里面）
...
容易看出实际上这就是internal hom的“精细化”：我们不再笼统地写下$[\Sigma, E]$，而是允许指定$\Sigma$上不同的元素应该被映射到哪条纤维上；一个space of sections中的元素当然也是$[\Sigma, E]$中的元素（如果后者存在的话），但是反之不然。
