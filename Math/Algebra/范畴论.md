...
定义：$\mathcal{C}$和$\mathcal{D}$之间的**adjunction**是一对函子$(F, G)$，$F:\mathcal{C} \to \mathcal{D}$，$G: \mathcal{D} \to \mathcal{C}$，使得我们有自然同构$\eta: \mathrm{Id}_\mathcal{C} \to G \circ F$，称为**unit**，以及自然同构$\epsilon: F \circ G \to \mathrm{Id}_\mathcal{D}$，称为**counit**。
如果$(F, G)$组成一对adjunction，记它们为$F \dashv G$，称$F$是$G$的**left adjoint**，$G$是$F$的**right adjoint**。

显然，如果$\eta$和$\epsilon$是自然同构，那么adjunction就是范畴间的等价关系。

TODO：
$$
\mathrm{Hom}_\mathcal{D}(FX, -) \simeq \mathrm{Hom}_\mathcal{C}(X, G-)
$$
...
| term | 态射 |
| type | 对象 |
| 从类型$T$中取出一个term$a$ | 构造态射$1 \stackrel{a}{\longrightarrow} T$ （从而一般来说需要有一个final object） |

## 笛卡尔闭范畴

本节给简单typed lambda calculus在范畴$\mathcal{C}$中提供一个语义。单纯的范畴里面只能从$X \to Y$和$Y \to Z$得到$X \to Z$，并且全体态射组成的集合显然不是原范畴中的对象。因此，我们需要以下额外结构：

- 能够做到“从类型$T$中取出一个term$a$”，从而需要有一个final object
...
- 函数也是term：现在我们实际上有两种函数，一种是$A \to B$，一种是$1 \to B^A$，它们实际上是一样的——$\mathrm{Hom}(1, B^A) \simeq \mathrm{Hom}(A, B)$，具体的同构关系是这样的：设$1 \stackrel{g}{\longrightarrow} Y^X$，则有
  $$
