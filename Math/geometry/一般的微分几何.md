...
设$\phi: M \to N$是一个从流形$M$到流形$N$的映射，$\mathcal{F}_M(k, l)$和$\mathcal{F}_N(k, l)$分别是$M$和$N$上的$(k, l)$型张量场的集合。
则从$\phi$可以得到映射$\phi^*: \mathcal{F}_N(k, l) \to \mathcal{F}_M(k, l)$，称为**拉回**，其定义为
$$
(\phi^* f)|_p = f|_{\phi(p)}.
$$
直观地说，拉回是定义了一个“场的坐标的坐标变换”，设$q=\phi(p)$，其中$p \in M$，$q \in N$，那么$\phi^*$大致上就是将映射关系$q \mapsto f(q)$转换为$p \mapsto f(\phi(p))$的高阶函数。
