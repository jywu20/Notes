...
$\phi^*$和$\phi_*$可以使用这种“左脚踩右脚”的方式推广到一般的张量场上。一个$(0, l)$型张量场可以使用它作用在若干个$(1, 0)$型矢量的结果定义，于是就有如下拉回
$$
...
在$\phi$是微分同胚时，拉回和推前可以认为是创建了原来的流形的一个略加扭曲的复制品，也可以理解成坐标变换。两者是等价的，即主动观点和被动观点的等价。
