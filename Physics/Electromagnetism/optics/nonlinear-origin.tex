...
现在我们获得了$\vb*{d}$，于是可以计算出$\vb*{P}$，于是进一步可以计算出$\chi^{(1)}$, $\chi^{(2)}$等等，依照定义
\[
...
在量子力学的框架下电子和光子的相互作用方式——一个电子吸收或者放出一个光子——自然地允许出现\eqref{eq:second-response-1}这样的过程；而另一方面，经典理论禁止多条外线的。
...
\subsubsection{双侧费曼图和凝聚态场论中的费曼图的一致性}
...
\subsection{密度矩阵表述}\label{sec:electron-density-matrix}

实际上，我们还可以将双边费曼图理解为关于密度矩阵的费曼图，在每个时间点，有\emph{两个}状态，分别对应密度矩阵的左矢和右矢，因此电子线包括左右两部分。
...
\end{equation}
其中$\rho^\text{eq}_{mn}$给出玻尔兹曼分布。

密度矩阵方法可以用于处理纯态，也可以用于处理热态。
