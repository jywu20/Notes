...
介质中能被称为“波”的东西当然不只有光。本节讨论光和介质中各种模式的耦合。
\autoref{sec:chi-2-wave}和\autoref{sec:chi-3-wave}给出的可以说是光子的弹性散射，而本节讨论的则是光子的非弹性散射，因为有能量被放进了介质中的其它模式，或是介质中的其它模式将能量转移给了光子。

布里渊散射，拉曼散射；

\subsection{Raman效应和受激Raman效应}

\subsubsection{经典光场和介质性能的波动}

设介质中的电子处在某种集体模式上。假定能够使用完全经典的波来描述这种集体模式，于是将$\alpha$泰勒展开到一阶，得到
\begin{equation}
...
\end{equation}
可以看到我们有三个过程：一个是普通的$\omega_l$光的传播，对应\concept{瑞利散射}，一个是\concept{Stokes过程}，即输出光频率为$\omega_l - \omega_q$，还有一个是\concept{反Stokes过程}，即输出频率为$\omega_l + \omega_q$。

Raman散射可以用一些经典图景分析。

Raman散射会产生长波光子，这有可能会淹没在荧光信号中。
...
受激Raman效应：强度变强了，荧光变弱了，灵敏度提高了
TODO：拍频
受激Raman效应包括Stokes过程和反Stokes过程，反Stokes过程就是CARS，这也是相位匹配的
