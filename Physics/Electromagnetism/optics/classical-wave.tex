...
但是实际上，得到了亥姆霍兹方程的解也就得到了时域波动方程的解，并且一些物理图像可以直接使用：一个亥姆霍兹方程乘上一个平凡的时间演化因子$\ee^{-\ii \omega t}$之后可以当成一个波动方程的解，而其中的光束可以当成一系列波包的线性叠加；由于目前我们在讨论自由理论，这些波包彼此没有相互作用，因此每个波包的运动轨迹一定是在光束内部的，从而，单独一个波包的运行轨迹也一定是沿着光线的。（但是要注意，光线是可以不止一条的，例如如果介质中折射率在变化，那么会有反射）
