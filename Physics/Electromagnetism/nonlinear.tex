    ...
    \[
        \begin{tikzpicture}
            \begin{feynhand}
                ...
                \propag [plain, mom={$\omega$}] (a) to (b); 
            \end{feynhand}
        \end{tikzpicture} = \ii q.
    \]
    由于外线代表关联函数，我们将$E(\omega)$省去了。
...
\begin{equation}
    \frac{x_1(\omega)}{E(\omega)} = \ii q \frac{\ii}{m (\omega^2 + \ii \gamma \omega - \omega_0^2)} = \frac{q / m}{\omega_0^2 - \omega^2 - \ii \gamma \omega}.
\end{equation}
一阶过程（即\autoref{fig:first-order-x3-external}）给出如下修正：
\begin{equation}
    \begin{aligned}
        \frac{x_2(\omega)}{E(\omega_1) E(\omega_2)} &= \frac{1}{2} \int \omega_1 \int \omega_2 (\ii q) (\ii q) \frac{\ii}{m(\omega_1^2 + \ii \gamma \omega_1 - \omega_0^2)} \frac{\ii}{m(\omega_2^2 + \ii \gamma \omega_2 - \omega_0^2)} \\ 
...
一个在线性光学中也会出现的现象是\concept{局域场强化}。这个修正的主要思想在于电极化$\vb*{P}$本身会产生一个电场，这个电场会叠加在外加电场上面，从而修正介质粒子感受到的局域电场。
从这个角度导出局域场强化是很容易的，不过我们采取一个更加“奇特”的方法，用自能修正和费曼图导出这件事。
