...
获得了麦克斯韦方程之后要做的就是求解，我们此处预先分析会解得什么。
当然，最为一般的解就是\autoref{sec:radiation}中的李纳-维谢尔势。
李纳-维谢尔势在近场区适用似稳场近似，而在远场区表现得就像无源电磁波。

\begin{itemize}
    \item 在远离任何源的地方，电磁场表现为电磁波。
    \item 在靠近源的地方，似稳场近似。能够做似稳场近似是因为存在一个足够大的、比磁场大很多的电流$\vb*{j}$，没有这个量的话是不能直接略去位移电流的，否则只能得到零解。
    真正的无时间变化的解肯定满足似稳场近似，这又可以分为
    \begin{itemize}
        \item 恒定电场，有稳定的电流。
        \item 静电场，没有电流。
    \end{itemize}
...
本节讨论电磁波在各种环境中的传播；电磁波的产生，即\concept{辐射}机制，留待\autoref{sec:radiation}中讨论。
...
\section{辐射}\label{sec:radiation}
