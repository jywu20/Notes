...
\paragraph{近似方案1：丢弃矢势平方项} 直接丢弃$\vb*{A}^2$项。此时如果采取辐射规范，根据$\vb*{p}$和$\vb*{A}$的可交换性，我们就有
\begin{equation}
...
在$\vb*{A}^2$项能够丢弃，但是$\vb*{A}$尚有比较大的空间变化时，

\paragraph{近似方案2：规范变换} 一开始并不丢掉$\vb*{A}^2$项，但是利用带点粒子高度定域这个特点。
