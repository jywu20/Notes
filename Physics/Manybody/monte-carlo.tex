...
我们当然也可以使用更加“连续”的方法，即给$H[\mathcal{C}]$加上一个动力学项，然后通过求解哈密顿方程来做更新。
在软物质凝聚态物理中这可以称为分子动力学模拟。分子动力学模拟的主要问题在于，自动生成的微分方程数值格式通常没法保证能量守恒，从而出现“模拟溶液系统，计算着计算着水自己开了”的荒诞结果。
但如果我们只是做统计系统模拟，其实能量守恒也不那么重要。

\begin{enumerate}
    \item $\ee^{- \frac{p^2}{2m}}$，直接采样
    \item 如果能量
\end{enumerate}
