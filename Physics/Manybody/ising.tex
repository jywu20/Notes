...
\[
    ...
    \end{aligned}
\]
在这里$S_{d-1}(\Lambda)$是$d-1$维超球面的面积，第二行积分式求导之后得到的结果中我们将$\Lambda / \zeta$替换为了$\Lambda$，第三行中我们用$g$代替了$g_0$，在$\epsilon$很小时这是可以的（如果$d$远大于$4$，那$g$和$g_0$之间的差异就不受小量控制了，误差会很大），也用$\mu$代替了$\mu_0$。
这些近似并没有什么问题，因为重整化参数跑动\eqref{eq:coupling-running-2}也只算到了二阶近似。
对$g$类似地可以得到
\[
    \begin{aligned}
