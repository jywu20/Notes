\chapter{较为寻常的金属和绝缘体的能带论描述}\label{chap:conventional-metal}
...
以上所有的做法都有浓重的半经典色彩，从本节开始我们在场论框架下比较系统地做计算。
...
\begin{itemize}
    ...
    \end{equation}
    \item 相互作用顶角为（为了节约符号，有时候我们会将$(\vb*{q}, \omega_n)$这样的四元组简写为$q$，其中$\omega_n = p^0$。）\input{metal/jellium/vertex.tex}
...
设对某个费米型准粒子系统，准粒子之间的相互作用强度不非常强地依赖于动量，不妨记作$M$（纯粹的库伦相互作用不满足这个条件，它强烈依赖入射电子的动量差，因此即使在费米面附近，相互作用强度也不能当作常数，因此能带电子加入库伦相互作用后并不严格就是稳定的准粒子，而仍然需要经过几轮重整化，让$M$变得“柔和”一些）。
...
最后，假定我们通过若干轮重整化群计算，已经得到了一种有费米型自旋$1/2$的准粒子，且符合费米液体的第二个假设的有效理论，我们分析朗道费米液体理论的适用条件，实际上就是要估算动量确定的准粒子的寿命，从而确定第一个假设是否成立。
从上面的论述可以看出，只有在费米面附近第一个假设才是严格成立的，在稍微远离费米面的地方，准粒子之间仍然有碰撞，因此费米液体理论的适用条件就转化成了这样一个问题：动量确定的准粒子寿命是否长到两次\emph{热涨落}发生的时间差？
...
现在列写玻尔兹曼方程。设粒子分布函数为$n_{\vb*{k} \alpha}(\vb*{r}, t)$，玻尔兹曼方程为
\begin{equation}
...
\end{equation}
这个方程形式上和普通的玻尔兹曼方程一样，但是$\epsilon$和$n$之间存在关系\eqref{eq:fermi-liquid-single-energy}（注意此时$\epsilon_{\vb*{k} \alpha}$也有$\vb*{r}$的依赖）。
有时我们将它称为\concept{朗道方程}。
朗道动理学方程的稳态或者说费米液体的热力学基态当然形式上和费米气体是完全一样的
\begin{equation}
...
我们采用弛豫时间近似，即
\begin{equation}
    \pdv{n_{\vb*{k} \alpha}}{t} + \pdv{n_{\vb*{k} \alpha}}{\vb*{r}} \cdot \pdv{\epsilon_{\vb*{k} \alpha}}{\vb*{k}} + \pdv{n_{\vb*{k} \alpha}}{\vb*{k}} \cdot \left( \vb*{F} - \pdv{\epsilon_{\vb*{k} \alpha}}{\vb*{r}} \right) = - \frac{n_{\vb*{k} \alpha}}{\tau},
    \label{eq:landau-equation-damping}
\end{equation}
由于弛豫是粒子碰撞导致的，$\tau \propto T^{-2}$。
\eqref{eq:landau-equation-damping}描述的系统中能够传播的波大致上可以分成两种。
...
而当$\omega \tau \gg 1$时，一幅非常奇怪的图景出现了：此时弛豫项几乎可以忽略，因此，外界扰动而产生的$n_{\vb*{k} \alpha}$涨落传播到的地方\emph{不会}发生任何弛豫现象，无法达到热平衡，且波动\emph{不会}受到任何阻尼。
此时的波动称为\concept{零声}，因为它看起来很像声波，但首先频率通常高于普通的声波，其次通常需要$\tau$比较大才能观察到，即需要在非常接近零温的系统中才能观察到。
...
因此，零温下的朗道方程——实际上就是粒子数的经典运动方程——和粒子数期望的严格运动方程之间实际上只相差一些由于交换算符而产生的$\delta$项，通常这意味着粒子数\emph{算符}满足的海森堡绘景运动方程的形式和零温朗道方程是一样的。
由于朗道方程代入$\epsilon_{\vb*{k} \alpha}$的定义之后是自洽的，这实际上是说，我们可以通过观察零温下的朗道方程就得到只关于朗道费米液体的密度涨落的等效哈密顿量，而且是货真价实的、是算符的、做了勒让德变换之后能够出现在路径积分中的哈密顿量。
对那些只有密度涨落才是重要的的系统，实际上我们确实只需要密度涨落的等效哈密顿量。
因此通过观察零温下的朗道方程，我们实际上可以将费米液体玻色化。

在零温下\eqref{eq:landau-equation-damping}完全就是
