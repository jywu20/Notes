...
或者等价地可以设
\begin{equation}
    {f}(\vartheta) = f^\text{S}(\vartheta) + \vb*{\sigma} \vb*{\sigma}' f^\text{A}(\vartheta)
\end{equation}
...
现在列写玻尔兹曼方程。设单粒子分布函数为$n_{\vb*{k} \alpha}(\vb*{r}, t)$，玻尔兹曼方程为
\begin{equation}
...
注意此时$\epsilon_{\vb*{k} \alpha}$也有$\vb*{r}$的依赖，且\eqref{eq:fermi-liquid-single-energy}中的二次量子化粒子数算符$\var{n_{\vb*{k}' \beta}}$被单粒子分布函数偏离平衡态的量$\var{n_{\vb*{k}' \beta}}$取代。
有时我们将\eqref{eq:landau-fermi-liquid-equation}称为\concept{朗道方程}。
...
由于只考虑近平衡态，我们将\eqref{eq:landau-equation-damping}中的$n_{\vb*{k} \alpha}$展开到平衡态附近的一阶，即考虑
\begin{equation}
    \var{n_{\vb*{k} \alpha}} = n_{\vb*{k} \alpha} - n_{\vb*{k} \alpha}^0
\end{equation}
的演化方程。注意到$n_{\vb*{k} \alpha}^0$不依赖于时间和坐标，而$\epsilon_{\vb*{k} \alpha}$不依赖于坐标，仅保留正比于$\var{n}_{\vb*{k} \alpha}$的项，得到
\begin{equation}
...
我们来计算$T=0$时的零声模式，此时\eqref{eq:delta-from-stable-fermi-liquid}右边为零，并考虑一个$f_{\alpha \beta \vb*{k} \vb*{k}'}$中无自旋依赖的系统。
为方便起见我们将$\vb*{k}$给定而携带一个自旋指标的分布函数记作$\hat{n}_{\vb*{k}}$，同理$f$记作$\hat{f}_{\vb*{k} \vb*{k}'}$。
我们有
\[
    \pdv{n^0_{\vb*{k}}}{\vb*{k}} = - \delta(\epsilon_{\vb*{k}} - \epsilon_\text{F}) \vb*{v}, \quad  \pdv{\epsilon_{\vb*{k}}^0}{\vb*{k}} \approx \vb*{v}, \quad \var{\epsilon_{\vb*{k}}} = \frac{1}{V} \sum_{\vb*{k}'} \hat{f}_{\vb*{k} \vb*{k}'} \cdot \var{\hat{n}_{\vb*{k}'}} = \int \frac{\dd[3]{\vb*{k}'}}{(2\pi)^3} \hat{f}_{\vb*{k} \vb*{k}'} \cdot \var{\hat{n}_{\vb*{k}'}},
\]
这里我们使用惯常的群速度定义
\begin{equation}
...
\begin{equation}
    \pdv{\var{\hat{n}_{\vb*{k}}}}{t} + \vb*{v} \cdot \pdv{\var{\hat{n}_{\vb*{k}}}}{\vb*{r}} + \delta(\epsilon_{\vb*{k}} - \epsilon_\text{F}) \vb*{v} \cdot \int \frac{\dd[3]{\vb*{k}'}}{(2\pi)^3} \hat{f}_{\vb*{k} \vb*{k}'} \cdot \pdv{\var{\hat{n}_{\vb*{k}'}}}{\vb*{r}} = 0.
...
此外，由于我们只关心费米面附近的行为，可以认为$\vb*{k}$提供的信息主要是关于其指向而不是其大小的（大小就是$k_\text{F}$），从而
\begin{equation}
    \var{\hat{n}_{\vb*{k}}}(\vb*{r}) = \ee^{\ii (\vb*{k} \cdot \vb*{r} - \omega t)} \delta(\epsilon_{\vb*{k}} - \epsilon_\text{F}) \hat{\nu}(\vu*{n}).
\end{equation}
