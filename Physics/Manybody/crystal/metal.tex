...
Hartree项和Fock项显然分别来自两种自能图，前者来自蝌蚪图而后者来自。% TODO：这个叫做什么图啊
...
这个说法其实并不是非常严谨，因为Hartree-Fock近似下的能量显然也考虑了一些电子关联效应。
...
本节讨论高密度电子气的行为。对这一类系统Hartree-Fock近似失效。
\concept{凝胶模型}，这是一种将库伦散射不做任何处理地当成微扰，并且将金属中的原子当成均匀的正电荷凝胶的金属模型。
前一个假设——微扰论适用——实际上是不正确的，因为库仑相互作用能的量级和电子动能是一样的，但是实验上，金属和近独立电子气的行为确实很相似（对这件事的解释和讨论见\autoref{sec:landau-fermi-liquid}），因此至少非微扰效应不会让系统基态相比于近独立电子气面目全非，做适当的费曼图重求和修正电子能带应该能够有足够好的精度。
...
由于可以忽略交换作用，可以完全照搬经典静电学，即认为一个电子的全部能量是其动能加上电势能，而不考虑任何交换能或者关联能，因为直观地看，交换能或者说Fock项真的对应一个两个电子交换位置的费曼图，但是既然电子如此稠密，这样的过程大概是不太能够发生的。
