...
本节讨论的系统满足所谓\concept{托马斯-费米近似}：在其中，电子充分稠密，从而相互作用能非常强，电子凭借其自身的动能无法远距离传播，从而系统状态可以使用电子数密度$n(\vb*{r})$标记，且相互作用能可以完全认为是来自密度-密度相互作用即Hartree项，而忽略交换作用等除此以外的效应。
...
玻尔兹曼方程（以及很多其它动理学方程）是格林函数运动方程的近似，玻尔兹曼方程中的单电子分布函数实际上是单电子格林函数（“单电子密度矩阵”）的一种简化，因此\eqref{eq:fermi-liquid-energy}中的第二项应该通过计算电子自能加以考虑。
\eqref{eq:fermi-liquid-energy}中的第二项在动量空间中给出
\input{metal/flt/vertex.tex}
由于是前向散射，入射出射电子动量不变，虚线上没有三维动量交换，只有松原频率交换。
它给出的全部正规自能图可以概括为
\input{metal/flt/self-energy.tex}
其中闭合电子回线意味着$\ii \omega_n = 0$，$-1$因子是相互作用顶角上的，闭合电子回线由于给出了$\expval*{c^\dagger c}$，不贡献$-1$因子；
因此加入了自能修正的单电子能量就是
\begin{equation}
    \epsilon_{\vb*{k} \alpha} = \epsilon_{\vb*{k}}^0 + \frac{1}{V} \sum_{\beta, \vb*{k}'} f_{\alpha \beta \vb*{k} \vb*{k}'} \expval*{n_{\vb*{k}' \beta}}.
...
注意到我们有
\begin{equation}
    n_{\vb*{k}, \alpha \beta} = \frac{1}{\ee^{\beta \epsilon_{\vb*{k}}} + 1},
\end{equation}
