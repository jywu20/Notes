...
\end{equation}
这些现象对应的序参量对应于Hubbard-Stratonovich参量（见后文）的不同选择；换句话说，不同的相互作用通道相互竞争，最终哪一种相互作用通道占据优势——从而，单单考虑它得到的相变出现——取决于很多条件。
...
\begin{figure}
    ...
    \caption{BCS理论中最为重要的相互作用通道}
\end{figure}

\subsection{库伯对}

\subsubsection{二电子配对的稳定性}
...
\subsection{s波BCS超导的平均场近似和Bogoliubov变换}
...
\end{equation}
我们在这里相当于做了两步近似，首先是假定只有相互作用通道
\begin{equation}
    H_\text{int} = - V_0 \sum_{\vb*{k}, \vb*{k}'} c^\dagger_{- \vb*{k}' \downarrow} c^\dagger_{\vb*{k}' \uparrow} c_{\vb*{k} \uparrow} c_{- \vb*{k} \downarrow}
\end{equation}
才是重要的，即实际上是在研究模型
\begin{equation}
...
\end{equation}
这个近似是合理的，因为在低能极限下，只有自旋相反的电子才会碰撞，然后对这个哈密顿量做平均场分解。这里我们没有再将$1/2$因子加入，因为，如果将$c_\uparrow$和$c_\downarrow$看成两种类型的场，则没有任何必要加入对称性因子。
% TODO：合理性
...
\end{equation}
不过实际上以上公式自身意义不大，因为首先很难精确计算$V_0$，其次$V_0$的微小变化会带来很大误差。
真正会在实验上验证的通常是
\begin{equation}
...
\begin{back}{金斯堡-朗道理论和Hubbard-Stratonovich变换}{gl-hubbard-stratonovich}
    ...
    如果$\lambda$选择适当，其涨落不是很大，且非零，那么系统就有对称性自发破缺，算符$\mathcal{O}[\psi]$大体上就是序参量，做完Hubbard-Stratonovich变换之后得到的关于$\lambda$的场论就是一个好的金斯堡-朗道理论。
...
\begin{figure}
    ...
    \caption{\eqref{eq:superconductivity-opposite-mom}做完Hubbard-Stratonovich变换后的相互作用顶角}
...
考虑最为一般的吸引相互作用
\begin{equation}
    S = \int_0^\beta \dd{\tau} \left( \sum_{\vb*{k}, \alpha} \bar{c}_{\vb*{k} \alpha} (\partial_\tau + \xi_{\vb*{k}}) c_{\vb*{k} \alpha} - \frac{1}{V} \sum_{\vb*{k}, \vb*{k}', \vb*{q}} g_{\vb*{q}} \bar{c}_{-\vb*{k}' \downarrow} \bar{c}_{\vb*{k}' + \vb*{q}, \uparrow} c_{\vb*{k} + \vb*{q}, \uparrow} c_{- \vb*{k} \downarrow} \right),
\end{equation}
做Hubbard-Stratonovich变换得到
\begin{equation}
    \begin{aligned}
        Z &= \int \fd{[\Delta, \bar{\Delta}]} \fd{[c, \bar{c}]} \exp\Bigl( - \int \dd{\tau} \Bigl( \sum_{\vb*{k}, \alpha} \bar{c}_{\vb*{k} \alpha} (\partial_\tau + \xi_{\vb*{k}}) c_{\vb*{k} \alpha} + \sum_{\vb*{q}} \frac{1}{g_{\vb*{q}}} \bar{\Delta}_{\vb*{q}} \Delta_{\vb*{q}} \\
...
配分函数写作
\[
    Z = \int \fd{[\Psi, \bar{\Psi}]} \fd{[\Delta, \bar{\Delta}]} \exp(- \int \dd{\tau} \left( \sum_{\vb*{q}} \frac{1}{g_{\vb*{q}}} \bar{\Delta}_{\vb*{q}} \Delta_{\vb*{q}} + \bar{\Psi} (\partial_\tau + h) \Psi \right) ),
\]
其中
\begin{equation}
...
积掉电子场，得到
\begin{equation}
    Z = \int \fd{[\Psi, \bar{\Psi}]} \fd{[\Delta, \bar{\Delta}]} \exp(- \int \dd{\tau} \sum_{\vb*{q}} \frac{1}{g_{\vb*{q}}} \bar{\Delta}_{\vb*{q}} \Delta_{\vb*{q}} + \ln \det (\partial_\tau + h) ).
\end{equation}
...
\begin{equation}
    S_{\Delta^2} = \sum_{q} \left( \frac{1}{g_{\vb*{q}}} + \frac{1}{V \beta} \sum_k G^\text{e}_{k+q} G^\text{h}_k \right) \bar{\Delta}_{q} \Delta_{q} = \sum_{q} \left( \frac{1}{g_{\vb*{q}}} - \frac{1}{V \beta} \sum_k G^\text{e}_{k+q} G^\text{e}_{-k} \right) \bar{\Delta}_{q} \Delta_{q},
\end{equation}
其中
\begin{equation}
    G^\text{h}_{k} = \frac{1}{\ii(- k^0) - \xi_{- \vb*{k}}} = - G^\text{e}_{-k}.
\end{equation}
我们现在切换回实空间。前面说过，能量最低的场构型中$\Delta$在时间和空间上都是均匀的，即参与库伯配对的两个电子动量基本上只差一个负号，从而，基态附近的$\vb*{q}$通常都很小，类似的$q^0$也很小，这样我们将$1 / g_{\vb*{q}}$项对$\vb*{q}$展开，由于BCS理论中有各向同性，得到
\[
    \frac{1}{g_{\vb*{q}}} = \frac{1}{g_0} + \frac{1}{6} \eval{\laplacian_{\vb*{q}} \frac{1}{g_{\vb*{q}}}}_{\vb*{q}=0} \abs{\vb*{q}}^2 + \cdots,
\]
而
\[
    - \frac{1}{V \beta} \sum_{k} G^\text{e}_{k+q} G^\text{e}_{-k} = - \frac{1}{V \beta} \sum_k G^\text{e}_{k} G^\text{e}_{-k} - \ii q^0 \frac{1}{V \beta} \sum_k \eval{\pdv{G^\text{e}_{k+q}}{(\ii q^0)}}_{q=0} G^\text{e}_{-k} - \abs{\vb*{q}}^2 \frac{1}{6 V \beta} \sum_k \eval{\laplacian_{\vb*{q}} G^\text{e}_{k+q}}_{q=0} G^\text{e}_{- k} ,
\]
