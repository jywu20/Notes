...
动能泛函通常比交换关联泛函还要难以写出。的确，可以仿照计算均匀电子气交换关联泛函的方法计算均匀电子气的动能泛函，但是直接把这些动能泛函拿去算固体系统效果是不太好的。% 交换关联泛函不需要考虑偏离均匀电子气很多的情况就够用这一点是由一些理论保证的
...
\begin{info}{Hubbard模型的密度泛函理论}{dft-hubbard}
...
\begin{back}{量子蒙特卡洛方法}{quantum-monte-carlo}
    ...
    \]
    那么只需要真的运行这个随机过程足够长的时间，就能够得到系统中各个构型的概率分布，从而可以计算各个物理量的期望、方差等一切信息。
...
另一个LDA泛函的局限性在于它存在\concept{自相互作用}：Hartree项假定库伦相互作用可以写成\eqref{eq:dft-hartree-term}的形式，那么如果系统基态有电子的波函数同时在$\vb*{r}$和$\vb*{r}'$处都有可观的振幅，\eqref{eq:dft-hartree-term}就会导致一个奇怪的“电子自己和自己相互作用”的能量。
...
此外，Hartree-Fock泛函由于是诸$\phi_i(\vb*{r})$组成的Slater行列式态。
...
\begin{info}{交换关联泛函有关的理论研究文献}{ex-functional-literature}
    见\cite{Pribram_Jones_2015}
\end{info}
