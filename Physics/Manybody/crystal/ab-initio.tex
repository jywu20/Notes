...
\subsubsection{单电子诠释}\label{sec:single-electron-in-dft}
...
对晶体的计算可以分成这么几个步骤：
\begin{enumerate}
    \item 定义晶体的几何结构，通常是通过指定一个晶胞形状，并且在晶胞内给出各个原子核的位置，这些信息——晶格矢量和晶胞内各个原子核的位置——称为\concept{超晶胞（supercell）}，通常要求超晶胞取最简单的形式，即为原胞；
    \item 
\end{enumerate}
