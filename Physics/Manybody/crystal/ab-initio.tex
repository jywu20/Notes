...
本节讨论一种常用的求解这些准粒子的方法：\concept{密度泛函理论（DFT）}。
...
\subsection{后处理和物理量测量}

本节讨论Kohn-Sham方程求解完成后怎样从计算结果解读出我们需要的各个物理量。

\subsubsection{和基态电子密度有解析形式关系的量}

基态能量实际上就是能量泛函作用在基态电子密度$\rho(\vb*{r})$之后的结果。

\subsubsection{单电子诠释和能带}\label{sec:single-electron-in-dft}
...
总之，只要交换关联泛函的质量足够好，我们就得到了外层电子或是全电子的模拟结果：这其中没有引入任何经验参数，并且每一条可能的能带都纳入了考虑。
