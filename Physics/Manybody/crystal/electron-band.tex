...
本节分析能带电子，所谓能带电子通常是在两个假设下计算出来的：首先是\concept{静态近似}，即暂时不考虑晶格振动，只是将晶格当成提供了周期性势场的一个背景；其次是\concept{近独立近似}，由于很难一上手就处理带有复杂相互作用的电子气，因此我们首先处理\concept{近独立电子气}，也就是电子之间近似没有相互作用的电子气。
静态近似需要使用电子-声子相互作用修正。
...
\paragraph{费米面} 没有什么保证费米面一定是第一布里渊区中的一个封闭曲面。它可以和第一布里渊区边界相交，从而和另一侧的边界上的自己相连。
...
\subsubsection{不同表象下近独立电子哈密顿量的形式}

现在我们有两种表示能带的方法，一种是Bloch基底下，用$\vb*{k}$做标签的表示法，一种是Wannier基底下用\eqref{eq:hamiltonian-in-wannier}表示的。
下面我们来分析\eqref{eq:hamiltonian-in-wannier}中的跃迁系数$t_{mi, nj}$具体是什么。
实际上是没有一种唯一的诠释方式的，因为\eqref{eq:hamiltonian-in-wannier}中的$i, j$等只需要取值范围正好是$1$到$N$，并且满足空间平移不变性就可以。

\paragraph{能带内跃迁} 我们可以不失一般性地取$m = n$，即将哈密顿量在这个标签上做对角化，此时有
\begin{equation}
...
在空间平移对称性成立，但是紧束缚近似未必成立时，我们用$t_l$表示从格点$\vb*{R}_i$到$\vb*{R}_i + \vb*{R}_l$的跃迁系数。
...
\]
从坐标表象切换到Wannier波函数相当于首先完成了在原胞内的积分，然后从Wannier波函数的表象（即\eqref{eq:multi-band-tight-binding}）计算能带就是完成了剩下的对$i$的求和，即\eqref{eq:lattice-fourier}。
...
\end{equation}
得到对应的跃迁系数。因此，给定一个能带之后，我们其实总是可以写出一个\eqref{eq:hamiltonian-in-wannier}形式的哈密顿量来复刻这个能带。

请注意上面的推导完全没有用到一个晶胞里面有几种原子之类的条件：$i, j$等都是\emph{晶格格点}，而不是具体的原子的位置。
对简单晶格，这是非常合理的，而对复式晶格，我们的推导给出了一种把它近似地看成简单晶格的方法。

\paragraph{含能带间跃迁的模型} 实际上，我们甚至可以胡乱指派一个其实并非我们的体系的能量本征态的Wannier波函数及其对应的Bloch波函数$\psi_{n \vb*{k}}$，然后根据\eqref{eq:sqrt-v-rep}写出一个修正后的紧束缚模型，不过这样就会有带间跃迁了；或者，如果这胡乱指派的Wannier波函数实际上更加接近单个原子周围的轨道，我们需要一步将单个原子周围的轨道做。
由于此时的Wannier波函数其实不正确，写出这种含有带间跃迁的紧束缚模型之后做的对角化将会重新设置能带标签。

这种稍微劣质一些的Wannier波函数可以取为单原子的轨道波函数，也可以通过线性组合单原子轨道波函数得到，需要线性组合是因为原子中的电子受其它原子作用而发生能级重组，即轨道杂化。
甚至还可以计算单个原胞内的成键轨道和反键轨道，然后把成键轨道和反键轨道当作Wannier波函数。
这和坐标表象、$i$表象和晶格动量表象之间的关系很像：从坐标表象到单原子轨道相当于稍稍对哈密顿量做了对角化，从单原子轨道到有杂化的轨道相当于又做了一些对角化，到最后得到正确的能带编号，就是完成了能带标签部分的对角化。

\paragraph{表象变换} 总结一下，形如\eqref{eq:hamiltonian-in-wannier}的近独立电子模型最大的好处在于提供了介于哈密顿量完全对角化的Bloch波函数表象（或者说晶格动量表象）和完全没有做过任何处理的坐标表象之间的许多表象。
...
从上到下，坐标变得模糊而动量变得清晰：先是单原子波函数的细节模糊了，然后是电子在晶格内的哪个原子附近（或者说，复式晶格中在哪套子格子附近）变得模糊了，最后电子在哪个原胞附近也模糊了，而$\vb*{k}$变得清晰了。
...
\subsubsection{跃迁系数的计算}

本节讨论给定了一种Wannier波函数之后要如何计算\eqref{eq:hamiltonian-in-wannier}中的跃迁系数。
...
\paragraph{热力学性质} 我们现在计算有限温下自由电子气的一些热力学性质。
...
\subsection{原子轨道组合和紧束缚模型}

\subsubsection{单电子模型及其跃迁系数}\label{sec:tight-binding-hopping}

\paragraph{简单晶格的原子轨道线性组合（LCAO）} 在简单晶格中，如果不同原子的轨道基本上没有重叠，那实际上单原子轨道波函数就是Wannier波函数。
设不同原子的轨道没有什么重叠，则有
\begin{equation}
