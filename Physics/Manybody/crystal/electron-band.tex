...
\begin{figure}
    \centering
    \subfigure[对简单晶格给定一组波函数$v(\vb*{r} - \vb*{R}_i)$，彼此有很强的交叠，即并不正交]{
...
\subsubsection{从任意的局域波函数获得Wannier波函数}
...
\end{equation}
也是正交归一化的。% TODO：检查归一化常数

由于$A_{\vb*{k}}$存在动量依赖，$w_i(\vb*{r})$和$v(\vb*{r} - \vb*{R}_i)$未必是一样的。
在$v(\vb*{r})$已经足够局域的时候，
