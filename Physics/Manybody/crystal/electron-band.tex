...
很难一上手就处理带有复杂相互作用的电子气，因此我们首先处理\concept{近独立电子气}，也就是电子之间近似没有相互作用的电子气。本节给出近独立电子气的一些一般性质。
...
在具体地引入周期势场之前，我们通过对称性分析来观察我们会得到什么样的结果。
...
\]
另一方面，第一布里渊区内部的矢量彼此的差值不可能达到一个倒格子格矢的大小，因此第一布里渊区内部的$\vb*{k}$不会导致重复的波函数。于是不失一般性地我们可以要求$\vb*{k}$在第一布里渊区内部；实际上我们可以要求$\vb*{k}$在任何一个倒空间的原胞内部。
...
\end{equation}
显然，$n$是另一个（离散的）好量子数。通过数自由度我们可以看出$n$对应着什么：$\vb*{k}$对应着晶格，从而$n$对应着一个原胞中不同的原子以及每个原子周围的电子轨道。
...
我们还没有看到周期势场具体是怎样让能带产生的，以及能带编号$n$到底标记了哪些自由度。
