...
\begin{info}{电子自能}{electron-self-energy}
...
\begin{info}{近独立费米气体}{free-fermi-gas}
...
\[
    {H} = \sum_{mi, nj} {c}^\dagger_{mi} \mel{mi}{\hat{h}}{nj} {c}_{nj},
\]
其中我们令
\begin{equation}
...
\begin{equation}
    t_{l} = t_{-l}^*.
\end{equation}
我们这里稍微滥用一下记号，用$i+l$表示正格矢$\vb*{R}_i + \vb*{R}_l$的编号。
...
\[
    \frac{1}{\sqrt{V}} \int \dd[3]{\vb*{r}} \psi_{n \vb*{k}}(\vb*{r})^* = \frac{1}{\sqrt{N}} \sum_i \ee^{- \ii \vb*{k} \cdot \vb*{R}_i} \frac{1}{\sqrt{V_\text{u.c.}}} \int_\text{u.c.} \dd[3]{\vb*{r}} u_{n \vb*{k}}(\vb*{r})^* ,
\]
...
这意味着给定一个形如\eqref{eq:hamiltonian-in-wannier}的哈密顿量，对$c_{mi}^\dagger$做傅里叶变换\eqref{eq:lattice-fourier}，\eqref{eq:hamiltonian-in-wannier}对角化，这个过程产生的傅里叶变换中的$\vb*{k}$实际上就是直接从周期势计算Bloch波函数而产生的那个简约波矢$\vb*{k}$。%
\footnote{
...
反过来，将电子的产生湮灭算符$c_{mi}^\dagger$做傅里叶变换\eqref{eq:lattice-fourier}，\eqref{eq:hamiltonian-in-wannier}即可写成对角化的，立刻可以看出能带。
...
\begin{figure}
    \centering
    \subfigure[对单带简单晶格模型给定一组波函数$v(\vb*{r} - \vb*{R}_i)$，不同$i$的波函数之间有很强的交叠，即并不正交；不同的颜色用于区分不同原子]{
        \includegraphics[width=0.4\textwidth]{band/hopping/overlap.png}
        \label{fig:simple-lattice-ortibal}
    }
    \subfigure[根据$v(\vb*{r} - \vb*{R}_i)$做正交化，得到一组Wannier波函数，以此为基底时，波函数本身的空间分布的细节部分被抹去，只留下“电子在哪个格点附近”的粗略信息；一些Wannier态之间容易发生跃迁（用较深的颜色表示），另一些Wannier态之间的跃迁不容易发生（用较浅的颜色表示）]{
...
首先设我们有一组大体上定域在各个原子附近的波函数（对简单晶格如\autoref{fig:simple-lattice-ortibal}所示，对复式晶格如\autoref{fig:complex-lattice-ortibal}所示），记作$v_{n}(\vb*{r} - \vb*{R}_i)$，这里$i$标记它定域在的晶胞的位置，$n$提供进一步的信息，如它是一个晶胞中的哪一种原子附近的哪一个轨道，或者是哪两个原子之间的成键轨道或是反键轨道，等等。
同一个$i$，不同$n$的$v_n(\vb*{r} - \vb*{R}_i)$之间的内积通常是零，但是不同$i$的$v_{n}(\vb*{r} - \vb*{R}_i)$内积通常不是零（\autoref{fig:simple-lattice-ortibal}直观地展示了这一点），需要对不同$i$的$v_{n}(\vb*{r} - \vb*{R}_i)$做正交化，让它们变得更加尖锐，这就得到了Wannier波函数（如\autoref{fig:simple-lattice-hopping}和\autoref{fig:complex-lattice-hopping}中的圆点所示），据此可以将哈密顿量写成\eqref{eq:hamiltonian-in-wannier}的形式（见\autoref{fig:simple-lattice-hopping}和\autoref{fig:complex-lattice-hopping}中的跃迁箭头）。
正交化不会混合不同的$n$，因此大体上可以认为标签$i$和$n$在正交化前后没有变。
...
在给定一组$v_n(\vb*{r} - \vb*{R}_i)$之后我们尝试做正交化。由于不同$n$的$v_{n}(\vb*{r} - \vb*{R}_i)$已经是正交的，不需要变动这个标签；我们下面暂时略去$n$。
首先定义
\begin{equation}
    \phi_{\vb*{k}}(\vb*{r}) = \frac{A_{\vb*{k}}}{\sqrt{N}} \sum_i \ee^{\ii \vb*{k} \cdot \vb*{R}_i} v(\vb*{r} - \vb*{R}_i),
\end{equation}
根据归一化条件
\[
...
可以计算得到
\begin{equation}
    \frac{1}{\abs*{A_{\vb*{k}}}^2} = \sum_i \ee^{\ii \vb*{k} \cdot \vb*{R}_i} \int \dd[3]{\vb*{r}} v(\vb*{r})^* v(\vb*{r} - \vb*{R}_i).
\end{equation}
我们会发现$\{\phi_{\vb*{k}}(\vb*{r})\}$是正交归一化的，因此对它们做反向的傅里叶变换得到的
\begin{equation}
    w_{i}(\vb*{r}) = \frac{1}{\sqrt{N}} \sum_{\vb*{k}} \ee^{- \ii \vb*{k} \cdot \vb*{R}_i} \phi_{\vb*{k}}(\vb*{r}) 
\end{equation}
也是正交归一化的；不过此时我们有
\begin{equation}
    \delta_{ij} = \int \dd[3]{\vb*{r}} w_i(\vb*{r})^* w_j(\vb*{r}),
\end{equation}
和\eqref{eq:wannier-function-normalization-uc}有所不同，在本节的习惯下面应有
\begin{equation}
    - t_{mi, nj} = \mel{mi}{\hat{h}}{nj} = \int \dd[3]{\vb*{r}} w_m^*(\vb*{r} - \vb*{R}_i + \vb*{R}_j) \hat{h} w_n(\vb*{r}).
\end{equation}

由于$A_{\vb*{k}}$存在动量依赖，$w_i(\vb*{r})$和$v(\vb*{r} - \vb*{R}_i)$未必是一样的——数学上可以证明$w_i(\vb*{r})$比$v(\vb*{r} - \vb*{R}_i)$更加局域。
在$v(\vb*{r})$已经足够局域的时候，
\begin{equation}
    \int \dd[3]{\vb*{r}} v(\vb*{r} - \vb*{R}_i)^* v(\vb*{r} - \vb*{R}_j) = \delta_{ij},
\end{equation}
此时$w_{i}(\vb*{r})$就是$v(\vb*{r} - \vb*{R}_i)$。

\subsubsection{一般的Wannier表象下的哈密顿量及其对角化} 

现在我们考虑基底$\{w_{ni}(\vb*{r}) \}$下的哈密顿量及其对角化。形式地我们总是可以写出
\begin{equation}
    - \frac{\laplacian}{2m} w_{ni} + U(\vb*{r} - \vb*{R}_i) w_{ni} = \epsilon_{n} w_{ni},
\end{equation}
不过要注意此时$U$未必在坐标表象下是对角化的；$U$与晶体的周期势场$V(\vb*{r})$需要区分。
由于此时有不同的$n$之间的跃迁，我们使用记号$t_{mn, l}$表示$t_{m(i+l), ni}$。
跃迁系数计算为
\[
    \begin{aligned}
        - t_{mn, l} &= \int \dd[3]{\vb*{r}} w_{m(i+l)}(\vb*{r})^* \left( - \frac{\laplacian}{2m} + V(\vb*{r}) \right) w_{n i}(\vb*{r}) \\
        &= \int \dd[3]{\vb*{r}} w_{m(i+l)}(\vb*{r})^* \left( \epsilon_n - U(\vb*{r} - \vb*{R}_{i}) + V(\vb*{r}) \right) w_{ni}(\vb*{r}),
    \end{aligned}
\]
第一项只有在$l=0$时才有非零值，是$\epsilon_n$；第二项和第三项可以使用周期性写成
\[
    \begin{aligned}
        &\quad \int \dd[3]{\vb*{r}} w_{m (i+l)}(\vb*{r})^* \left( - U(\vb*{r} - \vb*{R}_{i}) + V(\vb*{r}) \right) w_{n i}(\vb*{r}) \\
        &= \int \dd[3]{\vb*{r}} w_{m}(\vb*{r} - \vb*{R}_l)^* \left( - U(\vb*{r}) + V(\vb*{r}) \right) w_{n}(\vb*{r}),
    \end{aligned}
\]
因此我们有
\begin{equation}
...
而
\begin{equation}
    - t_{mn, l} = \int \dd[3]{\vb*{r}} w_{m}(\vb*{r} - \vb*{R}_l)^* \left( V(\vb*{r}) - U(\vb*{r}) \right) w_{n}(\vb*{r}), \quad \text{$m \neq n$ or $l \neq 0$}.
    \label{eq:off-diagnoal-hopping}
\end{equation}

我们下面要求$i$仅仅标记晶胞位置，晶胞内部有不同原子，每个原子周围有不同形状的Wannier波函数等事实全部使用$n$标记；这样$i$的自由度数目和$\vb*{k}$一致，这里的对角化过程相比直接从Bloch波函数获得的Wannier波函数表象下的对角化过程，唯一多出来的就是重新安排能带编号。
因此，如果我们采用这样的方案对角化哈密顿量：对$c_{mi}^\dagger$做傅里叶变换\eqref{eq:lattice-fourier}，然后对$\vb*{k}$相同，$n$不同的不变子空间做对角化，那么得到的$\vb*{k}$表象同样就是Bloch波函数表象。
将$i$表象切换到$\vb*{k}$表象，给出
\begin{equation}
    H = \sum_{\vb*{k}} \sum_{m, n} \sum_l t_{mn, l} \ee^{- \ii \vb*{k} \cdot \vb*{R}_l} c^\dagger_{m \vb*{k}} c_{n \vb*{k}} ,
...
由于Wannier波函数非常局域，$U(\vb*{r} - \vb*{R}_i)$总是非常“深”的，即其各个矩阵元基本上是负的且绝对值很大；$V(\vb*{r})$的“尖锐”与否从而影响了$V(\vb*{r}) - U(\vb*{r})$的大小。
% TODO
...
也可以从傅里叶变换的角度看这个问题：此时\eqref{eq:from-spectrum-to-tight-binding}中只有一个$l$分量，而自由电子则有很多相长干涉的$l$分量，从而紧束缚模型通常能带带宽不宽。
总之，以下几个说法基本上是等价的：
\begin{enumerate}
...
设$w_{s}(\vb*{r} - \vb*{R}_i)$为我们选定的Wannier波函数，其中标签$s$标记轨道而
...
\begin{equation}
    t_{ij} = \ee^{- e \int_i^j \dd{\vb*{r}} \cdot \vb*{A}},
\end{equation}
