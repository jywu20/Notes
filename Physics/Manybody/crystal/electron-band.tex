...
本节分析能带电子，并给出一些能够产生能带的模型。虽然在很多系统中相互作用的影响不能忽略，但能带理论仍然提供了一个很好的“自由理论”，可以在此之上讨论相互作用。
...
不过我们看到，周期势场让能带穿过第一布里渊区边界之后又回到第一布里渊区中，且第一布里渊区边界上简并解除，从而我们可以使用能带编号$n$和通过加减特定倒格矢回到第一布里渊区中的$\vb*{k}$标记各个电子模式。
...
在空间平移对称性成立，但是紧束缚近似未必成立时，我们用$t(\vb*{R}_l)$表示从格点$\vb*{R}_i$到$\vb*{R}_i + \vb*{R}_l$的跃迁系数。
...
从坐标表象切换到$i$表象相当于对哈密顿量做了一点对角化但没有完成，从$i$表象切换到$\vb*{k}$表象则完成了对角化。
对角化结果是
\begin{equation}
...
\paragraph{含能带间跃迁的模型} 实际上，我们甚至可以胡乱指派一个其实并非我们的体系的能量本征态的Bloch波函数$\psi_{n \vb*{k}}$及其对应的Wannier波函数，然后根据\eqref{eq:sqrt-v-rep}写出一个修正后的紧束缚模型，不过这样就会有带间跃迁了。
由于此时的Wannier波函数其实不正确，写出这种含有带间跃迁的紧束缚模型之后做的对角化将会重新设置能带标签。

这种稍微劣质一些的Wannier波函数可以通过线性组合单原子轨道波函数得到（对复式晶格，还可以组合同一晶胞内不同原子的轨道），下面是一些例子。
...
\begin{enumerate}
    ...
    \item 以原子间的成键和反键轨道为表象。电子出现在不同原子附近的概率振幅的信息被模糊化了，但是能带编号变得清晰了。
    \item 以自行设定的用不同电子轨道线性组合得到的Wannier波函数为表象。这个表象下的波函数只能够展示电子在不同晶胞上的概率振幅分布。保留了一些“一个电子是哪些轨道的混合”的信息，能带编号比较清晰。
    ...
    \item 以自行设定的用不同电子轨道线性组合得到的Bloch波函数为表象。波函数不显示任何电子的空间分布信息，但是保留了一些“一个电子是哪些轨道的混合”的信息。
