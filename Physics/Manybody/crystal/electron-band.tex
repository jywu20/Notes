...
前面的对称性分析告诉我们应该将$\vb*{k}$约束在第一布里渊区内，而在做近自由电子在周期势场下的微扰时我们会非常直接地看到原因。
...
这种电子状态称为\concept{Wannier波函数}。
我们使用$i, j, \ldots$表示电子位置，$m, n, \ldots$用于区分定域在$i$号晶胞附近的电子的电子云的形状（例如定域在某个格点附近的电子可以非常定域，也可以比较定域，有两种模式），$\vb*{r}_i$表示$i$处的格点的位矢，则定域在$i$号晶胞附近的Wannier电子的波函数可以用$w_{ni}(\vb*{r})$表示。
Wannier波函数可以写成如下形式：
\begin{equation}
...
在$i$仅仅区分不同原胞时，$i$的取值个数也是$N$，因此我们可以猜测，$i$与Bloch电子的晶格动量标记了同样的自由度，而$n$与能带编号标记了同样的自由度。
% TODO：链接到对角化

\subsubsection{Bloch波函数诱导出的Wannier波函数}
...
我们可以不失一般性地取$m = n$，即将哈密顿量在这个标签上做对角化，此时有
\begin{equation}
...
\subsubsection{含能带间跃迁的模型} 

实际上，我们甚至可以胡乱指派一个其实并非我们的体系的能量本征态的Wannier波函数及其对应的Bloch波函数$\psi_{n \vb*{k}}$，然后根据\eqref{eq:sqrt-v-rep}写出一个修正后的紧束缚模型，不过这样就会有带间跃迁了；或者，如果这胡乱指派的Wannier波函数实际上更加接近单个原子周围的轨道，我们需要一步将单个原子周围的轨道做。
...
本节只是从对称性的角度出发分析了哈密顿量的可能形式。下一节我们将写出跃迁系数并推导它和Bloch表象下的能带之间的关系。

\subsection{跃迁系数的计算}

\begin{figure}
    \centering
    \subfigure[同类型的Wannier波函数定域在不同格点附近（用不同颜色区分），彼此之间存在交叠，由于它们均不是系统的能量本征态，它们之间存在相互跃迁]{
        \includegraphics[width=0.45\textwidth]{band/hopping/overlap.png}
    }
...
\section{原子轨道组合和紧束缚模型}

\subsection{单电子模型及其跃迁系数}\label{sec:tight-binding-hopping}
