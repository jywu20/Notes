...
晶格可以根据其对称性或者说所属的\concept{晶体群}而被分类为\concept{晶系}。
所谓晶体群指的是这样的一个群$G$：它是欧氏空间$\mathbb{R}^n$的离散等距变换子群，并且$\mathbb{R} / G$是紧致的（即整个空间中任何一个点都可以等效到某个晶胞中）。
数学上可以证明这样的群一定包含$n$种彼此独立的平移变换（本该如此，因为我们需要这些平移变换来产生完整的布拉维格子），
...
此时我们说，能带之间\concept{打开了能隙}，让原本交叉的能带之间出现了间隙。
