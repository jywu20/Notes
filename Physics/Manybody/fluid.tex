...
考虑一个能量本征态，其中准粒子在费米面之上的数量为$\var{n}$（$\delta$表示相对基态的偏离），则总是把能量本征值相对于零温平衡态（由于费米海的结构可以非常复杂，平衡态能量反而是算不出来的）的变化写成以下级数展开（$\vb*{k}$在费米面附近）：
\begin{equation}
...
取到二阶项，因为一些微扰论计算表明前两项有时是同阶的，后面的高阶项（对应多体相互作用）则可以略去；把能量写成粒子数的函数假定了自旋守恒。
也可以从另一个角度看\eqref{eq:fermi-liquid-energy}的导出。我们知道由于相互作用的存在，总能量$E$肯定不是单粒子能量（比如说$k^2/2m$这种形式）简单加起来的结果，但是显然能量具有可加性，那么应该能够写出
\[
    E = \sum_{\vb*{k}} n_{\vb*{k}} \epsilon_{\vb*{k}},
\]
其中$\epsilon(\vb*{k})$是每个准粒子的能量，它的一部分是单粒子能量，一部分是其它粒子给它的相互作用能之和。现在设想我们改变了准粒子数分布，那么就有
\[
    \var{E} = \sum_{\vb*{k}} (\var{n_{\vb*{k}}} \epsilon_{\vb*{k}} + n_{\vb*{k}} \var{\epsilon_{\vb*{k}}}).
\]
$\epsilon$会依赖准粒子数分布是因为它有一部分来自相互作用能，由于我们只研究二体相互作用，我们有
\[
...
\]
代入$\var{E}$中就得到\eqref{eq:fermi-liquid-energy}；第二项的$1/2$因子是因为一对粒子会被计数两次，所以要除以$2$；由于我们假定准粒子分布相对于平衡态只有微小的偏离，$\epsilon_{\vb*{k}}$被取为平衡态的值。
...
或者等价地可以设
\begin{equation}
    \hat{f}(\theta) = f^\text{S}(\theta) + \hat{\sigma} \hat{\sigma}' f^\text{A}(\theta)
\end{equation}
从而将自旋守恒这一事实一并表示出来（$\hat{\sigma}^z$就是$z$方向的泡利矩阵），并将$f^\text{S}(\theta)$和$f^\text{A}(\theta)$展开成无量纲常数：
\begin{equation}
...
实际上，如果一个费米液体系统可以确定是一个实际的费米气体加入相互作用的结果，并且如前所述，能够保证准粒子个数和实际费米子的个数一样，自旋相同，等等，那么费米液体中准粒子的等效质量和实际费米子的质量有一个简单的，使用$\{F_l^\text{S,A}\}$写出的关系。
我们知道% TODO
...
本节考虑一个粒子间有斥力的电子系统，并且展示这个系统在温度不高时（确切地说，温度低于费米温度时）可以使用费米液体描述。
...
\]
二次量子化相互作用哈密顿量相当于将坐标表象下的测度取为$\int \dd[3]{\vb*{r}}$，于是动量表象下测度应当取为
\[
    \frac{1}{V} \sum_{\vb*{k}},
\]
现在切换到动量表象下，则代入
\[
    \hat{\psi}_\sigma^\dagger(\vb*{r}) = \frac{1}{V} \sum_{\vb*{k}} \ee^{- \ii \vb*{k} \cdot \vb*{r}} \hat{c}^\dagger_{\vb*{k} \sigma},
\]
从而可以验证，二次量子化哈密顿量为
\[
...
其中
\begin{equation}
    \mel*{\vb*{k}_1' \alpha, \vb*{k}_2' \beta}{\hat{U}}{\vb*{k}_1 \alpha, \vb*{k}_2 \beta} = \frac{1}{V} \int \dd[3]{\vb*{r}} U(\vb*{r}) \ee^{\ii (\vb*{p}_1 - \vb*{p}'_1)} = \frac{1}{V} \int \dd[3]{\vb*{r}} U(\vb*{r}) \ee^{\ii (\vb*{p}_2' - \vb*{p}_2)}.
\end{equation}
...
\[
    \frac{N}{V} \sim \epsilon_\text{F}^{3/2} \sim \left( \frac{k_\text{F}}{\hbar} \right)^{3},
\]
而考虑到会发生散射的费米子都在费米面附加，对稀薄的费米气体我们有
\begin{equation}
    \frac{k_\text{F}}{\hbar} \ll \frac{1}{r_0}.
\end{equation}



注：即使粒子间相互作用很强，由于我们只关心动量散射，可以使用一个较弱的相互作用代替原来的相互作用，只需要保证散射截面一致就可以。%TODO
