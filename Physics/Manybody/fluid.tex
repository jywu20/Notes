...
\]
其中$\epsilon_{\vb*{k}}$是每个准粒子的能量，它的一部分是单粒子能量，一部分是其它粒子给它的相互作用能之和。现在设想我们改变了准粒子数分布，那么就有
\[
...
\]
代入$\var{E}$中就得到\eqref{eq:fermi-liquid-energy}；第二项的$1/2$因子是因为一对粒子会被计数两次，所以要除以$2$；由于我们假定准粒子分布相对于零温只有微小的偏离，$\epsilon_{\vb*{k}}$被取为零温的值。
