...
为了方便起见我们将计算松原格林函数
\[
    C(\vb*{q}, \omega_n) = \frac{1}{\beta^2 L^{2d}} \sum_{\vb*{k}, \vb*{k}'} \expval*{\hat{c}^\dagger_{\vb*{k}+\vb*{q}, \uparrow}(\tau) \hat{c}^\dagger_{-\vb*{k}, \downarrow}(\tau) \hat{c}_{-\vb*{k}'+\vb*{q}, \downarrow}(0) \hat{c}_{\vb*{k}' \uparrow}(0)},
\]
其中所有算符都定义在松原频率$\omega_n$上。（我们利用了能量守恒的性质）

使用费曼图，并使用梯形图近似，顶角修正为（推导这个公式时需要注意顶角修正在这里只和输入的两个电子的动量和有关，和自旋、电子各自的动量无关）
\[
    \Gamma_{\vb*{q}} = g + \frac{g}{\beta L^d} \sum_{\vb*{p}} G^0_{\vb*{p} + \vb*{q}} G^0_{-\vb*{p}} \Gamma_{\vb*{q}},
\]
从而
\[
    \Gamma_{\vb*{q}} = \frac{g}{1 - \frac{g T}{L^d} \sum_{\vb*{p}} G^0_{\vb*{p}+\vb*{q}} G^0_{-\vb*{p}}}.
\]
...
在$T>T_\text{c}$时，这个关联函数的虚部给出了一个预料之中的指数衰减，但是$T = T_\text{c}$时出现奇异性。
...
\begin{equation}
    Z = \int \fd(\bar{\Delta}, \Delta) \exp(- \frac{1}{g} \int \dd{\tau} \dd[d]{\vb*{r}} \abs*{\Delta}^2 + \int \dd[d]{\vb*{r}} \ln \det \mathcal{G}^{-1}).
\end{equation}
上式实际上还有一个常数因子，因为做格拉斯曼场高斯积分的时候，二次型前面没有负号，不过它也不重要。
