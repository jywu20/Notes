...
低温下，气体的行为和气体粒子是费米子还是玻色子有密切的关系。

\subsubsection{自由度冻结}

\subsubsection{玻色-爱因斯坦凝聚}
...
本节讨论微分方程表述的经典动理学。由于热涨落，通常不能够直接使用哈密顿量做动力学演化而需要加入一些环境项而得到主方程。
...
本节讨论将费米气体加上相互作用之后的行为。

\subsection{弱相互作用费米子体系的激发}
...
\subsubsection{无阻尼的条件}

为什么流体流动竟然会形成阻尼？答案显然是流体在流动过程中，一些能量从流体的宏观动力学自由度（速度等）转移到了它的一些内部自由度上。
...
\[
    \eval{\frac{\epsilon_{\vb*{p}}}{p}} > 0,
\]
这就等价于，从坐标原点向$\epsilon_{\vb*{p}}-\vb*{p}$超曲面引切线，不应该出现斜率为零的切线。
例如，在$\vb*{p}$很小时$\epsilon_{\vb*{p}}$不应该是“平坦”的。
这种流体原则上就不会出现内摩擦的情况称为\concept{超流}。

显然，如果$\epsilon_{\vb*{p}} \propto p^2$，那么绝对不可能出现超流；而如果流体中的准粒子是某种声子，那么确实可能出现超流。
