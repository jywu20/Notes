...
\begin{equation}
    \begin{aligned}
        ...
        &+ \sum_{m, k} \int_0^t \dd{t_1} \int_0^t \dd{t_2} \mel{n}{\hat{H}'(t_1)}{m} \ee^{- \ii (E_m - E_n) t} \mel{m}{\hat{H}'(t_2)}{k} \ee^{- \ii (E_k - E_m) t} c_k(0) \\
        &+ \cdots,
    \end{aligned}
\end{equation}
这就是\textbf{Dyson级数}。我们当然还可以进一步扩充Dyson级数的适用范围，得到
\begin{equation}
    \begin{aligned}
        c_n(t) &= c_n(t') + \sum_m \int_{t'}^t \dd{t} (- \ii \mel{n}{\hat{H}'(t_1)}{m}) \ee^{- \ii (E_m - E_n) (t - t')} c_m(t') \\
        &+ \sum_{m, k} \int_{t'}^t \dd{t_1} \int_{t'}^t \dd{t_2} \mel{n}{\hat{H}'(t_1)}{m} \ee^{- \ii (E_m - E_n) (t - t')} \mel{m}{\hat{H}'(t_2)}{k} \ee^{- \ii (E_k - E_m) (t - t')} c_k(t') \\
        &+ \cdots.
    \end{aligned}
\end{equation}
...
\textbf{散射}指的是一种$\hat{H}'$实际上不含时的过程，通常我们讨论从$t=-\infty$到$t=\infty$的过程。
