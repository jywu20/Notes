...
在特定的条件下，一个量子理论可以退化为经典理论——各个量子数要足够大，以至于不连续性可以被忽略（所以散射态看起来更加像经典情形），但能量又不能太高（否则需要考虑圈图修正）。
此时可以使用经典的运动方程描述整个系统。
...
在计算费曼图时，前者只有一条真正的外线（因为在量子的观点下我们是在计算$\expval{\phi}$），外部输入通过外源加入，传播子表示一个场；后者可以有好多条外线，没有外源，外部输入就是粒子输入，传播子表示一个粒子的轨迹。
