...
事实证明，在量子理论中抽象代数具有非常重要的作用，因此我们将专门用一节介绍它。量子理论中常见的抽象代数结构包括离散群（例如空间群）以及李群，如角动量是$SU(2)$群的李代数的表示，基本粒子是庞加莱群的不可约表示。
...
这一段写得是不全面的。能够被称为“量子退化为经典”的情况很多的：大量子数近似（在量子数本身受到限制时——如自旋自由度中$S_z$的取值只有两个——这个近似就是错的）能够让量子动力学退化为经典动力学。
强烈的环境扰动能够让量子概率振幅退化为经典概率，把系统内部的纠缠什么的全部抹去（而与此同时系统和环境却纠缠在一起）。
...
这可以看成经典图景下的微扰意义上的因果律；量子理论中只有期望值不会违反因果律，费曼图计算的“中间过程”可以看起来违反因果律（因为这些中间过程并不是真正的“中间态”——系统并不是先演化到一个多粒子态上再演化到另一个多粒子态上）。
所以，的确——经典图景下其实也是会出现圈图发散的，“一个电子先发出一个光子，再吸收这个光子”这个过程——或者在经典图景下说，电子通过自己产生的电场和自己发生相互作用——原则上是需要考虑的。（对这些的论述见Solving Classical Field Equations一文）
...
经典系统也有费曼图这件事解释了一些奇怪的用语习惯，如即使我们讨论的问题是静态的，也常用“光束先打到这里，再打到那里”的话语来描述一个光学系统，即使我们都很清楚我们在讨论系统的稳态。
在使用这一类术语时我们实际上是在做微扰计算。
...
这个投影实际上就是将产生算符替换成右矢而将湮灭算符替换成左矢。
实际上，如果我们知道了二次量子化哈密顿量遵循的限制（或者说对应的场论遵循的限制，比如对称性），那么也可以使用一次量子化哈密顿量反推二次量子化哈密顿量。

关于时间演化需要特别说一句。我们刚才提到产生算符对应右矢，例如我们有
\[
    \ket{\vb*{x}} = \hat{\varphi}^\dagger(\vb*{x}, t) \ket{0},
\]
这里我们特意加上了$^\dagger$符号，因为在$\varphi$是复场时，$\varphi$通常对应湮灭算符而$\varphi^\dagger$对应产生算符。
但是当我们开始讨论时间演化时，很自然的问题是，这是什么绘景下的右矢？
其实那并不是什么绘景下的右矢，而只是海森堡绘景中的某个模式的占据数算符的一个本征态而已。
它的确有时间演化，但那是场算符的时间演化的一个“影子”。这种方式定义的$\ket*{\vb*{x}}$不是薛定谔绘景中的右矢。
不过，这并不是说海森堡绘景中的$\varphi$的时间演化和薛定谔绘景中的单粒子态之间没有关系。
设$\ket*{\psi}$是一个单粒子态，则
\[
    \braket{\vb*{x}}{\psi} = \mel*{0}{\varphi(\vb*{x})}{\psi}
\]
是一个绘景不变量。我们立刻注意到，如果$\varphi(x)$服从某个方程$L_{\vb*{x}} \varphi(\vb*{x}) = 0$，那么方程
\[
    L_{\vb*{x}} \mel*{0}{\varphi(\vb*{x})}{\psi} = 0
\]
也成立——而这正是$\ket*{\psi}$在坐标表象下的运动方程！
然后，我们立刻可以将$L_{\vb*{x}}$看成某个抽象的算符在坐标表象下的矩阵元，于是就得到了$\ket*{\psi}$的运动方程。
因此，相当有趣的，在哈密顿量中和左矢地位相似的场算符（大体上是湮灭算符）的海森堡绘景运动方程给出了单粒子态的薛定谔绘景运动方程。
