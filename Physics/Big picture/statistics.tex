...
所谓相互独立指的是对其中一个系统做某些操作（或者说，让其中一个系统和另一些东西产生相互作用）不影响另一个系统的状态。例如，对其中一个系统做测量不会影响另一个系统的状态。这就等价于两个系统没有量子纠缠。
设两个系统的密度算符分别为
\[
...
本节基本上只在正则系综下工作，因为巨正则系综等效为一个哈密顿量为$\hat{H}-\mu\hat{N}$的正则系综。
