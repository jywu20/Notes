...
仅考虑关于系统的哈密顿量不会导致粒子生灭的情况%
\footnote{虽然非相对论情况下基本粒子确实没有生灭，但分子甚至原子还是会有产生和消失。}%
，则系统的哈密顿量和诸粒子数算符对易。
...
我们称这样粒子总体上没有产生消灭，但粒子会在系统和粒子库之间流动的系统的平衡态为\textbf{巨正则系综}。

由于系统的哈密顿量和各个粒子数算符彼此对易，系统的哈密顿量的本征态可以被标记为
\[
    \ket{E^{(i)}, N_1^{(j_1)}, N_2^{(j_2)}, \ldots, k},
\]
我们使用和\autoref{sec:contacting-systems}中一样的记号，使用右上角标标记各可观察量的不同本征值。$k$指的是可能出现的额外的自由度，因为仅仅使用粒子数可能不足以完全区分$\hat{H}$的简并本征态。
...
按照\autoref{sec:contacting-systems}中的方式也可以得到连续能谱情况下的表达式。由于连续能谱意味着粒子数很大，我们也可以把诸$N_i$看成连续的，从而做替换
\[
...
可以从两方面看到这个事实。首先，在已知$d(E, N_1, \ldots, N_s)$时，仅仅考虑量子态在能量上的分布，就得到
\[
    d(E) = \int \dd{N_1} \dd{N_2} \cdots \dd{N_s} d(E, N_1, \ldots, N_s),
\]
在\eqref{eq:grand-canonical-ensemble-states}中应用此式，积掉所有粒子数变量就得到
\[
    d_T(E) = \int \dd{E_1} d_1(E_1) d_2(E-E_1),
\]
这正是正则系综的状态数公式\eqref{eq:canonical-state-continue}。从这个公式结合等概率原理就能够推导出正则系综。
也就是说，我们把粒子数信息当成了正则系综中用于区分能量简并态的东西。
另一种思路是，\eqref{eq:grand-canonical-ensemble-density-operator}由于$\hat{H}$和诸粒子数算符对易，可以写成
\[
...
\end{equation}
因此凡是只通过$\hat{H}$就能够得到的信息也可以通过密度算符$\trace_{\hat{N}} \hat{\rho}$计算得到。%
\footnote{回顾\autoref{sec:combining-systems}节中关于约化密度算符的论述，我们把系统哈密顿量以及额外的自由度$k$提供的信息看成一个系统，把系统粒子数算符提供的信息看成另一个系统，将密度算符中粒子数部分求迹，就能够得到一个约化密度算符。}%
当然，巨正则系综实际上服从正则系综这件事是可以预期的，因为推导正则系综时没有用到任何关于粒子数的知识，因此关于正则系综的论述自然也适用于巨正则系综。

% TODO:存疑，这部分东西似乎应该出现在热力学中！
实际上，巨正则系综也可以用于处理一部分粒子数可变的系统，前提是这所谓的“粒子”实际上是更小的粒子束缚而成的，而这更小的粒子的数目未曾变化。
按照化学中的说法，我们可以称数目可变的粒子为分子，称组成它们的、粒子数不变的粒子为原子，但这只是一个类比：例如，我们可以以夸克为原子而以中子、质子为分子，只要我们涉及的过程不包含夸克数目的改变即可。
不同分子的粒子数算符虽然与

% TODO:真的粒子数可变，如什么“夸克胶体“（？）怎么算？
% TODO:复合粒子的粒子数算符怎么处理？
...
\begin{equation}
    \dd{U} = \delta{Q} + \sum_i A_i \dd{B_i}.
\end{equation}

我们使用$\delta$表示热量微元是因为一般来说热量微元不是一个恰当微分形式%
\footnote{一些物理量可以最终写成密度算符的函数，如总能量、熵等，它们完全由系统的状态确定，称为状态函数或状态量；另一些物理量是在一个过程中得到定义的，如功、冲量，其通式为
\[
    A(S_1 \longrightarrow S_2) = \int_{S_1}^{S_2} \sum_i B_i \dd{C_i},
\]
或者写成微分形式
\[
    \var{A} = \sum_i B_i \dd{C_i},
\]
...
直觉上，可逆过程一般意味着比较少的能量转化为了不得不排放到外界的废热$Q_2$。例如，废热的一个重要产生途径为摩擦生热，而摩擦不是一个可逆的过程。因此不慎严格地说，可逆过程意味着摩擦等非理想因素被完全排除。
...
设$T_1>T_2$，从而$Q_1>Q_2$，可逆热机对外做功。考虑\eqref{eq:heat-efficienty}，对可逆热机有
\begin{equation}
    \eta = 1 - \frac{T_2}{T_1}.
\end{equation}
因此对任何热机，均有
...
\end{equation}
取等条件为热机可逆。
% TODO:没有说明热机不可逆时一定取不到等号

现在我们考虑这样的一个可逆热机。它有两个端口，其中一个连接一个完全固定且很大，因此具有基本上不变的温度的热源，我们取这个热源的温度为$T_s$；另一个端口连接一个任意的热源。
我们让这个热机依次和一系列热源接触，吸热或者放热，设当它和温度为$T$的热源接触时吸热$\dd{Q}$，而向温度恒定为$T_s$的热源接触时放热$\dd{Q_s}$（如果从温度为$T$的热源吸热，向温度恒定为$T_s$的热源放热，则以上两个量取负值），就有
\[
    \frac{\dd{Q}}{T} = \frac{\dd{Q_s}}{T_s}.
\]
我们总是可以合理设计热机（每一步热机从温度为$T$的热库吸收多少能量是可以调节的），让热机的一个循环内温度为$T_s$的热源没有净热量变化。
这样一来，热机
