...
首先尝试记有$\Omega_1 (E)$个彼此独立的系统的能量为$E$的态，有$\Omega_2 (E)$个彼此独立的热库的能量为$E$的态，有$\Omega(E)$个彼此独立的总系统的能量为$E$的态。
我们似乎有
\begin{equation}
    \Omega (E) = \sum_{n} \Omega_1 (E_1^{(n)}) \Omega_2 (E - E_1^{(n)}).
...
因此我们接受\eqref{eq:total-system-state-number}。

关于\eqref{eq:total-system-state-number}还有一个值得注意的地方。由于系统和热库的能级可以是连续的或者几乎是连续的，$\Omega$应该怎么定义实际上需要进一步澄清。
...
就得到了正确的结果，也就是
\[
    d(E) \dd{E} = \int d_1 (E_1) \dd{E_1} d_2 (E - E_1) (\dd{E} - \dd{E_1}),
\]
即
\begin{equation}
    d(E) = \int \dd{E_1} d_1 (E_1) d_2 (E - E_1).
\end{equation}
...
然而，由于$\Omega(E)$、$d(E)$和$\Omega_\epsilon(E)$之间的换算关系是完全均匀的，当离散谱各能级的间距很小时，可以把等概率原理应用到它们任何一个上面。
...
\begin{equation}
    Z = \sum_n \ee^{-\beta E_n} = \sum_{E^{(n)}} \Omega(E^{(n)}) \ee^{-\beta E^{(n)}}.
\end{equation}
...
\[
    \hat{H} = \sum_{\text{single particle terms}} \hat{H}_i + \text{interaction},
\]
...
\[
    \expval*{\hat{E}} = \sum_i E^{(i)} P(E^{(i)}) = \frac{1}{Z} \sum_i E^{(i)} \Omega(E^{(i)}) \ee^{- \beta E^{(i)}} = \frac{1}{Z} \sum_\text{eigenstate $\ket{n}$} E_n \ee^{-\beta E_n} ,
\]
可以得到
\begin{equation}
...
于是
\[
    \expval*{\ln \rho_n} = \ln \rho(\expval*{E}),
\]
我们就得到
\begin{equation}
    S = - \ln \rho(\expval*{E}).
...
\[
    P(E_1, N_1) = \frac{\Omega_2(E-E_1, N-N_1)}{\Omega (E, N)}
\]
% TODO：从正则系综导出巨正则系综
