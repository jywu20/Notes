...
从系统状态到实数的连续函数称为\textbf{状态函数}。由于热力学不分析系统的内部结构（这些要留给统计物理），实际上我们做的正好相反：我们使用状态函数来标记一个系统的宏观状态，这些被选出来的状态函数称为\textbf{热力学坐标}。%
...
\end{equation}
无论对什么求导，展开对配分函数的对数的二阶导数之后都会得到
\[
    \frac{1}{Z} \pdv[2]{Z}{X} - \frac{1}{Z^2} \left( \pdv{Z}{X} \right)^2,
\]
% TODO：涨落很小
...
\end{equation}
其中已经使用符号$N_i$代替了平均值$\expval*{\hat{N}_i}$，这是为了表明粒子数平均值是一个热力学坐标。
...
\begin{equation}
    \expval*{\hat{N}_i} = \frac{1}{1 - \eta \ee^{- (\alpha + \beta \epsilon_k)}} = \frac{1}{1 - \eta \ee^{- \beta (\epsilon_k - \mu)}}.
\end{equation}

% TODO：对粒子数分布，最可几=平均。话说似乎所有的热力学坐标的涨落在系统很大时都应该很小，所以对所有的热力学坐标而言，最可几=平均。实际上这有可能就是热力学坐标的定义，例如麦克斯韦分布中粒子速率分布相对平缓，则不会使用粒子速率分布作为热力学坐标？？
% 最可几=平衡？？
...
\begin{equation}
    \var{Q} = \sum_i \expval*{\hat{n}_i} \dd{\epsilon_i}.
\end{equation}
