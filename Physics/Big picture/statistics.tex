...
从希尔伯特空间的角度也可以得到这个结论，因为如果一个纯态系统的某些自由度与其它自由度从来不发生相互作用，那么系统的态空间实际上是前面这些自由度和后面这些自由度的直积，两个态空间中的态可以分别拿出来计算。
...
\end{equation}
粒子数前面的系数在使用$\beta$和$\alpha$（而不是化学势）表示时就和$\beta$无关了；这个$\alpha$当然和以粒子数算符为扰动的\eqref{eq:partition-function-with-disturbance}中的$\beta\lambda$具有一样的作用。
通过和正则系综类似的方法，可以计算出
\begin{equation}
...
\]
前面提到这是微扰，因此$y(t)$和$h(t)$的关系近似是线性的，从而我们可以使用一个格林函数联系两者，即
\[
