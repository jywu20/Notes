...
\end{equation}
在连续极限下，$\hat{A}_i$变成了量子场$\hat{A}(\vb*{x})$，我们这就得到了所谓的\textbf{统计场论}，这只需要将偏导数换成偏泛函导数$\fdv{\phi}$即可。

与量子场论中的路径积分相比较，$\hbar$的地位和$T=1/\beta$的地位是一样的。不同的$T$对应着不同的行为，正如不同的$\hbar$（实际上$\hbar$固定不变，场值在变，但也可以等效地认为场值不变而$\hbar$在变）对应不同的行为。
...
\[
    \rho \propto \ee^{-\beta \hat{H}} \sim \ee^{-\beta H(x, p)},
\]
于是对正则系综，有
\begin{equation}
...
\]
的不等式。微分号内的部分称为这种条件下的\textbf{热力学势}，有时也称为\textbf{自由能}。
% TODO:存在性，以及
...
格林函数的极点对应着集体激发模式？虚部代表模式的寿命。这里的格林函数是指场算符的格林函数？
...
\[
    ...
    \end{aligned}
\]
\autoref{sec:spectral-function}中会证明，被积函数在区域$0 < \Im t < \hbar \beta$内是解析的，从而就有
\[
    \begin{aligned}
...
实际计算中，基本上可观察量的每一项都含有相同数目的产生湮灭算符，因此两个可观察量的松原格林函数就都是玻色型的。

周期性条件\eqref{eq:period-matrubara}意味着，松原格林函数的傅里叶变换实际上就是傅里叶级数。那就是说，
\begin{equation}
    G_{AB}(\omega) = \int_0^\beta \dd{\tau} \ee^{\ii \omega_n t} G_{AB}(\tau),
\end{equation}
反过来就有
\begin{equation}
...
\begin{equation}
    G_{AB}(\omega) \xrightarrow{\ii \omega_n \longrightarrow \omega+\ii 0^+} G_{AB}^\text{ret} (\omega),
\end{equation}
就得到了推迟格林函数。
