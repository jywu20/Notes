...
\begin{enumerate}
    ...
    \item 截断$\Lambda$；
    \item 外部给定的、不参与重整化的常数（比如统计物理中的温度）。
\end{enumerate}
...
相变的特征包括临界性——相变点相较于整个参数空间永远是一个零测集；相变应该是系统的集体行为，因此正在发生相变的系统的关联长度应该是无限大；此外，相变点附近的物理规律总是服从幂律。

相变对应着系统非解析的特征。显然，由于配分函数及有关的量都是足够光滑的，非解析性只可能出现在系统趋于无限大时，此时配分函数带虚部的奇点可能会出现在实轴上，从而产生非解析性。
显然这意味着在相变点附近微扰论严格来说实际上是失效的。很多表面上微扰论的失效（如得出发散的结果）本质上是由于没有考虑高能标下的物理过程，在考虑这一点之后通过重整化可以引入一些修正因子来压低发散，从而得到正确的结果；相变点附近微扰论的失效则是有物理意义的。
因此可以预期，直接通过分析配分函数的奇点来分析相变在数学上是非常困难的，所幸可以通过重整化群的方式来理解相变。
...
$L$趋于无穷大时出现了不止一个普适类，这就表明在不同的外加参数下（例如不同温度下）系统可能出现截然不同的行为，从而每个普适类就是一个相，它们之间的转换就是相变。
