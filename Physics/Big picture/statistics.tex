...
从系统状态到实数的连续函数称为\textbf{状态函数}。由于热力学不分析系统的内部结构（这些要留给统计力学），实际上我们做的正好相反：我们使用状态函数来标记一个系统的宏观状态，这些状态函数称为\textbf{热力学坐标}。%
\footnote{所谓“宏观状态”实际上并不是一个良定义的概念。然而，如果使用几个状态函数就足以计算出我们想要的一切，那显然没有必要去分析其它物理量。因此，“热力学坐标”实际上是“我们打算计算的状态函数”。}
状态函数可以是某个良定义的可观察量的期望值及其函数，也可以是某个涌现出来的量，比如说熵或者温度。

首先引入\textbf{热力学第零定律}：若系统$A$和系统$B$接触后可以达成平衡，系统$B$和系统$C$接触后可以达成平衡，那么系统$A$和系统$C$接触之后也能够达成平衡。
