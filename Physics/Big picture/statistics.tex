...
% TODO：把前面求期望值什么的拿到这里来;配分函数差一个常数因子没有关系

\subsubsection{从正则配分函数计算各物理量}
...
很容易计算出
\begin{equation}
    \expval*{\hat{n}_k} = \frac{g_k}{1 - \eta \ee^{- (\alpha + \beta \epsilon_k)}} = \frac{g_k}{1 - \eta \ee^{- \beta (\epsilon_k - \mu)}}.
\end{equation}
...
\begin{equation}
    \expval*{\hat{n}_i} = \frac{1}{1 - \eta \ee^{- (\alpha + \beta \epsilon_i)}} = \frac{1}{1 - \eta \ee^{- \beta (\epsilon_i - \mu)}}.
\end{equation}
...
\begin{equation}
    \sum_k \frac{1}{1 - \eta \ee^{-\beta (\epsilon_k - \mu)}} = N,
\end{equation}
其中$k$跑遍粒子所有的自由度的全部可能取值，$N$是总粒子数。如果用$k$表示能级编号则上式需要改写成
\[
    \sum_k \frac{g_k}{1 - \eta \ee^{-\beta (\epsilon_k - \mu)}} = N.
\]
求解得到的$\mu$是$N,T$的表达式。这个方程通常过于复杂而不能解析求解，不过我们注意到
\[
    \pdv{N}{\mu} = \sum_k \eta \beta \frac{\ee^{-\beta (\epsilon_k - \mu)}}{(1- \eta \ee^{ - \beta (\epsilon_k - \mu)})^2},
\]
右边求和号中的每一项都是正的，因此
\[
...
\]
如果$\hat{O}$中的产生算符数目和湮灭算符数目不一致，那么$\mel{n}{\hat{O}}{n}$就一定是零，因此单个场算符的期望值是零，并且奇数个算符的乘积的期望值也是零，从而算符服从高斯分布
\[
    \expval{\ee^{\hat{A}}} = \ee^{\frac{1}{2} \expval*{\hat{A}}},
\]
再加上自由哈密顿量是二次型这个事实，我们确定Wick定理适用，并且凡是产生算符数目和湮灭算符数目不一致的$n$点关联都为零。
