...
\begin{equation}
    \expval*{A} = \frac{1}{Z} \trace \left(\hat{\rho} \hat{A}\right) = \frac{\trace \left(\hat{\rho} \hat{A}\right)}{\trace \hat{\rho}},
\end{equation}
纯度公式为
\begin{equation}
...
我们从能量的角度出发讨论问题。当然，也可以使用别的物理量，但能量由于和哈密顿量的联系以及其可加性，尤为方便。
设系统和环境中的一部分之间有能量和物质传递，且这一部分远大于系统，称其为\textbf{热库}。%
...
我们来分析系统取特定能量的概率。我们称系统处于某个能量上的所有态的总和为\textbf{能级}。
...
由于系统和热库总能量恒定，我们有
\[
    P(E_s) \propto d_s(E_s) d_r(E_T - E_s) \propto d_r (E_T - E_s),
\]
从而
\[
...
\[
    \expval*{E} = \sum_i E_i P(E_i) = \frac{1}{Z} \sum_i E_i g_i \ee^{- \beta E_i},
\]
可以得到
\begin{equation}
    \expval*{E} = - \frac{1}{Z} \pdv{Z}{\beta} = - \pdv{\ln Z}{\beta}.
\end{equation}
...
\end{equation}
显然，取$\lambda = 0$我们就回退到了没有扰动的系统的配分函数。现在我们计算
\[
    \pdv{Z(\beta, \lambda)}{\lambda} = \trace \pdv{\lambda} \ee^{- \beta (\hat{H} + \lambda \hat{A})}.
\]
