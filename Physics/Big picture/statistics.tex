...
就有% TODO
\begin{equation}
    \var{W}_\text{output} \leq \text{somgthing},
\end{equation}
...
\subsection{格林函数与量子场论的处理方法}

由\autoref{sec:linear-response}，系统可观察的各种特征全部可以由推迟格林函数推导出来。
换而言之，近平衡态理论就是要计算各种不同的可观察量的格林函数。
现在的问题是，怎么计算关于两个可观察量的推迟格林函数？任何一个系统都可以通过一个场论产生，例如非相对论多电子体系由一个薛定谔场产生，电子对撞由QED产生，等等，系统的各个状态可以使用一组产生湮灭算符（或所谓的场算符）产生，相应的，任何一个可观察量都是一系列二次量子化算符或它们的线性组合。
...
本节给出推迟格林函数以及其它几种与之相关的格林函数的定义。它们虽然并不都像推迟格林函数那样有着明确的物理意义，但在理论计算上很重要。
...
\begin{equation}
    G(t, t')_{AB} = - \frac{\ii}{\hbar} T_t \expval*{\hat{A}(t) \hat{B}(t')},
\end{equation}
...
定义\textbf{推迟格林函数}为
\begin{equation}
    G^\text{ret}_{AB} (t, t') = - \frac{\ii}{\hbar} \Theta (t-t') \expval*{\comm*{\hat{A}(t)}{\hat{B}(t')}_\eta},
\end{equation}
其中带$\eta$脚标的方括号定义为
\begin{equation}
...
在$\hat{A}$和$\hat{B}$都是可观察量时，推迟格林函数有明确的物理意义（就是对外界扰动的响应函数）；在它们是场算符时，看起来推迟格林函数只是对响应函数的模仿。它们的作用要在\autoref{sec:matrubara-theory}中才能更好地看出。

与推迟格林函数类似地还可以定义\textbf{超前格林函数}：
\begin{equation}
    G^\text{adv}_{AB} (t, t') = \frac{\ii}{\hbar} \Theta(t' - t) \expval*{\comm*{\hat{A}(t)}{\hat{B}(t')}_\eta}.
\end{equation}
...
大于格林函数是
\begin{equation}
    G^> (t, t')_{AB} = - \frac{\ii}{\hbar} \expval*{\hat{A}(t) \hat{B}(t')},
\end{equation}
小于格林函数是
\begin{equation}
    G^< (t, t')_{AB} = - \eta \frac{\ii}{\hbar} \expval*{\hat{B}(t') \hat{A}(t)}.
\end{equation}
...
费米子的推迟格林函数就是
\begin{equation}
    G^\text{ret}_{\sigma \sigma'}(\vb*{r}, t; \vb*{r}', t') = - \frac{\ii}{\hbar} \Theta(t-t') \expval*{\acomm*{\hat{\psi}_\sigma (\vb*{r}, t)}{\hat{\psi}_{\sigma'}^\dagger(\vb*{r}', t')}},
\end{equation}
类似地，玻色子的推迟格林函数为
\begin{equation}
    G^\text{ret}_{\sigma \sigma'}(\vb*{r}, t; \vb*{r}', t') = - \frac{\ii}{\hbar} \Theta(t-t') \expval*{\comm*{\hat{\psi}_\sigma (\vb*{r}, t)}{\hat{\psi}_{\sigma'}^\dagger(\vb*{r}', t')}}.
\end{equation}
以上两式当然来自以下统一的形式：
\begin{equation}
    G^\text{ret}_{\sigma \sigma'}(\vb*{r}, t; \vb*{r}', t') = - \frac{\ii}{\hbar} \Theta(t-t') \expval*{\comm*{\hat{\psi}_\sigma (\vb*{r}, t)}{\hat{\psi}_{\sigma'}^\dagger(\vb*{r}', t')}_\eta}.
\end{equation}

在系统是自由的时——也即，场的运动方程是线性方程时——场算符的编时格林函数确确实实是微分方程意义上的格林函数，具体来说，它是系统的薛定谔表象运动方程的格林函数。
对费米场，我们有
\[
    \hbar G_{\sigma \sigma'}^\text{ret} (\vb*{r}, t; \vb*{r}', t') = - \ii \Theta (t-t') \expval*{\hat{\psi}_\sigma (\vb*{r}, t) \hat{\psi}_{\sigma'}^\dagger (\vb*{r}', t')} + \ii \Theta(t'-t) \expval*{\hat{\psi}_{\sigma'}^\dagger(\vb*{r}', t') \hat{\psi}_\sigma (\vb*{r}, t)},
\]
求导之后可以得到
\[
    \begin{aligned}
        ...
        &= \hbar \delta(t-t') \delta(\vb*{r} - \vb*{r}') \delta_{\sigma \sigma'} + \Theta(t-t') \expval*{\dot{\hat{\psi}}_\sigma (\vb*{r}, t) \hat{\psi}^\dagger_{\sigma'} (\vb*{r}', t')} - \Theta(t'-t) \expval*{\hat{\psi}^\dagger_{\sigma'} (\vb*{r}', t') \dot{\hat{\psi}}_\sigma (\vb*{r}, t)}.
    \end{aligned}
\]
...
其中$A$可以是算符，代入前式得到
\[
    (\ii \hbar \partial_t - A(\vb*{r}, t)) G_{\sigma \sigma'}^\text{ret} (\vb*{r}, t; \vb*{r}', t') = \hbar \delta(\vb*{r} - \vb*{r}') \delta(t-t').
\]
% TODO:二阶方程，比如克莱因-高登
...
场算符的大于格林函数和小于格林函数是
\begin{equation}
    G^>_{\sigma \sigma'} (\vb*{r}, t; \vb*{r}', t') = - \frac{\ii}{\hbar} \expval*{\hat{\psi}_\sigma (\vb*{r}, t) \hat{\psi}_{\sigma'}^\dagger (\vb*{r}', t')},
\end{equation}
以及
\begin{equation}
    G^<_{\sigma \sigma'} (\vb*{r}, t; \vb*{r}', t') = - \eta \frac{\ii}{\hbar} \expval*{\hat{\psi}_{\sigma'}^\dagger (\vb*{r}', t') \hat{\psi}_\sigma (\vb*{r}, t)}.
\end{equation}
...
\subsubsection{频域上的场算符格林函数}

在系统具有时间平移不变性时，格林函数（无论是哪一种）的值仅仅和$t-t'$有关，于是使用$G_{\sigma \sigma'}(\vb*{r}, \vb*{r}', t-t')$标记它们。
这就意味着我们可以对$t-t'$做傅里叶变换，从而得到频域上的格林函数，记作$G_{\sigma \sigma'}(\vb*{r}, \vb*{r}', \omega)$。
在自由场的情况下，这就是外加一个周期性载荷（而不是冲击载荷），场给出的响应。

对处于真空态的自由体系——此时体系的状态就是$\ket{0}$，无需使用密度算符描述，即退化为零温场论——粒子数不变，于是总是可以找到一组好量子数，记作$n$（通常它就是动量和自旋），从而写出对角化的自由哈密顿量
\[
...
\footnote{自由场情况下各个粒子可以认为是彼此无关的，从而做替换$\ket{n} \longrightarrow \hat{\psi}_n$就从单粒子哈密顿量得到了场论哈密顿量，这个操作就是“二次量子化”这个说法的来源。}%
\[
...
\end{equation}
$\hat{G}$不是别的，正是单粒子哈密顿量$\hat{h}$所导致的薛定谔绘景运动方程的频域格林函数：
给方程
\[
...
% TODO:区分不同的格林函数；大概以$\omega_n$为宗量的是松原格林函数，以$\omega$为宗量的是实时格林函数？
% TODO:矩阵形式的格林函数
\[
    \hat{G}(\omega_n) = \sum_m \frac{1}{\ii \omega_n - \epsilon_m} \dyad{m}. 
\]
实际上就是
\[
    \frac{1}{E - \hat{H}}
...
\subsubsection{松原理论}\label{sec:matrubara-theory}
