...
我们来分析系统取特定能量的概率。由于系统和热库总能量恒定，我们有
\[
...
\]
最后，注意到系统的能量可以整体地加上一个常数而不影响其行为，因此上式右边只应该关于$E_s$和$E_{s'}$的差值，从而
\[
    \frac{P(E_s)}{P(E_{s'})} \propto f(E_s - E_{s'}).
\]
满足这个关系的唯一可能就是
...
对\eqref{eq:gibbs-distribution}做归一化，就有
\begin{equation}
    P(E_s) = \frac{1}{Z} \ee^{ - \beta E_s},
    \label{eq:distribution-of-energy}
\end{equation}
其中
\begin{equation}
...
\end{equation}
就是在引入未归一化的密度算符时提到的配分函数。
\eqref{eq:distribution-of-energy}给出了
