...
\subsection{费米场和玻色场的格林函数}

现在的问题是，怎么计算关于两个可观察量的推迟格林函数？任何一个可观察量都是一系列单体或多体二次量子化算符或它们的线性组合，因此只需要发展出一种方法，使得对一组产生湮灭算符，可以计算出


\begin{equation}
    G_{AB}(\omega_n) \stackrel{\ii \omega_n \rightarrow \omega + \ii 0^+}{\longrightarrow} G_{AB}^\text{ret} (\omega).
\end{equation}

% TODO：Wick定理的统计系综版本要求哈密顿量是二次型？？
