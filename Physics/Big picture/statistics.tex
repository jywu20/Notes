...
我们这里还隐含地引入了一个假设，就是内能$U$和系统是如何和外界连接的无关，换而言之，
% TODO:可加性，以及广延量之类的东西的定义
...
\]
这是一个非常强的论断，因为直觉上$S$的行为会和很多参数有关。
...
也有一些相变并不涉及对称性的变化，此时就不能够使用序参量来表征相变。

% TODO：相变的发生一定有涨落，因为系统要在可能的分叉中选择一个分支。怎么从数学上分析呢？？？分叉来自对称性自发破缺。
% 这里肯定涉及动力学过程，不可能整个过程都是静态的
...
设共有$g$个基态，随着系统规模的增大，我们有$g \sim N$，而零温下
\[
    S = \ln \Omega = \ln g \sim \ln N.
\]
其它广延量的增长和$N$同阶，因此零温下的熵可以忽略。因此，热力学极限下，零温的熵为零。
% TODO：导数为零？这个好像和量子信息论有关

热力学第三定律实际上是一种宏观量子效应。如果经典统计在温度趋于零时仍然适用，
