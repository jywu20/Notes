...
\begin{equation}
    F[\phi] = \int \dd[d]{\vb*{x}} \left( 
        \frac{1}{2} (\grad{\phi})^2 + \frac{1}{2} \mu^2 \phi^2 + g \phi^4 + B \phi
    \right).
    \label{eq:phi-4-g}
\end{equation}
...
标记箭头的目的主要是为了写出正确的动量守恒表达式。如\autoref{fig:vertex-intepretation}中，三个图的动量守恒表达式分别是
\[
...
\begin{enumerate}
    ...
    \]
    \item 乘上对称性因子%
    \footnote{有些书上会说“除以对称性因子”，但那是因为这些书中$\phi^4$项前面会乘以$1/4!$之类的因子，再乘上对称性因子，最后还是会得到“1除以某某”的因子。本文不采用这种记号。}%
    ...
    因此，计算积掉一部分自由度的自由能时使用的费曼图还是计算关联函数的费曼图，但是对称性因子却要除以外腿的数目的阶乘。
\end{enumerate}
...
相互作用部分为
\begin{equation}
    F_I[\phi] = \int \prod_{i=1}^4 \frac{\dd[d]{\vb*{k}_i}}{(2\pi)^d} \phi_{\vb*{k}_1} \phi_{\vb*{k}_2} \phi_{\vb*{k}_3} \phi_{\vb*{k}_4} (2\pi)^d \delta^d(\sum_{i} \vb*{k}_i).
...
\subsection{相互作用}\label{sec:interaction-rg}
...
\[
    \cexpval{F_I[\phi]}_+ = \int \prod_{i=1}^4 \frac{\dd[d]{\vb*{k}_i}}{(2\pi)^d} \cexpval{\phi_{\vb*{k}_1} \phi_{\vb*{k}_2} \phi_{\vb*{k}_3} \phi_{\vb*{k}_4}}_+ (2\pi)^d \delta^d(\sum_{i} \vb*{k}_i).
\]
...
\begin{equation}
    \begin{aligned}
        \begin{tikzpicture}
            ...
            \end{feynhand}
        \end{tikzpicture} &= 6 g_0 \int \frac{\dd[d]{\vb*{k}_1}}{(2\pi)^d} \int \frac{\dd[d]{\vb*{k}_2}}{(2\pi)^d} \int \frac{\dd[d]{\vb*{k}_3}}{(2\pi)^d} \phi^*_{\vb*{k}_2} \phi_{\vb*{k}_1} \frac{1}{k_3^2 + \mu_0^2} (2\pi)^d \delta^d(\vb*{k}_1 - \vb*{k}_2) \\
        &= 6 g_0 \int \frac{\dd[d]{\vb*{k}}}{(2\pi)^d} \phi_{-\vb*{k}} \phi_{\vb*{k}} \int \frac{\dd[d]{\vb*{q}}}{(2\pi)^d} \frac{1}{q^2 + \mu_0^2}.
    \end{aligned}
    \label{eq:one-loop-correction}
\end{equation}
这里我们对传播子的动量积分时将积分上下限取在$\Lambda/\zeta$和$\Lambda$之间，因为要积掉高能自由度。
对称性因子是$4!/(2 \cdot 2) = 6$，分母中的两个$2$分别来自从顶角引出的两条内线融合成了一个圈，以及两条外腿实际上是等价的这一事实。

\eqref{eq:one-loop-correction}给出了$\phi^2$项的系数的修正：
\[
    \frac{1}{2} {\mu'}^2 = \frac{1}{2} \mu^2 + 6 g_0 \int^\Lambda_{\Lambda/\zeta} \frac{\dd[d]{\vb*{q}}}{(2\pi)^d} \frac{1}{q^2 + \mu_0^2},
\]
...
\begin{equation}
    \mu^2(\zeta) = \zeta^2 \left( \mu_0^2 + 12 g_0 \int^\Lambda_{\Lambda/\zeta} \frac{\dd[d]{\vb*{q}}}{(2\pi)^d} \frac{1}{q^2 + \mu_0^2} \right), \quad g(\zeta) = \zeta^{4-d} g_0.
...
\autoref{fig:single-second-order-correction}导致一个$\phi^6$项。我们将
