...
现在我们转而分析量子平衡态统计系统。在本文中，如果我们要讨论空间坐标，一律默认为格点系统。
...
为了简写我们引入记号
\begin{equation}
    \hat{U}(\tau_2, \tau_1) = \prod_{n=n_1+1}^{n_2} \ee^{-\Delta \tau \hat{H}_0} \ee^{- \hat{H}_\text{I}(\vb{s}_n)}, \quad \vb{B}(\tau_2, \tau_1) = \prod_{n=n_1+1}^{n_2} \ee^{-\Delta \tau \vb{h}_0} \ee^{-\vb{h}_\text{I}(\vb{s}_n)},
\end{equation}
使用这些记号并考虑到\eqref{eq:imaginary-time-path-integral-with-aux-field}，
\[
    \trace(\ee^{-\beta \hat{H}}) = \sum_{\vb{s}} C(\vb{s}) \trace \hat{U}(\beta, 0) = \sum_{\vb{s}} C(\vb{s}) \det(1 + \vb{B}(\beta, 0)),
\]
而
\[
    \begin{aligned}
        ...
        &= \sum_{\vb{s}} C(\vb{s}) \det(1 + \vb{B}(\beta, 0)) \frac{\trace(\hat{O} \hat{U}(\beta, 0))}{\trace \hat{U}(\beta, 0)}.
    \end{aligned} 
\]
现在构造一个经典的随机现象，使得
\begin{equation}
    p(\vb{s}) = \frac{ C(\vb{s}) \det(1 + \vb{B}(\beta, 0))}{\sum_{\vb{s}} C(\vb{s}) \det(1 + \vb{B}(\beta, 0))}, \quad \expval{O}_{\vb{s}} = \frac{\trace(\hat{O} \hat{U}(\beta, 0))}{\trace \hat{U}(\beta, 0)},
\end{equation}
