...
对$J/K$很小的情况，系统基态是一个$\tau^z$全为$1$的态和只有一个$\tau^z$为$-1$的态的线性叠加，前者的权重接近$1$而后者的权重接近$J/K$，即
\[
    \ket*{\text{ground}} = \ket*{\tau^z = \uparrow \uparrow \cdots \uparrow} + \frac{J}{K} \sum_i \ket*{\tau^z = \uparrow \cdots \downarrow_i \cdots \uparrow} + \cdots,
\]
如果格点$i$在$C$内部，那么它不会对期望值有什么贡献，因为对应的
% TODO
