...
\begin{equation}
    \hat{H} = - K \sum_{\pair{i, j}} \hat{\sigma}^x_{ij} - J \sum_{(i, j, k, l) \in \Box} \hat{\sigma}^z_{ij} \hat{\sigma}^z_{jk} \hat{\sigma}^z_{kl} \hat{\sigma}^z_{li}.
\end{equation}
...
\end{equation}
这正是\concept{横场伊辛模型}，它是一个二维量子模型，其配分函数的精确形式对应一个三维经典统计模型，实际上这个三维经典统计模型就是一个伊辛模型。
