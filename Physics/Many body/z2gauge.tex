...
现在的问题是，\Ztwo规范场在零温下的两个相都是什么？三维伊辛模型具有一个顺磁相和一个铁磁相，由于铁磁序的形成需要更多相互作用，$J/K$超过某个点时会出现铁磁相，否则出现顺磁相。
而由于$\hat{\tau}^z$实际上标记了磁通量（$1$对应磁通量为零，$-1$对应磁通量为$\pi$），三维伊辛模型的铁磁相意味着磁通量或者都是$1$或者都是$-1$。
直觉上看，铁磁相中电子可以畅行无阻，而顺磁相中由于磁通量有正有负，电子将会“很困惑”，不知道怎么走，因此三维伊辛模型的铁磁相应该对应一个非局域的态而顺磁相应该对应一个“局域”或者“束缚”的态。
...
容易看出$W(C)$给出了从某个虚时间点开始产生一对e激发，经过一段时间之后又湮灭的概率（在经典理论中这是良定义的，因为没有任何不确定关系；具体有没有手段可以用量子测量的标准方法测出这个概率则是另外一回事），随着$C$扩大它理所当然会衰减，如果随着$C$扩大它衰减得很快那么就说明e激发总是成对地被束缚在一起。
...
我们尝试将$W(C)$映射到横场伊辛模型中，容易验证
\begin{equation}
    W(C) = \expval{\prod_{I \in D} \tau^x_I(\tau)},
\end{equation}
其中$D$是$C$围成的区域；上式成立是因为$C$围成的区域内部的$\sigma^z$会被乘两次，于是就给出$1$，于是
\[
    \sum_{l \in C} \sigma^z_l = \sum_{l \in D} \sigma^z_l,
\]
依照$\tau^x$的定义即得到我们需要的表达式。
由于只有两个相，可以在$J/K$很小或很大时分别做微扰论，由此得到的关于相的结构的信息在整个相内部都是成立的。
对$J/K$很小的情况，系统基态是一个$\tau^x$全为$1$的态和只有一个$\tau^x$为$-1$的态的线性叠加，前者的权重接近$1$而后者的权重接近$J/K$，则容易验证
