...
\end{equation}
这些位置矢量构造了一架三维网格，这个网格称为\textbf{布拉维格子}，这些矢量称为\textbf{布拉维格矢}，$\{\vb*{a}_i\}$称为\textbf{晶格常数}。
...
并非所有晶体中的物理量都是周期性的，它们的傅里叶变换中的动量不局限在第一布里渊区内。
...
使用幺正变换，可以得到
\begin{equation}
    G_{dd, \sigma} = \sum_n \frac{1}{\ii \omega_n - \epsilon_n} \braket{\text{d} \sigma}{n \sigma} \braket{n \sigma}{\text{d} \sigma}.
\end{equation}
