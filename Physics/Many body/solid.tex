...
\footnote{这个拟设为什么合理涉及到平均场理论，这里暂时不涉及。}%
...
，对费米子系统，处在能量本征态$\ket{n}$上的粒子数的平均值为
\begin{equation}
    \expval*{\hat{n}_n} = \frac{1}{\ee^{\beta (E_n-\mu)} + 1}.
\end{equation}
...
\end{equation}
因此诸$\vb*{G}_m$也构成一个布拉维格子，我们称它为\textbf{倒格子}，与$\vb*{R}_n$构成的空间网格（称为\textbf{正格子}）相区分。倒格子的基矢量和正格子的基矢量互为共轭基矢量。
两种格子的基矢量可以通过下式
\begin{equation}
...
\end{equation}
及其线性组合。因此\eqref{eq:periodic-wavefunction}构成晶体中电子气中单个电子的态空间的一组基，且$\vb*{k}$是一个好量子数。
...
\[
    -\frac{\laplacian}{2m_\text{e}} \psi(\vb*{r}) + V(\vb*{r}) \psi(\vb*{r}) = E \psi(\vb*{r}),
\]
得到
\begin{equation}
    \left( - \frac{(\grad+\vb*{k})^2}{2m_\text{e}} + V(\vb*{r}) \right) u_{\vb*{k}}(\vb*{r}) = E u_{\vb*{k}}(\vb*{r}).
    \label{eq:block-energy-problem}
\end{equation}
在$\vb*{k}$已知的情况下求解该本征值问题，可以得到一组$u$以及对应的$E$。由于先前要求$u(\vb*{r})$是周期函数，该本征值问题必定给出离散谱，也即，我们会获得一组$(\psi_{\vb*{k},n}, E_{\vb*{k}, n})$，使得
\begin{equation}
    \left( - \frac{(\grad+\vb*{k})^2}{2m_\text{e}} + V(\vb*{r}) \right) u_{\vb*{k},n}(\vb*{r}) = E_{\vb*{k}, n} u_{\vb*{k},n}(\vb*{r}).
\end{equation}
