...
此时我们说，相互作用在能带之间打开了一个能隙，让原本交叉的能带之间出现了间隙。}%
...
\begin{equation}
    \hat{H} = - t \sum_i (\hat{c}_i^\dagger \hat{c}_{i+1} + \text{h.c.}) - \mu \sum_i \hat{c}_i^\dagger \hat{c}_i + \sum_i (\Delta \hat{c}_i^\dagger \hat{c}_{i+1} + \text{h.c.} ).
    \label{eq:kitaev-chain-hamiltonian}
\end{equation}
这个模型通常是这么来的：一个一维纳米线被放置在一个超导体上，两者的相互作用诱导前者也发生$U(1)$对称性破缺，然后我们使用平均场理论分析问题而引入一个$\Delta$参量。
\eqref{eq:kitaev-chain-hamiltonian}是一个p波超导模型。
\eqref{eq:kitaev-chain-hamiltonian}是一个紧束缚模型，
对\eqref{eq:kitaev-chain-hamiltonian}做傅里叶变换然后再做Bogoliubov变换，可以得到以下能谱：
\begin{equation}
    E_{\vb*{k}} = \pm \sqrt{(2t \cos k - \mu)^2 + 4 \abs{\Delta}^2 \sin^2 k}.
\end{equation}

只有能隙开闭可能产生相变。
在化学势很低时，即$\mu$趋于负无穷时，几乎没有电子。因此从$-\infty$到$-2t$的部分肯定是平庸的？
