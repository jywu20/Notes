...
设$\Phi$为库伯对序参量，我们知道它是一个复场。有时称它为超导波函数，虽然它并不是任何粒子的波函数，但它服从的方程和薛定谔方程形式一致。
$U(1)$对称性在一般的物理规律中从来没有失效，于是根据对称性我们写下一个理论
\begin{equation}
...
通过量纲分析可以发现没有提到的项在重整化不动点附近全部是不相关的，在重整化群作用下会被压低。
