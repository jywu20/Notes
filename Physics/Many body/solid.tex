...
\begin{equation}
    \epsilon_{k} = \pm v_\text{F} (k - k_\text{F}).
\end{equation}
方程前面加正负号是因为色散关系是左右对称的，所以在两个费米点处的斜率互为相反数。
从费米面以下到费米面以上能量总是增加的，于是斜率为正表示$k>0$，斜率为负表示$k<0$。

我们将讨论费米点附近的低能有效理论，则晶格上的坐标表象湮灭算符（即和Wannier波函数有关的那个）几乎以$k_\text{F}$为频率振荡，于是做拟设
\begin{equation}
    \hat{c}_{n\sigma} = \ee^{\ii k_\text{F} na} \hat{c}_{\sigma \text{R}} + \ee^{ - \ii k_\text{F} na} \hat{c}_{\sigma \text{L}},
\end{equation}
其中$\hat{c}_{\sigma \text{L}}$和$\hat{c}_{\sigma \text{R}}$在空间上是缓变的。
R和L分别表示对应的模式在$k>0$处（称为\textbf{右模式}），以及对应的模式在$k<0$处（称为\textbf{左模式}）。
