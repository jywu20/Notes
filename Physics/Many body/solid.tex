...
如果$\vb*{\phi}$非零，那么显然$\expval*{\hat{\vb*{S}}_i}$在$i \in A$时和$\expval*{\hat{\vb*{S}}_i}$在$i \in B$时差一个负号，正好意味着形成了反铁磁序。
...
\end{equation}
不失一般性地令晶格常数为1，则第一布里渊区为$[-\pi, \pi)^2$，且
\[
    (-1)^i = \ee^{\ii \pi (i_x + i_y)},
\]
...
% TODO
布里渊区折叠了，费米面交叉处打开能隙，形成一系列小费米pocket。

超越平均场的涨落：低能激发的自由度是什么？

最基本的散射过程：一个自旋波模式和费米子发生散射
如果是低能有效理论，那么由于能标要对齐，$\epsilon_{\vb*{k}}, \epsilon_{\vb*{k} + \vb*{q}}$

\[
    \hat{H}_\text{eff} = \lambda (\phi_{\vb*{q}} \hat{c}^\dagger \hat{c}) + \hat{H}[\phi] + 
\]
