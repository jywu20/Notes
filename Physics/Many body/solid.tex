...
这样整个系统的哈密顿量就是
\begin{equation}
    \hat{H} = \sum_{\vb*{k}, \alpha} (\epsilon_{\vb*{k}} - \mu) \hat{c}_{\vb*{k} \alpha}^\dagger \hat{c}_{\vb*{k} \alpha} - V_0 \sum_{\vb*{k}, \vb*{q}} \sum_{\alpha, \beta} \hat{c}^\dagger_{(\vb*{k} + \vb*{q}) \alpha} \hat{c}^\dagger_{( - \vb*{k} - \vb*{q}) \beta} \hat{c}_{-\vb*{k} \beta} \hat{c}_{\vb*{k} \alpha}.
...
\begin{equation}
    \hat{H}_\text{MF} = \sum_{\vb*{k}, \alpha} (\epsilon_{\vb*{k}} - \mu) \hat{c}_{\vb*{k} \alpha}^\dagger \hat{c}_{\vb*{k} \alpha} - V_0 \sum_{\vb*{k}, \vb*{k}', \alpha, \beta} \left(
...
\begin{equation}
    ...
    ) = -2V \sum_{\vb*{k}} \expval*{\hat{c}_{-\vb*{k}\uparrow} \hat{c}_{\vb*{k} \downarrow}} ,
\end{equation}
则可以证明
\begin{equation}
...
对\eqref{eq:kitaev-chain-hamiltonian}做傅里叶变换然后再做Bogoliubov变换，可以
