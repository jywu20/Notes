...
实际上，外场的束缚（如晶格束缚；本文剩下的部分将紧密围绕这一点展开讨论）、电子-声子相互作用（见\autoref{sec:phonon}）等都会让\eqref{eq:full-electron-gas-hamiltonian}中各项的系数发生变化，但是对称性决定了出现在哈密顿量中的只有这些项，于是我们写出一般形式的哈密顿量：
\begin{equation}
    \hat{H} = \sum_{\vb*{k}, \sigma} (T_{\vb*{k}} - \mu) \hat{c}^\dagger_{\vb*{k} \sigma} \hat{c}_{\vb*{k} \sigma} + \sum_{\vb*{k}_1, \vb*{k}_2, \vb*{q}, \sigma} \hat{c}^\dagger_{\vb*{k}_1+\vb*{q}, \sigma} \hat{c}^\dagger_{\vb*{k}_2-\vb*{q}, \sigma} V_{\vb*{q}} \hat{c}_{\vb*{k}_2 \sigma} \hat{c}_{\vb*{k}_1 \sigma}. 
\end{equation}
相应的，热力学作用量为
\begin{equation}
    S = \sum_n \left( \sum_{\vb*{k}, \sigma} (-\ii \omega_n + T_{\vb*{k}} - \mu) \hat{c}^\dagger_{\vb*{k} \sigma} \hat{c}_{\vb*{k} \sigma} + \sum_{\vb*{k}_1, \vb*{k}_2, \vb*{q}, \sigma} \hat{c}^\dagger_{\vb*{k}_1+\vb*{q}, \sigma} \hat{c}^\dagger_{\vb*{k}_2-\vb*{q}, \sigma} V_{\vb*{q}} \hat{c}_{\vb*{k}_2 \sigma} \hat{c}_{\vb*{k}_1 \sigma} \right). 
\end{equation}
上式中的动能项和相互作用项的形式由对称性确定，具体系数可以暂时不设置具体值（因为直接从\eqref{eq:full-electron-gas-hamiltonian}出发做重整化群计算显然是非常困难的）。
处理这类模型的常用方法包括：
\begin{itemize}
    \item 直接做费曼图计算，由于相互作用是二体的，在相互作用较弱时计算一到二圈图就可以得到很好的效果；
...
再次强调，这个“电子能量形式固定，改变化学势往系统中填充电子”的物理图像不适用于强关联系统，因为在那里等效的电子能量形式\eqref{eq:single-electron-hamiltonian}随着电子填入会发生剧烈变化；相应的，从这个物理图像衍生出来的所有理论，如能带理论（见\autoref{sec:energy-band}）也不再适用。
...
，这个近似要求以下两点：
\begin{itemize}
    \item $\var{\hat{n}}$和$V_\text{eff}$之间的响应函数近似是自由电子的$\var{\hat{n}}$-$\var{\hat{n}}$格林函数；
...
这样，计算出自由电子的密度-密度格林函数之后问题就完全解决了。依照定义，
\[
    \begin{aligned}
    ...
    \end{aligned}
\]
其中最后一个等号用到了Wick定理。由于自旋守恒性，只有$\sigma = \sigma'$的项才非零，则得到
\[
...
\end{equation}
考虑到\eqref{eq:ext-electron-ret-nn}，当$\omega = \omega_\text{p}$时格林函数出现奇点，即此处有集体激发。实际上，这正是\textbf{等离子激元}。
...
\end{equation}
即光子的一个模式和固体发生耦合，打开了一个能隙。在假定光速无限时，丢弃不可见、恒定的无穷大项就得到
\[
...
不过，一个正比于$\omega_n$的项意味着实空间中有某种阻尼，这也是正确的，因为自旋波模式如前所述会衰变。这种阻尼或者衰变就是\textbf{朗道阻尼}。
...
\]
我们将$\sigma_{xy}$这种横向的电导称为\textbf{霍尔电导}，记为
\begin{equation}
    \sigma_\text{H} = \frac{ec}{B} n_\text{e}.
\end{equation}

定义\textbf{磁通量子}（它是超导中磁通量量子化的单位乘以2）
\begin{equation}
    \Phi_0 = \frac{h c}{e},
\end{equation}
...
因此，随着电子数上升，在填充延展态时电导线性上升，而在填充定域态时电导没有变化。这就形成了\textbf{电导平台}。
相应的，平台处的霍尔电导为
\[
...
关于为什么霍尔电导取非常简洁的形式（通常的电导涉及关于材料的复杂性质），\textbf{Laughlin论证}给出了一个解释。
