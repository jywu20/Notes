...
\end{enumerate}
从而可以确定所有物理量。% 这里似乎还做了一个半经典近似，将电势能当成经典情况实际上是忽略了交换能

由于没有电子间相互作用且体系充分大，我们使用费米-狄拉克分布
\begin{equation}
    n(\vb*{r}) = \sum_{\vb*{k}, \sigma} \frac{1}{1 + \ee^{\beta (\epsilon_{\vb*{k}} - \mu)}},
\end{equation}
由于体系足够大，屏蔽作用让外加电荷的作用几乎是局域的，则$\var{n}(\vb*{r})$可以写成$V_\text{eff}(\vb*{r})$的函数而不涉及长程相互作用，而由于外加电荷很小，采取线性近似（第二个等号是因为费米-狄拉克分布的形式）
\[
    \var{n} = \pdv{n}{V_\text{eff}} V_\text{eff} = - \pdv{n}{\mu} V_\text{eff},
\]
定义
\begin{equation}
    N(\mu' - \mu) = \pdv{n}{\mu'},
