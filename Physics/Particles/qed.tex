...
真空极化实际上就是光子自能。我们做类似\autoref{sec:vertex-function-symmetry}中的对称性分析。
光子自能的表达式一定是某个关于光子动能$p$的标量乘以某个正比于$p$的二次项的二阶张量。
...
\begin{equation}
    \begin{aligned}
        ...
        &\quad \quad \times \left( \frac{2}{\epsilon} - \gamma_E + \bigO(\epsilon) \right) \\
...
其中我们定义
\begin{equation}
    \tilde{\mu}^2 = 4 \pi \ee^{-\gamma_E} \mu^2.
\end{equation}
\eqref{eq:regularized-photon}给出正规化之后的光子自能。可以看到的确存在发散，不过发散已经被提取出来了，它是$\epsilon$的一个一阶极点。
