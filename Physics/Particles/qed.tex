...
这看起来很不自然，但在很多情况下——例如，在凝聚态物理，包括量子光学中——光子可以分成截然不同的两部分，一部分是物质之间交换的、基本上导致一个库伦相互作用的光子，一部分是遥远处的光源产生的“入射光”。
这两部分光子彼此没有任何相互作用（能标足够低，不会有QED的四光子等效相互作用），并且前者从来不会出现在外线中。
这种情况下使用等效的电子-电子库伦相互作用就是非常合理的。
