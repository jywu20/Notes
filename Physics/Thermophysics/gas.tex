...
理想气体的配分函数是
\begin{equation}
    Z = \int \prod_i \dd{p_i} \dd{q_i} \ee^{- \beta \sum_i \frac{p_i^2}{2m}}.
\end{equation}
首先尝试经典的计算，即认为$p,q$可以连续取值。设单个粒子有$s$个自由度，其中有三个是其质心的坐标，那么
\[
    Z = \prod_i \int \dd{q_i} \int \dd{p_i} \ee^{- \frac{p_i^2}{2m} },
\]
其中
\[
    \prod_i \int \dd{q_i} = \const \cdot V^{N},
\]
前面的常数是粒子内部自由度积分的结果，而
\[
    \prod_i \int \dd{p_i} \ee^{- \beta \frac{p_i^2}{2m}} = \left( \frac{\pi m}{\beta} \right)^{\frac{Ns}{2}},
\]
于是
\[
    Z \propto \const \cdot V^{N} \left( \frac{\pi m}{\beta} \right)^{\frac{Ns}{2}},
\]
前面的系数并不重要，故略去，得到
\begin{equation}
    Z = V^{N} \left( \frac{\pi m}{\beta} \right)^{\frac{Ns}{2}}.
\end{equation}
