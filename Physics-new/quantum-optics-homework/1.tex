...
\end{equation}
\eqref{eq:scully-1-2-temp-3} can be shown inductively. The cases of $n=1, 2$ are trivial. Suppose \eqref{eq:scully-1-2-temp-3} for an arbitrary $n$, then 
\[
    \begin{aligned}
...
\begin{itemize}
    ...
    \[
        ...
        \end{aligned}
    \]
    It can be seen that the $P$-function behaves badly, which is expected since a Fock state $\ket*{n}$ with completely determined photon numbers is far from a classical optical field.
