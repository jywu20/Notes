...
to introduce \eqref{eq:semidirect-product-def}.
...
\begin{equation}
    \cdots \to C^{n-1} \stackrel{\dd^{n-1}}{\longrightarrow} C^n \stackrel{\dd^n}{\longrightarrow} C^{n+1} \to \cdots ,
...
\concept{$n$-cocycle} if and only if $\dd{\omega} = 0$, the set of which is denoted as $Z^n$, and an $n$-cochain $\omega$ is said to be an 
\concept{$n$-coboundary} if and only if $\omega = \dd{\mu}$, the set of which is denoted as $B^n$. Of course, $n$-coboundaries are all 
