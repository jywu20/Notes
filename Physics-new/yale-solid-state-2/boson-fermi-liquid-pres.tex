...
\begin{frame}
    ...
    \onslide<1->{
        \textbf{Serious quantitative prediction} Bethe-Salpeter eq. (BSE)
...
\begin{frame}
    ...
    \onslide<2->{
        ...
        \begin{itemize}
            \item Zero sound in uncharged Fermi liquid: 
                collective density fluctuation in $\vb*{k}$- (but not $\vb*{r}$-) space
            \item Plasmon in charged Fermi liquid = zero sound + long range interaction 
...
\onslide<3->{
    ...
    \textbf{EOM governing zero sound} No nonlinearity, no dissipation: 
...
\onslide<2->{
    ...
    \begin{equation}
        ...
        \laplacian \varphi = e \cdot \frac{1}{V} \sum_{\vb*{p}} \var n_{\vb*{p}}(\vb*{r}).
    \end{equation}
}
