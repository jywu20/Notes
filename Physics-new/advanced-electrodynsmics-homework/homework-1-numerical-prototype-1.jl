...
# Plots works awfully for the task. Makie is used instead.

function plot_vector_field(f)
    f = vec(f)
    norms = map(norm, f)
    arrows(vec(x), vec(y), vec(z), 
        map(v -> v[1], f),
        map(v -> v[2], f),
        map(v -> v[3], f),
        color = norms
...
##
plot_vector_field(electric_spherical_wave(1, 3))