...
\begin{equation}
    ...
    \label{eq:re-2}
\end{equation}
where we have packaged all symmetric factors in the equations above into $f(\vu*{s}, \vu*{s}')$.
...
also, in computer graphics, the position is referred to as $\vb*{p}$, 
which may be confused with the momentum,
but since the latter is given no place in computer graphics,
this notation change will not cause any confusion.
...
(see \prettyref{fig:approx-point-source});
...
\begin{figure}
    \centering
    \input{optical-path/approx-point-source.tex}
...
\begin{figure}
    \centering
    \input{optical-path/light-path-integral.tex}
...
The problem is shown in \prettyref{fig:surface-rendering-example}: 
