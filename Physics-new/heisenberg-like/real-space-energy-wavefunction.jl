...
mutable struct HeisenbergBlock
    ...
    # The projector from the Hilbert space of the spin chain to a low enengy subspace
    proj::SparseMatrixCSC{Float64, Int64}
end
...
function HeisenbergBlock()
    HeisenbergBlock(zeros(local_dim, local_dim) |> sparse, 1, Sz, S⁺, zeros(local_dim, local_dim) |> sparse, σ0)
end
...
function add_site(block::HeisenbergBlock)::HeisenbergBlock
    ...
    HeisenbergBlock(ham_new, len + 1, Sz_new, S⁺_new, block.dm, block.proj)
end
...
##
# TODO: how to recover the wavefunction???
ψ = real.(ψ)
plot()