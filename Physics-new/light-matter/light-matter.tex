...
one thing we can do is to add another term to the RHS of \eqref{eq:spontaneous-emission-diagram-1} 
...
In this formalism, then, spontaneous emission is not included as a part of $\epsilon$;
rather, it's comparable to a dephasing term in TD-aGW, 
which appears at the RHS of a master equation but doesn't change the total energy of the system. 

One thing, however, is spontaneous emission can be not noisy at all and preserve the pureness of the photon state.
Consider for example a system with a single atom: 
a photon from far afar comes in, interact with the atom,
and then the output state (at $t=\infty$) looks like the electric field stimulated by a dipole
(See \href{../quantum-optics/12-2.pdf}{this note} for what the spontaneous emitted electric field looks like.).
The input and output states have a unitary relation,
and the photon state starting as a pure state never becomes a mixed state!
Indeed we can also build a radiation transfer equation that 
...
\[
    ...
    \end{tikzpicture}
\]
and it seems that in this way spontaneous emission seems to be described by a non-equilibrium $\epsilon$. 
...
the blue propagator may be the equilibrium photon Green function and therefore only contributes to $\Im \epsilon$,
but it can also be a non-equilibrium Green function.
...
\begin{equation}
    \int \frac{\dd{\omega}}{2\pi} \frac{1}{\omega - \omega_g + \ii 0^+ \sgn(\omega)} \frac{1}{\underbrace{\omega - \omega_p}_{\omega'} - \omega_m + \ii 0^+ \sgn(\omega')} \frac{1}{\underbrace{\omega - \omega_p - \omega_q}_{\omega''} - \omega_n + \ii 0^+ \sgn(\omega'')} ,
    \label{eq:chi-3-diagram}
\end{equation}
...
all electron degrees of freedom are \emph{internal} lines,
and the indices $g, m, n$ need to sweep over all possible single-electron states.
$\omega_i$ is actually $\omega_i - \mu$; 
...
and do contour integral (and also pay attention to the fact that there is only one occupied state and therefore there is at most one pole that can appear on the upper plane),
...
One lesson we can learn from doing \eqref{eq:chi-3-diagram} is that in order for a loop like $g \to m \to n \to g$ to have non-zero contribution to the optical response,
we have to make sure not all states in the loop are in the same empty/occupied subspace,
otherwise all poles are on the upper plane or the lower plane,
and the contour integral gives zero.
Intuitively this is quite expected: we need to have electron-hole pair creation to have optical response!
