...
\A This can be viewed as a version of scattering theory, where the scattering eigenstates are made of both the input wave 
function and the output wave function.
...
we see that the correlation function (calculated with $\ii 0^+$ etc.) can only show the response of the system \emph{before} Poincaré recurrence.
So there is nothing wrong here.
Actually, most of the time people only care about \emph{linear response}, which assumes the change of the density matrix is linear to the external field, which is definitely wrong when the time is long enough (because it is proportional to things like $\sin t$).
