...
%Straight Lines [id:da3755411564937934] 
\draw    (93.71,95.53) -- (104.78,74.31) ;
\draw [shift={(105.71,72.53)}, rotate = 477.55] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (12,-3) -- (0,0) -- (12,3) -- cycle    ;
\end{tikzpicture}
    \end{gathered}.
\]
...
What the causality condition really requires is that there is no well-fabricated ``global'' events in the remote past that give rise to 
...
So in conclusion, there is particle decay in unitary systems because we implicitly and manually \emph{require} so: 
It either comes from weird properties of our theory when the universe is infinite or comes from the fact that the decay products are observed by the environment and therefore hidden non-unitary properties exist in our theory.
