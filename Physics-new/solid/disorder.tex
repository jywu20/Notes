...
\begin{figure}
    \centering
    \input{insulator-transition-figs/localization/band.tex}
...
\input{disorder/single-impurity/ring.tex}
...
\input{disorder/single-impurity/vertex.tex}
直观地看，这是说电子运动路径如果经过它曾经经过的某一点，那么这个路径的权重就很大。
这样\eqref{eq:impurity-three-propagator-example}可以直观地写成
\input{disorder/single-impurity/three-propagator.tex}
我们也可以定义动量空间中的费曼规则。计算二体关联函数的二阶微扰$G^{(2)}_{\vb*{i} \vb*{k}}$的傅里叶变换，有
\[
    \begin{aligned}
...
\]
因此动量空间的顶角就是
\input{disorder/single-impurity/vertex-mom.tex}
...
\input{disorder/single-impurity/three-propagator-mom.tex}
