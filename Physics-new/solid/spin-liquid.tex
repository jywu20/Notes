...
\begin{info}{XY模型中的拓扑序}{xy-topological-order}
    ...
    Sachdev在\cite{sachdev_xy}和\cite{Sachdev_2018_full}中，从一个三维的扩展的XY模型（可以看成二维的一个强关联的量子XY模型的零温路径积分，其中XY自旋可以发生各种时间演化，体现了强烈的量子涨落）出发，通过一个离散Hubbard-Stratonovich变换，引入了$\mathbb{Z}_2$规范场。注意引入Hubbard-Stratonovich变换之后，XY自旋自由度的哈密顿量将形如
    \begin{equation}
