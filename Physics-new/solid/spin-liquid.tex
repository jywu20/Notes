...
以RVB为基态的体系中，自旋二聚体的量子涨落现在转移到了自旋子的量子涨落上面。因此，我们需要设法用自旋算符表示出自旋子，然后去得到关于自旋子的有效理论。
这里我们也可以看出为什么要构造RVB态：如果自旋二聚体态实际上接近体系的能量本征态，那么系统的低能自由度仍然是比较平凡的——我们可以将这里的spinon看成类似于XY模型中的涡旋的拓扑孤子，它唯一略有不同寻常的地方就是总是成对出现。
真的重要的是我们将要介绍的强烈的量子涨落引起的演生规范场，是它让体系处在一个无法用纯经典的图像（如“涡旋如同粒子一样”）理解的量子序当中。
我们这里构造的自旋子等是真正有趣的量子序的某种\emph{前体}，它可以看成对演生规范场而言的某种“外加测试电荷”。

\begin{info}{XY模型中的拓扑序}{xy-topological-order}
    Sachdev在\cite{sachdev_xy}一文中，向XY模型引入了$\mathbb{Z}_2$规范场，并且明确地展示了XY模型中的涡旋和$\mathbb{Z}_2$规范场的vison（见\autoref{sec:gauge-charge-flux-z2}）如何冻结为一个自由度。
\end{info}
...
无物质场耦合的\eqref{eq:z2-2d-hamiltonian}中的e激发实际上是非常病态的一个东西，因为它是完全守恒的，没有任何时间演化：如果一开始某个格点上没有e激发，那么就是没有。
因此如果我们只是研究纯\Ztwo规范理论，那么可以认为系统中没有任何e激发，即对所有$\vb*{i}$，施加约束
\begin{equation}
    Q_{\vb*{i}} = 1.
    \label{eq:no-charge-constraint-z2}
\end{equation}
等价地说，我们可以往哈密顿量中引入一个罚函数
\begin{equation}
    H = - g \sum_{\vb*{i}} Q_{\vb*{i}} , \quad g \gg 1,
\end{equation}
即我们可以把纯\Ztwo规范场中的e激发当成一个能隙无限大的元激发：我们只需要关注m激发或者说关注vison就行了。
然而，我们这里研究\Ztwo规范场是为了研究自旋液体的行为，因此讨论e激发仍然是有意义的：例如，如果在纯\Ztwo规范场中引入两个e激发，发现系统能量正比于e激发间距，即e激发是禁闭的，那么\eqref{eq:z2-spin-liquid-ising-gauge}中的费米子——在自旋液体中就是spinon——也是禁闭的。
\eqref{eq:z2-spin-liquid-ising-gauge}中的费米子可以没有能隙，从而其中的e激发也是没有能隙的，这和研究纯\Ztwo规范理论时取约束\eqref{eq:no-charge-constraint-z2}是矛盾的。
