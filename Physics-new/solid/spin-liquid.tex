...
量子涨落的可能体现是我们将要介绍的演生规范场，在二维体系中它将导致拓扑序。
我们这里构造的自旋子等是拓扑序中的任意子（或者别的有趣的量子序中的点粒子）的某种\emph{前体}，它可以看成对演生规范场而言的某种“外加测试电荷”。
...
分数化本身和拓扑序没有直接关系，演生规范场才是决定拓扑序行为的关键。
...
流体动力学中的涡旋有时间演化这件事的量子版本就是涡旋态是大量含有涡旋的流速场构型态的线性叠加。
...
\begin{info}{XY模型中的拓扑序}{xy-topological-order}
    ...
    Sachdev在\cite{sachdev_xy}和\cite{Sachdev_2018_full}中，从一个三维的扩展的XY模型（可以看成二维的一个强关联的量子XY模型的零温路径积分）出发，通过一个离散Hubbard-Stratonovich变换，引入了$\mathbb{Z}_2$规范场，并且明确地展示了XY模型中的涡旋和$\mathbb{Z}_2$规范场的vison（见\autoref{sec:gauge-charge-flux-z2}）如何冻结为一个自由度。
    这直观地展示了强关联体系如何产生演生规范场和拓扑序，以及KT相变中的涡旋是拓扑序中的任意子的前体这一事实，另一方面也直观地展示了二维规范场和某种粒子（在这里是KT相变中的涡旋）耦合时，规范荷如何被粘到粒子上。
\end{info}
...
从拓扑序的角度看，无物质场耦合的\eqref{eq:z2-2d-hamiltonian}——所谓的\concept{Wegner模型}——实际上是非常病态的一个东西。
Wegner模型中的e激发是完全守恒的，没有任何时间演化：如果一开始某个格点上没有e激发，那么就是没有。
...
然而，我们这里研究\Ztwo规范场是为了研究自旋液体的行为，因此讨论e激发仍然是必要的，即我们需要考虑\eqref{eq:no-charge-constraint-z2}以外的gauge sector。
...
此外，\eqref{eq:z2-spin-liquid-ising-gauge}中的费米子可以没有能隙，从而其中的e激发也是没有能隙的，这和约束\eqref{eq:no-charge-constraint-z2}是矛盾的。
存在e激发的某个gauge sector中的基态中，$\sigma^z_{\vb*{i}}$不再是确定的了（实际上此时系统基态是一个弦网凝聚态），因为我们需要$Q_{\vb*{i}}$有确定的值，但是vison仍然可以是确定的，因为它是Wegner模型的能量本征态。
因此，此时体系的激发态中存在清晰可辨的e激发和m激发，从而有交换相位等等，即系统具有拓扑序。

在约束\eqref{eq:no-charge-constraint-z2}下没有任何量子涨落，$\sigma^x$算符是完全缺席的，此时的vison激发和普通的涡旋毫无区别。
在引入e激发之后（或者等价地说，向哈密顿量中引入\eqref{eq:z2-charge-hamiltonian}，但是让$g$很小，来制造量子涨落），我们就得到了拓扑序。
因此，与\autoref{info:xy-topological-order}类似，可以将约束\eqref{eq:no-charge-constraint-z2}下的vison激发看成$\mathbb{Z}_2$拓扑序中的m激发的前体，只不过此时描写我们得到的拓扑序的有效理论就是$\mathbb{Z}_2$规范理论本身而已。
