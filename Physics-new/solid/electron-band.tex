...
\input{../formalism/perturbation-notime.tex}
...
一维情况下，在加入周期势之后，能带在第一布里渊区边界变平了。
高维空间中设$\vb*{n}$是第一布里渊区的法向量，则$\vb*{n} \cdot \grad_{\vb*{k}} \epsilon_{n \vb*{k}} = 0$。
为什么？TODO
...
另一方面，我们有\eqref{eq:from-spectrum-to-tight-binding}，
由于傅里叶变换是线性的，如果动量空间中的两个函数非常接近，那么实空间中它们的对应物也应该足够接近；
特别的，如果有一条带是平带，那么根据傅里叶变换的唯一性，我们必然发现$t_{\vb*{l}}$在$\vb*{l} \neq 0$时为零。
于是，似乎如果能带较窄的话，电子的跃迁能力——通过$t_{\vb*{l}}$表征——也应该较差。
然而应注意以下微妙的因素：出现在\eqref{eq:from-spectrum-to-tight-binding}中的Wannier函数未必足够局域；
如果这些Wannier函数非常不局域，那么即使$t_{\vb*{l}}$在$\vb*{l} \neq 0$时为零，
我们也不能说电子的跃迁能力真的很差：我们只能说电子的跃迁方式很特别而已。
例如，设有一维晶体，其每个原胞包含两个地位不同的原子，费米面附近的能带由如下紧束缚模型描述：
\begin{equation}
    H = - t \sum_i c^\dagger_{i + 1, \text{A}} c_{i \text{B}} + \text{h.c.},
\end{equation}
其中的$c_{i, \text{A/B}}$涉及的Wannier函数已经经过了适当的局域化。
这个哈密顿量在傅里叶变换后是
\begin{equation}
    H = - t' \sum_{\vb*{k}} \pmqty{c_{\vb*{k} \text{A}}^\dagger & c_{\vb*{k} \text{B}}^\dagger}
    \pmqty{
        0 & \ee^{- \ii k a} \\
        \ee^{\ii k a} & 0
    } \pmqty{ c_{\vb*{k} \text{A}} \\ c_{\vb*{k} \text{B}} }.
\end{equation}
对角化这个矩阵，我们清楚地看到有两条平带，分别位于$\epsilon = \pm t'$；$\epsilon = t'$对应Bloch波函数
\begin{equation}
    \psi_{\vb*{k}, \text{low}} = \frac{1}{\sqrt{2}}
    ( - \psi_{\vb*{k} \text{A}} + \ee^{\ii k a} \psi_{\vb*{k} \text{B}} ),
\end{equation}
而$\epsilon = -t'$对应
\begin{equation}
    \psi_{\vb*{k}, \text{high}} = \frac{1}{\sqrt{2}} 
    ( \psi_{\vb*{k} \text{A}} + \ee^{\ii k a} \psi_{\vb*{k} \text{B}} ),
\end{equation}
对这两个Bloch波函数做离散傅里叶变换，我们得到
\begin{equation}
    w_{i, \text{high}} = \frac{1}{\sqrt{2}} (- w_{i \text{A}} + w_{i \text{B}}), \quad 
    w_{i, \text{low}} = \frac{1}{\sqrt{2}} (w_{i \text{A}} + w_{i \text{B}}).
\end{equation}
可是这并不能够说明这个体系中的电子“跃迁能力差”：事实上，这个体系中的电子可以跨原胞跃迁，于是我们看到，由能带中的Bloch态直接得到的Wannier波函数的确并不局域。
如果我们只保留一条能带——比如说能量较低的能带——那么我们获得的紧束缚模型和单原子极限下电子不从一个原子跃迁到另一个原子，从而形成单带的那种紧束缚模型的形式是完全一致的，可是这两个体系有着巨大的差异，这种差异体现在它们使用的Wannier波函数上。
反过来说，如果我们（通过第一性原理计算发现）某条能带转化而来的Wannier波函数并不能做到局域，这就意味着，如果我们想用一个紧束缚模型描述它，并且这个紧束缚模型基于足够局域的Wannier基底，则我们\emph{必须}引入这条能带以外的能带（在上文的例子中，可以引入$\epsilon = t'$那条能带）。

以上提到的现象——表面上看起来相同的紧束缚模型中Wannier函数可以具有不同的局域性——经常出现在\concept{拓扑能带论}中：
拓扑非平庸的能带在被连续变动到一条平带后，必然和普通的单原子极限的能带有着本质不同，
从而，如果我们要求Wannier函数足够局域，拓扑非平庸的能带一般来说需要通过一个多带模型来捕捉。
不过没有明显的拓扑性质的能带同样可以有这种性质，此时我们说，这条能带具有\concept{fragile topology}。

窄带一般意味着强关联效应，因为此时库仑相互作用的强度压倒了能带电子的动能（实际上是裸的电子的动能加晶格势能）。
不过这倒未必总是导致有趣的效应：原子的内层电子基本上不跃迁，因此能带也非常窄。
然而原子的内层电子是离子实的一部分，所以在一般的凝聚态体系中并不导致什么强关联效应。
对较低能标的理论而言，窄带只有在费米面附近才有关注价值。
