...
```
"""
function read_qe_bands_x_size(path::AbstractString)
    data = readlines(path)
    data = split(data[1])
...
"""
function read_qe_bands_x_energy(band_plot_path::AbstractString, nkp::Integer, nbnd::Integer)
...
"""
Read spin polarization files (`.dat.1`, `.dat.2`, `.dat.3` files)
generated by `bands.x`.

Example: 
```julia
band_dat_path = "WTe2_bands.dat"
spin_dat_path = "WTe2_bands.dat.3" # Or maybe .dat.1, or .dat.2
nkp, nbnd = read_qe_bands_x_size(band_dat_path)
Sz_kn = read_qe_bands_x_spin(spin_dat_path, nkp, nbnd)
```
"""
function read_qe_bands_x_spin(band_plot_path::AbstractString, nks::Integer, nbnd::Integer)
