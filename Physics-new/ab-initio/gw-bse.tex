...
TODO: why can we and should we define the length unit and how it affects the following steps
...
More explicitly, we have (below $m$ means $m_{\text{e}}$)
\begin{equation}
    \begin{aligned}
...
BerkeleyGW, on the other hand, uses a slightly different system of atomic units: 
the Rydberg system, where $e^2$ is 2, instead of 1.
This means 
...
\end{equation}
This expression not only has the same structure as \eqref{eq:light-matter.epsilon-img},
but is actually equal to the latter, 
because the expectation value TODO: this is trivially true when there is no electron-hole interaction, but is it true when there is?
