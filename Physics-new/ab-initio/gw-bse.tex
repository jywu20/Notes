...
As a demonstration of how exciton effects are more conveniently captured by dynamic $GW$, 
we point out that by calculating $G^<$ using time-dependent $GW$, 
...
where in the lowest order approximation, we assume that there is only one exciton in the system,
...
\footnote{
    ...
    \cite{perfetto2016first,rustagi2018photoemission},
...
Indeed for ARPES experimental settings with short probe pulses 
...
It's still possible to add even more terms to the RHS of \eqref{eq:g-lesser-transition-prob},
which in principle can also be traced by correcting the Fermi golden rule.
...
but the effects of most of them are weak.
