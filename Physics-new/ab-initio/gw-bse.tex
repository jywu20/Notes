...
\subsection{The position of imaginary units}
...
\begin{enumerate}
    ...
    \item Do \shortcode{2.2-wfnq} with a \emph{finer} $\vb*{k}$-grid.
    \item Do a \shortcode{epsilon} calculation in \shortcode{1-epsilon}. The steps are listed below:
    \begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            \item Whether we are setting \shortcode{eqpoints} instead of \shortcode{kpoints}.
            \item Make sure the $\Gamma$ point is the \emph{only point} included in the \shortcode{qpoints} block.
            \item Whether the $\Gamma$ point is in the format 
            \begin{lstlisting}
                qx qy qz 1 2 
            \end{lstlisting}
            Here \shortcode{qx qy qz} are coordinates of the displacement made in \shortcode{2.2-wfnq}.
        \end{itemize}
    \end{enumerate}
\end{enumerate}
