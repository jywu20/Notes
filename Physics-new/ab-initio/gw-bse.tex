...
For example, by calculating $G^<$ using time-dependent $GW$, 
we are able to get the ARPES signature that comes from the electron 
in the exciton created by optical pumping:
the ARPES signature can be captured by Fermi golden rule, 
which contains a $\sum_v \abs*{\braket*{c, v}{\text{exciton from light field}}}^2$ factor, 
and by optical theorem we have (note that we don't put $\Im$ before the RHS, 
...
we will soon find that the RHS is just the Dyson series with $GW$ approximation
(note that the propagator in the middle is confined to valence modes, 
and eventually we find the irreducible self-energy diagram 
of the RHS is just the $GW$ diagrams, without any double counting).
Thus the RHS of \eqref{eq:g-lesser-transition-prob} can also be used to 
evaluate the ARPES spectrum; 
and indeed for ARPES experimental settings with short probe pulses 
...
When Fermi golden rule does work, 
in kicking out the electron in ARPES,
the energy conservation relation 
\begin{equation}
    E_{S \vb*{Q}} = \omega - \underbrace{\epsilon_{v, \vb*{k} - \vb*{Q}}}_{< 0}
\end{equation}
that comes with the LHS in \eqref{eq:g-lesser-transition-prob}
is equivalent to ``self-driven'' Floquet effect caused by the $\ee^{- \ii E_{S \vb*{Q}} t}$ factor in $G^<$ to $G^<$ in the RHS \cite{chan2023giant}.
