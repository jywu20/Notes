...
\[
    ...
    \end{tikzpicture}
\]
The band renormalization effects may be explained by non-diagonal terms in Fermi golden rule, 
as the final ARPES response is actually given by the follows:
\[
    \sum \tikzset{every picture/.style={line width=0.75pt}} %set default line width to 0.75pt        
    \begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1, baseline=(XXXX.south) ]
    ...
    \end{tikzpicture}.
\]
Here the $\delta$ function is harder to write 
compared with the case in standard Fermi golden rule.
