...
最后，我们分析伊辛模型的对称性。容易看出它具有格点平移和旋转对称性，以及自旋翻转对称性$Z_2$。
前两个对称性在$a \to 0$时变成连续对称性，但$Z_2$对称性始终是离散的。
...
最后，有必要强调，我们在这里做的事情虽然比使用一个笼统的$m$替换整个系统的状态要靠谱，但\prettyref{sec:reliability-mf}中已经讨论过的那些问题仍然会困扰我们。
有些系统可能就没有长程的场论极限（或者虽然有但是非常不常规），或者可能有非平庸的涌现自由度。
前者的例子如所谓fracton，后者的例子如自旋液体。
我们在这里做的各种推导的有效性其实是有运气的成分的，是所谓的educated guess。
在\prettyref{sec:ab-initio-effective-theory.cutoff}中我们将具体看到从伊辛模型推导$\phi^4$理论的过程中需要做各种危险的近似。
...
更进一步，在临界点附近计算出模型参数的标度律之后可以直接得到临界指数。
我们知道关联长度只应该和温度有关，依照定义有
\[
    \xi t^\nu \sim 1,
\]
于是
\[
    \Delta_{\xi} + \nu \Delta_t = 0,
\]
而由于$\xi$在不动点附近在重整化群下的变换就是长度单位变换（这是肯定的：$\xi$在临界点发散，系统有多大它就能有多大，因此能够修正它的只有长度单位变换，除此以外的细节是不重要的），我们有
\[
    \nu = \frac{1}{\Delta_t}.
\]
而$t \sim \mu^2$（见\eqref{eq:mf-parameter-assumption}相关的论述），于是
\begin{equation}
    \nu = \frac{1}{\Delta_t} = \frac{1}{\Delta_{\mu^2}}.
\end{equation}
同时，根据
\[
    \Delta_\phi - \Delta_B = - \frac{\gamma}{\nu}, \quad \Delta_{\phi} + \Delta_B = d,
\]
有
\[
    2\Delta_B = d + \frac{\gamma}{\nu} = d + 2 - \eta,
\]
于是
\begin{equation}
    \eta = 2 + d - 2 \Delta_B.
\end{equation}
于是可以直接从参数的反常量纲出发计算出$\eta$和$\nu$，进而得到所有临界指数。
具体公式为
\begin{equation}
    \left\{
        \begin{aligned}
            &\alpha = 2 - \frac{d}{\Delta_t}, \quad \beta = \frac{d - \Delta_B}{\Delta_t}, \quad \gamma = \frac{2\Delta_B - d}{\Delta_t}, \\
            &\delta = \frac{\Delta_B}{d-\Delta_B}, \quad \nu = \frac{1}{\Delta_t}, \quad \eta = 2 + d - 2\Delta_B.
        \end{aligned}
    \right.
    \label{eq:from-scaling-to-dimension}
\end{equation}
...
\subsection{$4-\epsilon$展开与Wilson-Fisher不动点}\label{sec:4-epsilon}
