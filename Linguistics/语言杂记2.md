...
看来依存语法和成分语法还是有区别的，因为两种语法暗示的最自然的认知过程是不一样的。

# 2024.12.21

大语言模型的成功，在理论语言学上可以做各种解释。
...
- 有些impossible languages更加subtle。我们注意到（见https://arxiv.org/pdf/2502.18795）大语言模型能学习这些更加subtle的impossible language，虽然似乎表现出了对possible language的偏好。因此有些语法不受人类欢迎似乎是因为domain general的因素，有些则（暂时）无法使用domain general的机制解释。
  又比如https://arxiv.org/abs/2502.12317指出："Our experiments show that these LMs are often slower to learn these subtly implausible languages, while ultimately achieving similar performance on some metrics regardless of typological plausibility."我认为某些impossible language被LLM以更慢的速度完全地学到其实是performance和competence的区别的一个体现：这些语言对LLM来说更加难学（domain general learner's inductive bias），但最后仍然能学到（within competence），而对人类来说可能是beyond the competence of human language faculty的。
- 大语言模型实际上只学到了可能的自然语言的一小部分。这是我本人玩ChatGPT的经验：LLM处理类似于the old man the boat这样需要想一下的句子是吃力的，处理王冕死了父亲这种句子也会出问题。
  ...
  不过，https://direct.mit.edu/ling/article-abstract/55/4/805/113304/Using-Computational-Models-to-Test-Syntactic 一文指出，LLM似乎能够基于不大的样本就学到很多句法知识（如wh-movement）。
  另一方面，[Large Language Models and the Argument from the Poverty of the Stimulus](https://direct.mit.edu/ling/article/doi/10.1162/ling_a_00533/120382/Large-Language-Models-and-the-Argument-from-the)指出前一文可能误读了数据，而一旦stimulus给够，神经网络反而是真的学到了东西，那poverty of stimulus其实是对的。
  类似的研究见https://arxiv.org/abs/2410.18225。
- 我认为大语言模型真正能告诉我们的信息可能是一个类似于“热力学第二定律”这样的东西：“长成这样这样的句子不是可能的英语句子的全部，但它们出于某些原因更加可能出现；还有其他的一些东西，不是任何可能的自然语言的句子，但是它们和前述比较可能出现的自然语言的句子具有某些共性”。作为类比：物理学中的热力学第二定律不能从动力学推导得到，而必须加上诸如“体系周围有庞大的热库“等条件，然而常见的experimental setting恰恰是满足这些条件的；而能用类似热力学第二定律解释的当然不止物理系统，可能还有社会组织等等。
  关于这一论断，请看https://arxiv.org/pdf/2408.09544的图4：LLM language包含了"These languages could be languages that humans will never learn, even if given the relevant input.
  Or they could be extensions of the languages that humans would learn say with English, and these later cases are in fact not only attested and obvi ously very foreign to humans, but there existence is a necessary, mathematical consequence of the form of these models."
...
(但这些stored structure是不是和构式语法所说的一样还是一个问题：LLM平等地攻击所有的理论语言学模型)

# 2025.3.22

其实形式普遍性还有一种，就是句法和语义的对应关系。下面的对应关系大概是不存在的：
- 用vowel quality连续地表达时态（比如说元音越高表示离现在越近）；这种符号体系在人类的视觉认知里面是有的，比如说音量条，但在语言里面没有
- 明确地把事件论元当作一个论元来处理；这样的结果是常规的事件句实际上不存在了，因为事件句可以用谓词句取代：“存在一个事件，它发生在某某时候，它的参与者是某某人，……”然而实际语言将事件句和谓词句用相对统一的结构描述

以上论据表明，一些人所谓的language universal来自共同的经验或者共同的语义框架的论断不能成立，因为同样的语义可以用截然不同的语法表示出来。

# 2025.10.05

在reddit上面逛，有的时候就会看到一些非常草生的对生成语法的批判。
今天看到一个人说语料库语言学说明句子合法性的判断非常gradient，所以生成语法是错的。

照惯例，这个是对gradient的来源的误解，因为显然并没有一种名叫普通话或者标准英语或者大阪腔的脑控机器，可以让这些语言的母语者脑子里装上一模一样的东西：
不同人的I-language总是多多少少会有些不一样的，所以自然当他们产生的语料放在一起的时候，会有gradient；
进一步，人脑子里可以有所谓competing grammar，甚至于如果你只承认microparameter的存在，那语法差异其实就是（抽象的，词缀什么背后的语法范畴也算在内）虚词的用法，那么自然可以有无穷无尽的competing grammar，
所以gradient在单个人的脑子里也是存在的。
生成语法的重点不是说gradient是不是存在，而是很简单的一个问题：“你看，某人承认满足这样、这样、这样的条件的句子通顺，不满足这些条件的句子不通顺，他把注意力放在了这些语法特点上面，说明人类的语言机能能够做这些方面的区分，这说明人类的语言机能有什么样的结构呢？”
对学CS的人来说其实这个是parser架构的问题，如一个正则语法不能很好地判断一个句子是或者不是合乎某种（可能是个人的，见上）自然语言的语法，说明人类的语言机能肯定不是正则的；
然后大名鼎鼎的cross serial dependency in Swiss German又说明只靠上下文无关语法不能定义一个句子在瑞士德语中是不是正确。

类似的，一种常见的指责是“语境会显著地影响一个句子是不是可以接受，所以生成语法是错的”。
可是生成语法当然考虑语境：整个CP层都是在考虑语境。
区别无非是，生成语法把语境要么作为语用的一部分（旧信息、新信息），要么作为一种区别特征，从而把语境纳入了既有的框架里面。

所以靠语料库语言学或者语言的交际功能等等是驳不倒生成语法的。
你要么靠认知神经科学那种办法，要么表明有特定的结构是没法使用feature+roots的框架分析的。
那么后者的一个例子是所谓comparative correlatives, the more the more结构，长期以来有人认为它是一个构式，没法进一步拆分。
但实际上就是可以搜到很多用生成语法分析这个结构的文章，分析得也不错，而且他们做的假设也有其他语言的旁证。

说到底还是，生成语法是结构主义语法的一个必然的延伸，因为既然你使用一些notation去分析语法结构，自然要问“这些notation是不是足够而且必要”，那自然就踏入了生成语法的领域。
实际上哪怕你支持行为主义，那还是要分析哪些行为是可以习得的，哪些不行，那其实你是要给人类的domain general learning ability做一个“语法”了。

# 2025.11.20

我的一些评论:

对emoji的研究早就不少了（基本上是把emoji作为一种句末语气词来研究[赞同]——例子如前一句所示，声调不归最简方案管归音系学管（普通话的声调音系学上没啥反常的）。

“人类这个物种存在以来，人类绝大多数时间里面都没有语言学意义上的语言”这是胡扯，建议还是读一读少数族裔语言的各种调查，观察一下没有复杂语义需求的情况下语法结构仍然能复杂到怎样的程度。

语料足够丰富的情况下你当然可以不把语言看成symbolic的东西，不过你可以试一试两眼一抹黑的情况下去给一个完全陌生的小众语言做描写，看看任何一种data driven的方法用不用得上。藏缅一票小语种面临生存危机呢，你能不用传统方法搞定对这些语言的documentation那当然是很大的突破……

有人反驳：

关于第一点，现在生成范式内对emoji的本质的共识是句末语气词的话，这类研究跟作为象形符号的emoji的研究冲突吗？这两种思路的研究者会互相理解交流，还是老死不相往来？这两种思路的研究者还可以再发十年的文章，但也丝毫不影响llm在此期间已经能产出合适地使用emoji的句子了。关于第三点，我理解作者批判的是生成范式，那生成范式对类型学做出的贡献能达到它从类型学里索取的程度吗？小语种缺的就是描写，类型学学者和小语种学者缺的就是一个趁手的描写工具，既然生成派探索的是语言的内在本质，又何必把自己降格到趁手的描写工具的地位？事实上生成语法作为描写工具也没有超出其他学派、结构主义甚至语文学传统的地方吧。

我的回复：

第一点的话肯定是不冲突的，至于两个思路之间的关系，象形符号是语义而句末语气词是语法，两者算是互补关系吧。

llm的成就非常大，生成圈子对此装聋作哑肯定是不正确的，不过整个传统语言学圈子都很难说对此作出了像样的回应。

第三点我觉得你这个叙述很奇怪，“生成范式对类型学的索取”——好像后者吃亏了……实际上大家各自获得引用量，不是什么坏事。

然后从描写的角度看的话，生成范式对各种scope effect的研究，比如说对复杂句子里形容词、副词的排序的研究，对信息结构如何让语序重排的研究，以及为了验证不同的树的画法哪个更合理而对各种语言细碎细节的探索肯定都是大大拓宽了语言描写的视野的。

那么将生成范式和结构主义和语文学比较的话，前者的一个优势就是会引导你去想“把这些结构组合起来怎样”（如前所述），从而可以大大拓宽描写语法的视野，另一个特点（不知道算不算优势）就是生成范式里面用了哪些假设是明白的告诉你的，而后两者未必，很多描写会变成很模糊的“一个词代表一个想法”这种说法。实际上生成语法和结构主义在描写上是可以通约的，详情可以看邓思颖那本形式汉语句法学。

他的回复：

感谢回复。在第一点里用llm破题是因为，毕竟语言学各流派中唯有生成派是旗帜鲜明地拥护符号主义、rationalism和语言的（特定于人类的）本质主义特性的，其他学派比如认知派可以用它浩繁的术语体系去贴合llm研究，结构主义本身就支持分布语义学，更大范围内的各个功能主义学派由于对语言表现给以了足够的重视所以和llm也没有理念冲突，只有生成派是最需要回应llm的挑战的。关于第三点，（a）从学术生态来讲类型学学者肯定和生成学者是共生关系，类型学学者之所以较多选用生成范式作为描写框架肯定有生成范式自身的优越性在，但大部分情况下类型学的、濒危语种的描写只需用到标准理论和扩展标准理论时期的框架，Xbar和管约论的采用似乎只取决于作者的生成学说素养而不是描写上的必要性，更遑论MP了，实际上MP以来生成派的跨语言描写努力是倒退了（D'Alessandro, R. (2019). The achievements of Generative Syntax）。（b）在验证树的画法谁更合理、假设特征之间谁更有可能这类理论优劣评判的问题上，我看过的生成派文献如果不是诉诸语料库方法（这算什么事），就是最终实际上归结到经济性原则（能追溯到Zipf）、有标无标原则（能追溯到Jespersen或者更早？）之类一般化的认知原则上，像Locality和Antisymmetry这种真正来自生成研究内部的新原则少之又少，有特殊意义的更不多，要支撑UG提出的语言官能的独立和特殊性尚属欠缺。同时，我理解句法与其他层级的界面交互会在多大程度上妨碍句法自治还没有一个定论，在这种前提下，我个人的主张是合语法性要与可接受度一样，不能看作一个categorical/非黑即白的概念，这点如果能成立也是与生成句法的主流方法论相冲突的。（c）最后，在描写所用概念及其定义的清晰性上，生成句法确实天然优于语文学。

我的回复：

你说的很多东西我觉得没错，所以我下面的回复集中在我觉得有争议的地方。

首先我觉得“功能派和llm的理念重合”的说法是不太对的，因为他们其实也没少谈人类的独特性，他们和生成的冲突主要还是集中在domain specific or domain general的问题上。

具体到符号主义的问题上，别的学派我不清楚，能把“我们信什么”说清楚的几个构式语法学派基本上也都是符号主义的（看着有点hpsg的感觉）。所以我说对llm的回应是大家都要想的问题。

你的另外一个论点，就是说合语法性作为gradient的概念，其实有不少人探讨过。一个比较好的比喻是这样的；ising model的结构是离散的，但是它热化以后能产生连续的输出。人的语言官能里面含有硬的结构不妨碍合语法性的gradient的性质，比如说你可以想象一个人的头脑里有多个语法然后他的语言产出是以某些方式抽样出来的：这种情况并不否定语法有硬的结构，就好比ising和Heisenberg虽然有连续的输出但背后的underlying degrees of freedom不一样。

实际上这种连续和离散的对立是比较well established的语言学分支也要面对的问题，最典型的就是青年语法学派的音变规则。音变在语言使用者的群体里面肯定是“软”的，但是不妨碍它背后有一些硬的结构，可以用符号主义的规则来描写。

你的还有一个点，就是说语法自治性的问题，我觉得说“有待观察”没错，但我个人倾向于认为一定程度的语法自治性是存在的。到目前为止的fmri研究都不能说“语法能力就是domain generic problem solving”或者“语法能力就是交流能力”，那最保险的还是假设一个语法能力的存在……关于只属于自然语言的原则的存在性问题，我觉得也有类似的特点，就是“说不清”：我不是很清楚人的非语言能力里面有没有类似于移位以及它受到的各种限制或者形态学（可以看distributed morphology的讨论）这样的东西，只能说我直觉上认为不太有。

我同意GB和MP时代很多人把注意力放到了理论内部，不过我觉得发现看起来这么繁复的结构主义方法到最后可以规约成merge加各种特征的相互作用还是挺有意思的吧。当然他们错过了很多描写小众语言的机会这个没得洗，我确实觉得理论语言学圈子应该多做田野。

我的另一个回复：

然后关于探索语言的内在本质这件事：如果一个学科内的各种现象在一个框架里面都能得到自然的描写，那么确实可以说这个框架是触及本质的，具体到语言学的例子就是结构主义，生成，“类型学语法”（就是Dixon那套所谓basic linguistic theory）都可以说是触及本质的，而且他们彼此实际上也可以通约，前两者的通约我已经写了，dixon虽然反生成但他，以及绝大多数参考语法的作者，在语言描写的时候都不会假定语法结构是语义的完全透明的反映，这个哲学在操作上其实和生成没啥区别的。

自闭症的语言能力研究：

‘Form is easy, meaning is hard’ revisited: (re) characterizing the strengths and weaknesses of language in children with autism spectrum disorder Letitia R. Naigles* and Saime Tek

我的其余回复：

讲真不做约化的方案我们也什么都做不了，其实语言学界的情况和凝聚态有点像。
LLM这个东西让人感觉就是玩Hubbard model或者别的什么模型（软凝聚态那种模型可能更有感觉），能玩出来一些实验上看到的现象，但要定量地说这些模型和实际体系有多像其实很难说。
目前的句法理论像什么呢，就像什么有机化学推箭头、晶体系统里面看到准二维结构就认为有准二维行为、什么笼状结构能装东西这样。

其实如果真的把LLM作为语言学的奠基，那就可能需要接受某些小众语言可能永远写不出来让人满意的参考语法，纯靠某种也许可以定量描述但没法symbolic描写的vibe，很多人会觉得这个逻辑上的后果过于不同寻常。

其实价键理论这个比喻挺好，就好比machine learning potential跑分子动力学时没有使用任何symbolic的价键理论，但是最后跑出来的东西又可以画分子式然后各种符号化分析。

涌现的概念多讲其实没意思，凝聚态上能讲涌现是因为有universality class之类的工具可以用，语言学上没有这种东西能用。

# 2026.02.20

“纯”描写语言学训练的人其实也多少能意识到一点生成语法有道理的地方。
比如说你做argument polypersonal indexation，如果采用agent→instrument→theme这样的记号，其实就是在隐含地承认论元处在一个hierarchy上，那其实已经承认了VP shell理论了。

但是现在大背景下面骂乔派的人太扶手椅是政治正确，你不是向柏霖你也不敢在画树的时候搞X-bar对吧。

# 2026.05.29

整理电子书的时候注意到的没存下来的引用：关于最简方案和心理语言学的关系：Merging Generative Linguistics and Psycholinguistics一文, by Jordi Martorell