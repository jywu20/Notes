...
那么将生成范式和结构主义和语文学比较的话，前者的一个优势就是会引导你去想“把这些结构组合起来怎样”（如前所述），另一个特点（不知道算不算优势）就是生成范式里面用了哪些假设是明白的告诉你的，而后两者未必，很多描写会变成很模糊的“一个词代表一个想法”这种说法。实际上生成语法和结构主义在描写上是可以通约的，详情可以看邓思颖那本形式汉语句法学。
