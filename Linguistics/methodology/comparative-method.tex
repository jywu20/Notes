...
which is then placed into a rigid template; \prettyref{sec:morphological-wordhood-template}).
...
\subsubsection{Phonological realization guided by lexicon}
...
and \emph{slot-filler} morphology, which is linear and flat and defined in terms of slots,
...
which is indeed what is observed in Chintang \citep{bickel2007free}.

\subsubsection{Comparison with morphological words in parsing}\label{sec:morphological-wordhood-template}
...
roughly speaking this means whatever seems to have a fixed morphological pattern
is recognized as a morphological word.
...
\prettyref{sec:derivation-wordhood} and \prettyref{sec:phase-wordhood}.

First, we have a ``morphological templates'' identified in a surface-oriented analysis
that do not seem to need any post-syntactic relocations.
...
This is indeed the case of Modern Japanese discussed in \prettyref{sec:phase-wordhood}.

Second, we have \concept{clitics},
which, just like ordinary affixes, needs to be attached to something,
but they are less picky in choosing the host.
...
\emph{seems} to suggest that they pass the morphological wordhood test in \prettyref{sec:morphological-wordhood-production},
as they are dislocated by morphological rules as well \citep{harley1999distributed},
But they do not pass the morphological wordhood test in that they are not a fixed part of any fixed morphological pattern.
...
on the other hand, affixes reordering is also possible, but it does not incorporate a clitic into the units they have already formed
\citep{embick2007linearization}.
This generalization, if true, 
eliminates the subtlety of morphological wordhood because of the existence of clitics.
There are however controversies on whether a clitic can be incorporated into these units
formed by typical affixes and possibly roots.
It seems that in Udi, we have personal agreement formatives
that can be attached to both focused elements and verbs,
with their position in the verb morphological template being variable \citep{harris2000word}.
...
In Chintang, prefixes and clitics have quite similar behaviors in the phonology-driven reordering process,
although the prefixes pass clear grammatical wordhood (i.e. morphological wordhood in this note) tests \citep{bickel2007free}.

\subsection{Phonological wordhood}

Phonological wordhood does not always respect syntactic ``wordhood'' defined in \prettyref{sec:derivation-wordhood} or \prettyref{sec:phase-wordhood}.
...
TODO: split of phonological word: 支楞起来
...
We emphasize that what is discussed in the whole \prettyref{sec:grammar} is
the ideal competence of a person already fluent in a language.
An actual human learner, despite being able to acquire a 
