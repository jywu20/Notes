...
\begin{exe}
    \ex\label{ex:np-structure-corrected} [the_{\text{\category{definiteness}}} two_{\text{\textcolor{blue}{\category{plural}}}} [ugly [black-bird]_{\text{nominal compound}}\textcolor{blue}{-s_{\text{\category{plural}}}}]_{\text{modification}}]_{\text{noun phrase}}
\end{exe}
...
although it is not as drastic as \prettyref{sec:derivation-deep-tree} and \prettyref{sec:phase-flatten-tree}.
...
and it does not always respect syntactic ``wordhood'' defined in \prettyref{sec:derivation-deep-tree} or \prettyref{sec:phase-flatten-tree}.
...
in the sense of \prettyref{sec:derivation-deep-tree} or \prettyref{sec:phase-flatten-tree},
...
There, a morphological word (i.e. the realization of a flattened-tree syntactic ``word'' defined in \prettyref{sec:phase-flatten-tree}) breaks into pieces,
...
A big unit like a clause is likely decomposed when transmitted.%
\footnote{
...
Language change involves both phonological and morphosyntactic changes.
...
As extension creates new forms which are absent in the parent language,
reanalysis should be our main focus when studying the unit of language transmission
from the morphosyntactic perspective.
Below, we consider certain situations where the definition of \term{word}
is unclear, and discuss the units historical linguistic laws work on in these situations.

\subsection{Phonological word containing multiple morphological words}

When a phonological word consists of multiple morphological words,
will the phonological word be the transmitted unit,
or will the morphological words be the transmitted unit?
...
We may want to argue that flattened-tree syntactic ``wordhood'' (\prettyref{sec:phase-flatten-tree})
...
(like those in \prettyref{sec:phase-flatten-tree}),
