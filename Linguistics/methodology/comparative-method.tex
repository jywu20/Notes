...
each containing a half of the stem, which also gets split in the process.
TODO: ref
...
\footnote{
    ...
    We aim to avoid buzzwords without theorization in this note.
}
...
or more rarely leads to back-formation,
...
\subsubsection{Size limit in reanalysis and phonological wordhood}\label{sec:reanalysis-size-limit-phonological-word}
...
\subsubsection{Reanalysis of phonological words without clear morphosyntactic structure?}
...
Furthermore, the phonological word \form{to=the=fat} at least contains three morphological words, no more, no less,
just like \form{an=uncle} or \form{who's=done=it},
...
It is indeed possible that a phonological word containing
...
clipping in English disproportionally creates nouns with a diminutive meaning
\citep{jamet2009morphophonological}
and a large number of clipped forms can be analyzed using template morphology,
...
\subsubsection{Extremely big morphological words}
...
We summarize our discussions above into the following list:

\begin{itemize}
    \item In \emph{syntactic} reanalysis,
    the basic unit of transmission is an idiom, 
    and what actually gets changed in the surface form, if any,
    is usually a phonological word with a certain morphosyntactic status,
    often the fixed part of the idiom (\prettyref{sec:syntactic-reanalysis}).
    \item In \emph{morphological} reanalysis,
    ...
    or a phrase or even a clause (\prettyref{sec:reanalysis-form-change}).
...
TODO: more discussions on clipping
...
For words with simple morphological structures, focusing on content words is just another way to say focusing on the roots.
