...
\usepackage{xr-hyper}
\usepackage[colorlinks, linkcolor=black, anchorcolor=black, citecolor=black, filecolor=black, bookmarksnumbered]{hyperref} % linkcolor=black, anchorcolor=black, citecolor=black, filecolor=black
...
The morphology of Semitic languages, Hebrew included, is famous for its rich system of nonlinear verbal and nominal morphology based on roots and patterns.



Although the nonlinear morphological pattern is often analyzed in terms of roots and patterns,
a recent trend is to argue that ``word-based'' morphology is sufficient to capture the morphological phenomena in Semitic languages (\prettyref{sec:morphology.general}). 
It should be noted that the controversy is more about realizational aspects of morphology and is not contradictory to the idea of the root, without part of speech tags, as a synchronically valid unit in grammar.
Indeed, \citet[\citepage{21}]{wilson2020concatenative} explicitly mentions that these so-called word-based analyses essentially ``take as their underived \emph{root} form an articulable and fully vocalized root morpheme\dots''
...
Noun phrases most frequently appear in usual argument positions.
Hebrew in general lacks a case system.
There exists an object marker \form{et}.
Hebrew also has a preposition system.
Topicalization and focalization is well attested in Hebrew without explicit markers.
However, Modern Hebrew \acp{np} do not permit \category{wh}-extractions
\citep{shlonsky2012some}.

The internal structure of a noun phrase, just like that of a clause,
can also be divided into a hierarchy of systems.
(\ref{ex:grammatical.np.1}) illustrates a typical Hebrew noun phrase with a preposition,
with different systems labeled by colors.

\begin{exe}
    \ex\label{ex:grammatical.np.1}
    \gll bi- [[ktav \textcolor{red}{ha}-ishum]_{\text{\textcolor{cyan}{modified}: \textcolor{blue}{CP}}} [\textcolor{red}{ha}-rishon-\textcolor{cyan}{∅}]_{\text{\textcolor{cyan}{attr}: ordinal}}]_{\text{\textcolor{red}{definite} NP}} \\
    in write<\category{nmz}>:\textcolor{blue}{\category{cs}} \textcolor{red}{\category{def}}-guilt<\category{nmz}> \textcolor{red}{\category{def}}-first-\textcolor{cyan}{\category{sg.m}} \\
    \glt\translate{in the first writing of accusation (i.e. the first indictment)}
\end{exe}

Definiteness in Hebrew is a part of the nominal inflectional paradigm in Hebrew.
Definiteness is marked by the prefix \form{\textcolor{red}{ha-}}
and spreads to every word that inflects according to the nominal paradigm in a noun phrase
(except words in the construct state; see below);
indefiniteness has no explicit marker.

Agreement of gender and number between attributives and forms they modify can also be seen in (\ref{ex:grammatical.np.1}),
colored in \textcolor{cyan}{cyan}.
The usual order is for attributives to appear after the head.

Further, in (\ref{ex:grammatical.np.1}) that the possessive structure \form{ktav ha-ishum} does not contain a genitive case form or a preposition.
This is known as a \concept{construct phrase} (\textcolor{blue}{CP} in \ref{ex:grammatical.np.1}),
which is a unique genitive construction seen not only in Hebrew
...
known as the \concept{construct state} (\textcolor{blue}{\category{cs}} in \ref{ex:grammatical.np.1}),
while the possessor is marked for its definiteness (or lack of).

The possessor can also be a construct phrase,
containing one possessor modifying the possessed (without definiteness marking).
Repeating this process, we have a chain of nouns/coordinated nouns
meaning \translate{A of B of C of \dots}
...
Because of the adjective-after-head word order,
almost all adjectives appear after the construct chain;
the definiteness agreement rule also means all adjectives following the chain also agree with the definiteness value of the last component in the chain.

We note that this linear order has several structural ambiguities.
The word order corresponding to \translate{the colorful chairs of the new class}
...
but the same surface form can also mean \translate{the chairs of the colorful new class}.%
\footnote{
    [chairs-\category{m.pl} [\category{def}-class-\category{f} \category{def}-new-\category{f} \category{def}-colorful-\category{m.pl}]]
}
Moreover, the relative scopes of the possessive phrases and other constituents in a noun phrase can't be seen from the linear order.%
\footnote{
    In English, for instance, we have \form{these teachers' desks},
    in which the possessive is comparable to the subject,
    and \form{teacher's desk} TODO
}

Besides the construct phrase, Hebrew also has the \form{shel} genitive,
which is a genitive construction almost identical to English \form{of} genitive.
...
\section{Semitic templatic morphology}\label{sec:morphology.general}
...
\subsection{Is there stem-based morphology?}
...
\subsection{Are there really templates?}
...
\footnote{
    ...
    but this requires us to stipulate that there exists an abstract component instructing 
    vowel copying in the template as well,
    before things are put together.
    This is extremely unnatural, if not theoretically infeasible.
}
...
\subsection{Are there consonant skeletons?}
...
a ``word-based'' morphology \citep{benmamoun20085}.
