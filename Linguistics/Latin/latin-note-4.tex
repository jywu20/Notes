...
but closer examination reveals that the order alternations
can be explained by information packaging and/or prosody
(\prettyref{chap:order}).

At the surface level, the nucleus clause contains a (omissible) subject
(\prettyref{sec:grammatical.clause.subject}),
a main verb, a possible auxiliary verb,
and possible adjuncts (including peripheral arguments and \ac{tam}-adverbials)
and internal arguments.\footnote{
...
The inflectional ending of the main verb (\prettyref{chap:verb}), 
together with \ac{tam} adverbials and an optional auxiliary verb,
marks \ac{tam} grammatical categories (\prettyref{sec:grammatical.clause.tam}).
The distinction between what are known as peripheral arguments and core arguments
...
\begin{todobox}{What else to be done for the subject}{subject-todo}
    \begin{itemize}
        \item Passivization
        \item Psych-verb (\prettyref{box:psych-verbs})
...
\begin{todobox}{Core arguments: what are they?}{core-arguments}
    \begin{itemize}
        \item Structural affinity to the verb: 
        anaphora in space of \ac{vp}
        \item Obligatoriness: finiteness
    \end{itemize}
\end{todobox}
...
The term \term{peripheral arguments} is used in \citet{dixon2009basic1}
...
Parameters traditionally taken as core-peripheral distinctions
involve structural closeness of the clausal dependent to the main verb,
obligatoriness of the argument,
and the internal makeup of the argument.

\subsection{Core arguments} 

\paragraph*{Prototypical transitive construction}\label{sec:grammatical.clause.core.transitive}
In Latin 
...
may have behaviors comparable to A or P in prototypical transitive constructions:
...
In Latin, it seems that unaccusative verbs are all deponent
\citep[\citepages{308-309}]{oniga2014latin}

\paragraph*{Benefactive constructions}
Verbs meaning \translate{giving} in Latin 

\paragraph*{Directional constructions}
Some Latin verb frames seem to have a directional small clause
(\prettyref{sec:grammatical.clause.small}) hidden in them.
In the surface form, 
this results in a preverb
(usually the \emph{path} in the directional construction)
appearing before the verb root,
and the \emph{location} appears as an argument of the verb.
The case marking of the location argument may reflect
what type of location it is
(and hence is dictated by the preverb,
as if the preverb is used as a standalone preposition
and the location is governed by it),
or in may appear in accusative.
...
we delay our detailed discussions to \prettyref{chap:order}.
...
Also, grammaticalization from lexical words to functional words is not synchronic among speakers.
...
\chapter{Constituent order and the information structure}\label{chap:order}
