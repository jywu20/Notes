...
We carry out this report and this chapter round by round: 
first we distinguish words and morphemes, 
then we briefly describe the clause structure and the noun phrase structure,
and then according to how roots fit into these environment,
we define parts of speech. 
The following chapters each focus on a different topic touched in this chapter.
...
and a neutral order can be established; 
order alternations on top of it
can be explained by information packaging and/or prosody (\prettyref{chap:order}).

At the surface level, the nucleus clause contains a (omissible) subject
(\prettyref{sec:grammatical.clause.subject}),
a main verb and a possible auxiliary verb (\prettyref{chap:verb}),
...
the semantics difference between S=A and S=P valency alternations (see below), etc.
The mapping from deep positions to surface positions
(like \term{subject}) is influenced by the \concept{alignment typology} of the language
and also \concept{voice} constructions.
...
The argument structure is often related to the \emph{event structure} of the verb,
and therefore the verb frame may also influence the \ac{tam} marking
(\prettyref{chap:verb-frame}).

Regarding alignment typology,
...
\begin{todobox}{What else to be done for the subject}{subject-todo}
    \begin{itemize}
        \item Passivization
        \item Describe phenomena in the last paragraph.
    \end{itemize}
\end{todobox}

\begin{todobox}{Clause template}{clause-template}
    When all information packaging devices are removed, 
    we have SOV order, and the position of O varies depending on its information structure.
\end{todobox}
...
\begin{itemize}
    ...
    We define the core argument structure as a structure expressing 
...
\begin{todobox}{Obligatoriness}{obligatoriness}
    
    
    Because of semantic or pragmatic reasons,
    a main verb still has some control over its adjuncts:
    \form{it rains} will never have an instrument adjunct.
    Is it also possible for the main verb to select structural adjuncts
    in the \emph{syntactic} way?
    \form{They treated us rather badly} -- any structural test to decide whether the manner phrase is argument or adjunct?
    
    \citet{mcinnerney2022argument} claims that in English the post-verbal constituents have a flat structure.
...
\paragraph*{Manner}
In Latin the manner phrase also seems to modify the whole core argument structure,
and therefore is considered peripheral \cite[\citepages{101-109}]{devine2006latin}.
...
core arguments have deep and surface status in the clause.
...
The \concept{direct object} is the surface grammatical relation of the P argument
...
\begin{todobox}{Indirect object, secondary object}{indirect-object}
    Is the concept \concept{indirect object} universal enough? 
\end{todobox}
...
involves an agent, which initiates the event
and exists independently of the event,
and a patient argument, which is affected by the event,
...
We can also observe S=A and S=P labile valency alternation.
...
if we acknowledge that the subject or the object is a core argument,
...
\begin{todobox}{Instrument argument?}{instrument-argument}
    How to show that an instrument argument is indeed a core argument?
\end{todobox}
...
\begin{itemize}
    ...
    By default, the direct object appears first,
    indicating that 

    This order may be alternated to reflect
    realignment of the (rather weak) information structure statuses of the arguments
    within the extended argument structure

    This does not change the broad scope focus of the clause.

    \item \emph{Scrambling for presupposition}.
    There exist several ``weaker'' information packaging constructions
    which promote a constituent to a position lower than the subject

    where a constituent is shifted leftward 
    ...
    In Latin, we have strong and more prototypical topic and focus constructions
    that change this information structure 
...
Grammaticalized syntactic objects strictly speaking should not be included as a part of the lexicon:
...
The distinction between lexical and functional prepositions is hard for a classical language,
...
but there is no basis to analyze them as grammaticalized directional markers:

It however is also possible that a preposition is an adverbial modifier of an existing prepositional construction
...
\section{The argument-adjunct distinction}
...
\begin{infobox}{Psych-verbs}{psych-verbs}
    Structural and semantic properties of psych-verbs; UTAH.
\end{infobox}
...
\subsubsection{\form{Accusativus cum infinitivo}, or the autonomous infinitive}\label{sec:complement-clause.infinitive.aci}
