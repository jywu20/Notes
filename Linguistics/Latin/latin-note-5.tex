...
\ac{tam} adverbs, and core and peripheral arguments.
...
as these phenomena can still be understood in a movement-based account \citep[\citepages{73-75}]{danckaert2017development}.
...
It is rare, if not impossible, for \form{non} used as a clausal negator (and not in e.g. \form{non \dots{} sed \dots}) to appear before the highest verb/auxiliary in a clause \citep[\citepages{39-44}]{danckaert2017development}.
...
The main difference between Latin (which now should be more accurately known as a discourse-configurational language) and languages with more rigid word order therefore can be attributed to the former having far more information structural devices.

First, in broad focus sentences in which the whole situation of affairs described by the sentence is the focus
(and hence sentence-level topicalization and focalization should be absent),
...
This means that Latin has both core verb phrase-level information structure movements
and what is known in other languages as scrambling,
which lifts arguments to the region of adverbs with scopes over core and peripheral arguments.

Third, sentence-level topicalization and focalization obviously exist in Latin. 



\subsubsection{``Small clauses''}\label{sec:grammatical.clause.small}
...
Third, Latin subject often has specific, wide scope interpretation. 

Agreement may be influenced by factors other than structural prominence of an argument,
...
In clauses with neutral information structure,
...
In traditional grammar, the non-subject parts of a clause are collectively known as the predicate,
containing \ac{tam} markers (\prettyref{sec:grammatical.clause.tam})
and the (extended) argument structure (\prettyref{sec:grammatical.clause.peripheral}, \prettyref{sec:grammatical.clause.core}).
