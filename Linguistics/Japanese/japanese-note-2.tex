...
(\prettyref{sec:adjectival-noun-internal-form}),
...
(\prettyref{sec:auxiliary-verb-suffix}, \prettyref{sec:verb-complex-previous}).
In functional items,
...
morphologically, we have plain and polite forms (\prettyref{tbl:tense-polarity-politeness}).
Besides, we also have derivational honorifics prefixes (\prettyref{sec:verb-honorifics})
and periphrastic ways to make a clause ``polite'' (\prettyref{sec:honorifics-constructions}).
These constructions fit in the larger picture of politeness in Japanese (\prettyref{sec:politeness-general}).
...
(\prettyref{sec:nominal-polite}, \prettyref{sec:tense-polarity-polite}, 
\prettyref{sec:verb-honorifics}, \prettyref{sec:honorifics-constructions}).
...
\subsubsection{Euphonic changes related to \corpus{t}}\label{sec:t-euphonic}

In the past tense (\prettyref{sec:indicative-paradigm}),
the \corpus{te}-form (\prettyref{sec:te-form}),
and the condition form (TODO:ref),
we see the need to put a suffix starting with \corpus{-t-} to a base ending with a consonant.
As is said in \prettyref{sec:suffix-chain},
the default way is to insert an \corpus{i} after the consonant,
but this \corpus{Cit} sequence has already observed some historical sound change,
which results in a new form in the internal morphology of verbal elements (\prettyref{sec:euphonic-form}).
...
\begin{table}[H]
    ...
    \begin{tabular}{cccccc}
        ...
        s & さ \corpus{sa} & し \corpus{shi} & す \corpus{su} & せ \corpus{se} & そ \corpus{so} \\ 
        z & ざ & じ & ず & ぜ & ぞ \\ 
        t & た & ち & つ & て & と \\ 
        d & だ & ぢ & づ & で & ど \\ 
        n & な & に & ぬ & ね & の \\ 
...
\subsection{The dative \corpus{ni}}

\subsubsection{Purpose}\label{sec:purpose-ni}

The particle \corpus{ni} is also used to indicate purpose.
In this case, an infinitive clause -- a clause ending with the continuative form (\prettyref{sec:non-finite}) 
-- may appear before \corpus{ni} 
to indicate the purpose of the matrix clause.
...
in traditional Korean grammars (\prettyref{box:affix-conjugation-theory}).
...
\prettyref{sec:auxiliary-verb-suffix} justifies this distinction.
...
\begin{theorybox}{About internal morphology of conjugation suffixes}{affix-conjugation-theory}
    ...
    the progressive verb\corpus{-te iru} construction (\prettyref{sec:te-iru})
    as verb-\corpus{teiru}.
    That is, he recognizes the suffix \corpus{te} 
    -- usually analyzed as morphologically the same as 
    the \corpus{-te} in the non-finite \corpus{te}-form (\prettyref{sec:te-form}),
    ...
    except perhaps \citet[\citepage{76}]{hasegawa2014japanese} -- 
    ...
    For a more precise discussion on the first approach, see \prettyref{box:internal-morphology-terminology}.
\end{theorybox}
...
\begin{theorybox}{Terminological notes}{internal-morphology-terminology}
    ...
    The form of a base (one of those listed in \prettyref{sec:internal-forms}) 
...
(\prettyref{sec:auxiliary-verb-suffix}),
...
claim that the -\corpus{mai} construction (\prettyref{sec:mai-mood}) 
...
and hence the name.
It's obtained by inserting \corpus{-i} to the end of a c-stem,
and doing nothing to a v-stem.
...
(\prettyref{sec:copula-da}, \prettyref{sec:adjectival-noun-internal-form}).
This is the only synchronic reason to maintain a distinction 
between the attributive form and the terminal form.

\subsubsection{The hypothetical form}\label{sec:hypothetical-form}
...
Similar to the case in the irrealis form (\prettyref{sec:irrealis-form}),
the hypothetical form seems to be the application of the ``removing the second \corpus{r}'' rule 
(\prettyref{sec:removing-rs}).
...
It's historically the stem plus \corpus{-ya-u},
and the \corpus{y} sound is deleted after a consonant,
so in the same way we say valency changing constructions apply on the irrealis form, 
we say the volition form was obtained by adding \corpus{-yau} to the irrealis form.
Then sound changes happened,
and \corpus{-au} became \corpus{-\={o}}.
Some books call it \term{the second irrealis form}.
It's used in the volitional mood and idiomatic constructions involving the volitional mood ending 
(\prettyref{sec:volition-mood}).
...
Note that the creation of a new form doesn't summarize the consonant contraction rules.
...
We can still define the base forms mentioned in \prettyref{sec:internal-forms},
but this doesn't predict the full paradigm of,
say, the tense-politeness-polarity complex (\prettyref{sec:tense-polarity-polite}).

The base forms mentioned in \prettyref{sec:internal-forms} for \corpus{da} is listed 
in \prettyref{tbl:da-conjugation}.
...
\corpus{de aru} is the continuative form plus the attributive form of \corpus{aru} (\prettyref{sec:aru}).
TODO: the origin of \corpus{are}
This paradigm is seen in the conjugation of adjectival nouns (\prettyref{sec:adjectival-noun-internal-form}).
...
\begin{tabular}{lcl}
    \toprule
                                    & \multicolumn{2}{c}{bases}                                                                                                                                  \\ \midrule
{Irrealis}     & \multicolumn{1}{c}{\begin{tabular}[c]{@{}c@{}}だろ\\ では\end{tabular}}                        & \begin{tabular}[c]{@{}l@{}}\corpus{daro}\\ \corpus{de wa}\end{tabular} \\
{Continuative} & \multicolumn{1}{c}{で}                                                                      & \corpus{de}                                                            \\
{Terminal}     & \multicolumn{1}{c}{だ}                                               & \corpus{da}                                                            \\
{Attributive}  & \multicolumn{1}{c}{{\begin{tabular}[c]{@{}c@{}}である\\ な\end{tabular}}} & \begin{tabular}[c]{@{}l@{}}\corpus{de aru} \\ \corpus{na}\end{tabular}           \\
{Hypothetical} & \multicolumn{1}{c}{\begin{tabular}[c]{@{}c@{}}であれ\\ なら\end{tabular}}                       & \begin{tabular}[c]{@{}l@{}} \corpus{de are} \\ \corpus{nara} \end{tabular}         \\
...
TODO: whether the -na ending in the attributive form is still in contemporary use (\prettyref{sec:copula-da}).
...
so it involves a part of \prettyref{sec:tense-polarity-polite},
but not all of it).
In the latter cases, 
...
As is said in \prettyref{sec:auxiliary-verb-suffix},
though in Japanese, 
...
the past polite negative form in the indicative paradigm (\prettyref{sec:indicative-paradigm}), 
...
The case of non-finite clauses is discussed in \prettyref{sec:non-finite}.
...
(\prettyref{sec:daro-form}).
\prettyref{tbl:tense-polarity-politeness} shows the indicative paradigm for regular verbs. 
...
Besides the modern forms listed in \prettyref{tbl:tense-polarity-politeness},
the \corpus{-nai} negative non-past plain form has two more archaic and dialectal allomorphs: 
\corpus{-nu} and \corpus{-n}.
The latter is the regular contraction of the former.
The two markers are all attached to an irrealis base.
Using the two negators are considered somehow informal,
but \corpus{-n} is used at the end of the \corpus{-masen} form,
and the latter is considered formal.

It should be noted that \prettyref{tbl:tense-polarity-politeness} is not the whole story 
...
(\prettyref{sec:honorifics-constructions})
to convey higher degree of politeness.
And although only tense is shown in the paradigm,
Japanese does have periphrastic constructions for aspect (\prettyref{sec:aspect}).
...
In the ending \corpus{-masen},
the final \corpus{-n} is the contracted version of \corpus{-nu} discussed above,
while the \corpus{-mase-} part is the contraction of the earlier \corpus{mawirase-} \translate{to do, to offer}.
We may say \corpus{-mase-} is the irrealis form of \corpus{masu},
because historically it was so,
and synchronically, in other cases \corpus{-n} is attached to an irrealis base.
...
(the volition form of \corpus{da} in \prettyref{tbl:da-conjugation})
...
may be seen as another presumptive mood beside the \corpus{dar\={o}} form (\prettyref{sec:daro-form}),
...
TODO: 惯用型 https://zhuanlan.zhihu.com/p/35089043

\subsection{The \corpus{-mai} negative volitional mood}\label{sec:mai-mood}
...
This mood is largely replaced by the negative \corpus{dar\={o}} form (\prettyref{sec:daro-form}).
...
TODO: distribution: as the purpose (\prettyref{sec:purpose-ni})
...
a continuative base plus \corpus{-te} plus \corpus{onbin} rules (\prettyref{sec:t-euphonic}).
...
It's called the gerund because it is used to form the periphrastic progressive aspect 
in a way resembling the English \corpus{be doing} progressive.

The negative marker \corpus{-zu}; 
the particle \corpus{ni} of purpose -- where should I place it?

\subsection{The \corpus{-ba} conditional mood} \label{sec:eba-form}

Attaching \corpus{-eba} to a c-stem and \corpus{-reba} to a v-stem 
and we get the \corpus{-ba} conditional mood.
In other words, attaching \corpus{-ba} to the hypothetical form of a c-stem 
(\prettyref{sec:hypothetical-form})
and \corpus{-reba} to the hypothetical form of a v-stem,
and we get the \corpus{-ba} conditional mood.
This alternation of the form of the conditional mood marker seems to be yet another example of 
the ``no second \corpus{r}'' rule (\prettyref{sec:removing-rs}),
and therefore we can analyze the deep form of 
the conditional mood marker involved in the \corpus{-ba} construction 
as \corpus{-reba},
in which the \corpus{r} sound is thrown away when the suffix is added to a c-stem.
This is also the likely origin of the hypothetical form:
since \corpus{r} is lost for c-stems,
effectively, attaching \corpus{-reba} to a c-stem 
means to add \corpus{-e} to the end of the stem and then attach \corpus{-ba}.
...
When the polite forms of the verb (\prettyref{sec:tense-polarity-polite}) 
...
Some adverbial constructions are already covered in the framework of adverbial \ac{np}s 
(\prettyref{sec:purpose-ni}, TODO).
