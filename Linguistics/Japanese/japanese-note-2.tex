...
This is exemplified in marking of the passive and the causative (\prettyref{sec:valency-changing}),
TODO: list
...
\begin{learnbox}{Remembering kanas}{kana-remember}
    ...
    (\prettyref{sec:valency-changing}),
...
来る, 青い.
When conjugated, the final kana indicates the word's internal morphology (\prettyref{sec:conjugation-class}). 
...
and then use it to construct a progressive construction (\prettyref{sec:progressive-aspect}). 
...
and the combination of the two (\prettyref{sec:valency-changing}). 
...
\begin{enumerate}
    \item The valency changing part (\prettyref{sec:valency-changing}).
    \item TODO: desiderative taku
    \item The tense, polarity and politeness complex (\prettyref{sec:tense-polarity-polite}).
\end{enumerate}
...
(like \prettyref{sec:tense-polarity-polite}) 
...
\subsection{Tense, polarity and politeness}\label{sec:tense-polarity-polite}
...
\subsubsection{The indicative paradigm}\label{sec:indicative-paradigm}
