...
\begin{theorybox}{Negation in Cartography}{negation-position}
    ...
    is prevalent: Both narrowing and widening the scope are possible 
    \cite[\citepage{40}]{moscati2010negation}.
    The exact status of negation in syntax is still a debated topic.
    As is said above, if we assume the syntactic structure faithfully 
    ...
    that there now seems to be ``real'' head movements that change the semantics.
    The Cartography tradition however tend to treat syntactic negation markers as specifiers
    (\citealt[\citepage{101}]{zanuttini1997negation};
    \citealt[\citesec{4.7}]{radford2009analysing})
    -- essentially the negation-like-adverb approach introduced above in the main text -- 
    and this scope widening approach at least doesn't face theoretical barriers.

    In either case, the position of PF-active NegP varies cross-linguistically.
...
(\prettyref{chap:information-packaging}),
