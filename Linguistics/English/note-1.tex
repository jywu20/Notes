...
\section{The template of the noun phrase}
...
\begin{theorybox}{Cartography of DPs}{dp-structure}
    In Cartography, 
    the determiner function D_{\text{det}} is similar to the role of FinP in the split CP,
    ...
    There is a D_{\text{deixis}} position 
...
It's possible for a nominal to appear without any determiner (\prettyref{sec:np.nominal.nominal-attributive}).
Above the nominal layer, 
we have a series of determiner-like grammatical functions,
filled by various forms.
Here I intentionally avoid labeling anything in \prettyref{fig:np-template} 
as the determiner,
because TODO 
...
\section{Semantic concepts}\label{sec:np.semantics}

It should be noted that the surface realization of the template \prettyref{fig:np-template} 
...
\subsection{\acs{np} in clauses}

The interpretation of a form \corpus{\dots something \dots} basically is 
...
this leads to the concept of definiteness (\prettyref{sec:semantic.definite}).

The speaker/writer may only talk about a certain part of the objects retrieved
from the conversational context, 
or he or she may talk about all of them; 
the \acs{np} may be interpreted as a whole in the predication,
...
or does it mean the student selects the four courses into the worksheet of the course selection system 
and submits the selection at once?)

Another parameter is about (\prettyref{sec:semantic.ref}).
...
is usually the determiner function \prettyref{sec:np.det}.
...
because the nominal \corpus{man} \emph{itself} doesn't contribute to this uniqueness condition:
The level of identifiability required for definiteness 
is with the current conversational context and the nominal,
the uniqueness condition can be established.
...
Thus, at the end of \prettyref{sec:np.semantic.identify} TODO

It should be noted that there is something asymmetric between definiteness and indefiniteness:

TODO: the status of indefinite is not the same as the status of definite,
syntactically and semantically \citep{gianollo2021reference,klockmann2020article}
...
while definiteness 
(to be understood as Russell's $\iota$ (\prettyref{box:theory-of-description}))
...
Compare, say, \corpus{all [examples]_{\text{head}} of these statements}
