...
though \citet{cgel} seems to want to achieve a compromise between lexical and functional heads 
...
``freely added adjuncts or modifiers fundamentally different from '' (\prettyref{box:tp}).
...
\begin{infobox}{Two-step description of verb-argument relations v.s. one-step description}{verb-argument-relation}
    ...
    I will not do a complete argument structure-clause structure division here,
    but it's a good idea to summarize types of clausal complements 
    and argument positions in \prettyref{sec:valency.overview},
    and then go on to discuss how they are matched with each other 
    with each type of verbs.
\end{infobox}

\section{Overview of arguments of verbs}\label{sec:valency.overview}

\subsection{Arguments in clausal dependents}\label{sec:valency.overview.dependents}

Putting grammatical items aside,
...
\subsection{Argument semantic roles}\label{sec:valency.overview.semantic-roles}

The content of this chapter doesn't touch much in \acs{tam} adverbials or speech-act related adverbials.
...
\begin{infobox}{The S, A, P, G, T, ect. labels}{sap-labels}
    ...
    The typological literature largely uses the S, A, P 
    (or O, which is the notation of \citet{dixon2009basic1}) labels.
    These labels have dual roles:
    ...
    in determining the correct way of coarse-graining of semantic positions.
    ...
    they should never appears as different clausal complements in a canonical clause.
    ...
    The meaning of deep S, A, P labels is clear: 
...
\subsubsection{Accusativity}\label{sec:simple-clause.subject.accusative}
...
\subsubsection{The subject in clausal derivation}
...
\subsection{The passive \corpus{by}-phrase}\label{sec:valency.overview.by-phrase}
...
\section{Overview}\label{sec:clause-template}
...
\begin{infobox}{Focusing is not valency changing}{focus-not-voice}
    ...
    Some grammars analyze the fronted constituent as the subject. TODO: ref
\end{infobox}
