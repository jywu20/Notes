...
\begin{theorybox}{Cartography of DPs}{dp-structure}
    In Cartography, 
    the determiner function D_{\text{det}} is similar to the role of FinP in CP,
    ...
    so the various D-like functions -- corresponding to FinP and ForceP -- 
    all looks quite close.
\end{theorybox}
...
either by retrieving it from the conversational context,
or by some uniqueness conditions.
Therefore, definiteness implies identifiability.

The first origin of identifiability gives rise to 
the famous \corpus{a}-\corpus{the} alternation in discourses:
When a person or an object first appears,
\corpus{a} is used,
and then the entity is referred to with \corpus{the}.
The second origin of identifiability can be seen in examples like 
\corpus{the father of Tom}:
We don't say \corpus{a father of Tom},
...
Note that the first type of identifiability is a weakened version of the second:
...
It can be seen that there are several degrees of strength of identifiability.
...
will be discussed in \prettyref{sec:np.det}.
...
\begin{theorybox}{The weakness of the naive theory of description}{theory-of-description}
    ...
    Russelle's theory of description interprets \corpus{the} 
...
In pure semantics, quantification and definiteness 
(to be understood as Russell's $\iota$ (\prettyref{box:theory-of-description})) expel each other.
...
\section{Determiners and determinatives}\label{sec:np.det}
