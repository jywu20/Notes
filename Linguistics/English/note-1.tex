...
about how ``close'' they are to the lexical head 
(i.e. the root; this is also the term used in \citet{kramer2010amharic}).
...
Some constituents -- like the negative adverb \form{not} or \form{rarely} -- 
...
For example, instead of \form{*go-ed}, we have \form{went}.
...
The verb nominalization process involves the suffix \form{-tion} or sometimes \form{-ing}, etc.
The third aspect is about established structures and their meanings:
The structure \form{[fire-man]_{\text{compound}}} 
is a compounding structure,
with \form{man} being the lexical head and \form{fire} being the modifier,
...
Thus, \form{in spite of} deserves a dictionary entry;
we know it's better to analyze it as a template of 
a hierarchical structure \form{in [spite [of $x$]]}
...
like \form{went}, is also not a constituent.)
...
(as in, say, the archaic \form{till death do us part} that is still used today) -- 
...
\begin{enumerate*}
    ...
    as in \form{pre- and post-processing},
    while on the other hand,
    some orthographical multiple word constructions 
    are also not active in syntax,
    like \form{apple tree} or \form{American history teacher},
    ...
    \form{American history teachers} should be syntactically partitioned as 
    \form{[American history teacher]-s},
...
(as in \form{American history teacher}, 
...
like, say, the derivation of \form{near} from prototypical prepositions.
...
\form{[Some students]_{i} are rather frustrated by the new policy. 
[They]_i are protesting outside for their rights.},
we have two choices.
We may consider the two sentences as \emph{one} sentence
and regard the anaphoric \form{they} as a partial spellout 
...
and interpret this as the reference of the pronoun (\prettyref{sec:np.template}).

We may also conceive this as a default $\exists$-elimination:
When I hear \form{some students are rather frustrated by the new policy},
...
Consider the sentence: \form{John didn't drive that night}. 
...
Definitely not. 
The word \form{drive}, despite being called a predicate (or \term{predicator}) in syntax,
...
like that \form{John} is somehow higher than the verb \form{drive} in the clause,
...
\begin{itemize}
    ...
    Since \form{I don't think this is the case} 
    refers to a situation that isn't far from \form{I think this is not the case},
    we say the scope of \form{not} is expanded,
    ...
    the syntactic structure between \form{bechelor} and \form{single man} 
...
The future time is marked by the auxiliary \form{will} or \form{would} 
or the \form{be going to do} construction (\prettyref{sec:future}).

Besides the regular \acs{tam} system,
there are also some periphrastic constructions marking specific \acs{tam} configurations,
like \form{used to} or \form{would rather} (\prettyref{sec:semi-auxiliary}).
...
\begin{theorybox}{Alignment and the subject}{alignment-subject}
    ...
    as is observed in the Spanish \form{se lo} construction
...
\begin{theorybox}{Clause and sentence}{clause-sentence}
    ...
    like the indirect quoted question in \form{I asked [why he was always late]},
...
\begin{theorybox}{Valency changing}{valency-changing}
    ...
    The clause \form{John and Mary will meet tomorrow} 
    has the same meaning of \form{John will meet with Mary tomorrow},
    but it's unlikely the relation between the two arises from some syntactic operations in the \vP{} layer:
    The case may just be that \form{meet} is compatible with two \vP{} structures,
    ...
    and becomes the \form{by}-phrase and can no longer be promoted to the subject position.
...
The first is the \form{be}-passive (\prettyref{ex:be-passive}), 
and the second is the lately grammaticalized \form{get}-passive.
...
\begin{exe}
    \ex\label{ex:be-passive} \begin{xlist}
        ...
        \ex {} [%
            ...
            [hit]_{\text{\formcat{ed}-participle}} %
...
(the \formcat{ing}-participle and the \formcat{ed}-participle),
the bare infinitive and the \formcat{to}-infinitive (\prettyref{sec:simple-clause.nonfinite-clause}).
...
\begin{exe}
    \ex \label{ex:overview.nonfinite-finite-adverbial-ex-b1} \begin{xlist}
        ...
        _{\text{temporal:\formcat{ing}-clause}}
...
in a similar position with \form{even} or \form{just}
...
are peripheral arguments (\prettyref{sec:peripheral-arguments}) -- 
...
When we see the single/plural \form{-\emptymorpheme}/\form{-s} alternation, it has to be a countable noun.
...
as in, say, dephrasal derivations like \form{his holier-than-thou attitude} \citep[\citepage{1646}]{cgel};.
Whether the \form{-ly} suffix for adjective-to-adverb derivation 
...
The compound noun \form{acceptability judgement},
for example, is better analyzed as 
\form{[acceptability judge]-ment},
...
\subsubsection{The \form{-ly} derivation}
...
\begin{infobox}{Form and function in \acs{np} constituent order typology}{np-order}
    The labels in \prettyref{fig:np-template} are about function and not form;
    in typology, we use the symbols Dem, Num, A, and N to represent
    ...
    doesn't mean the attributives are all adjectives, etc.
\end{infobox}
...
\begin{figure}[H]
    ...
    each grey arrow represents interpretation of one \acs{tam} category}
...
a habitual adverb \form{usually}, 
the present perfect tense i.e. the \form{have done} construction,
and the progressive aspect i.e. the \form{be doing} construction.
...
\subsubsection{\form{Do} insertion}\label{sec:verb-inflection.do}

\paragraph{Obligatory \form{do} insertion}
...
\paragraph{\form{Do} for emphasis} Unlike
...
\begin{itemize}
    ...
    (in, say, a road sign reading \form{you are now leaving New York}).
    In either cases, 
...
or the \form{be going to do} construction (TODO: ref),
and a normal future event is expressed using the auxiliary \form{will}.
...
TODO: \form{were I you} and \form{I wish he realized \dots} \citep[\citepage{153}, {[15]}]{cgel}
...
it's better to treat them as peripheral arguments (\prettyref{sec:peripheral-arguments}),
...
\chapter{The verb phrase}\label{chap:valency}

This chapter mainly discusses the grammatical relations 
between the main verb and clausal dependents lower than the subject.
Information packaging devices may change the position of these clausal dependents 
as well as the position of the main verb (TODO: ref)
...
Building a clause with correct verb valency concerns at least three steps.
...
Each step describes a distinct set of grammatical relations.
...
either the purely semantic ones or ones bearing syntactic significance 
(for example, A might be a shorthand for the causer-like group -- see below),
and sometimes are coarse-grained clausal positions
(for example, O might be a shorthand for anything that looks like the prototypical object).
The confusion in usage doesn't create much chaos
...
Indeed, the common practice in grammar writing is to conflating the three steps,
...
There are, however, always some cases 
in which conflating the three layers obscures what's really going on.
In existential constructions (TODO: ref), i.e. \form{there be} clauses,  
the subject is the dummy \form{there}.
Now what's the A argument?
In this case we have to tell step 2 from step 3.
In a typological sketch of the language, still,
...
Alignment typology needs to separate step 2 from step 3.
In both accusative languages and ergative languages, 
we know in monotransitive and intransitive clauses,
there are only two complement types (in step 3),
but in step 2, we have S, A, and P. 
The mapping between the two steps is studied in alignment typology:
For example, merging S and A in step 2 into the subject in step 3 shows accusativity.
Sometimes people say that S and A are \term{marked in the same way} 
which means S and A are the same with respect to clausal position,
and the difference between A and S is a matter in the second step,
i.e. the syntactic semantic role step.

Another case in which conflating the three steps creates chaos
is valency changing.
Here we use surface and deep S, A, and O positions to classify valence changing devices.
The meaning of deep S, A, P labels is
the coarse-grained semantic roles.
Again, we see coherence between purely semantic argument roles and 
...
The above examples show that conflating step 2 and step 3 
doesn't make sense in some cases.
Still, conflating step 1 and step 2 is also not a good idea
in, say, description of ditransitive verbs, 
where the semantic theme and the semantic receiver 
may be grammaticalized in two different ways (\prettyref{sec:valence.ditransitive}).
So all the three steps need to be recognized in a fine-grained grammatical description.

Finally, when necessary, 
we need to split the S, A, O labels.
In English, both \form{the door} in \form{the door opens}
and \form{the cat} in \form{the cat kills} 
are given the S label,
despite evidence from the two labile verbs suggests that 
\form{the door} is in a more patientive position (\form{I closed [the door]}), 
while \form{the cat} is in a more agentive position (\form{[the cat] killed a bird}),
because both \form{the cat} and \form{the door} are in the typical subject position 
in the finished intransitive clauses.
For split-S languages, however, there is no uniform S role,
and Sa (like \form{the cat} mentioned above)
and So (like \form{the door})
are distinguished. 
...
\begin{theorybox}{Conflating the three steps}{a-position}
    The three-step model is briefly mentioned in \citep[\citepage{12}]{devine2006latin}.
    ...
    The term \term{$\theta$-role} as the label for roles in step 2
    and the term \term{participant role} as the label for roles in step 1 avoid confusion 
    ...
    Conflating argument labels in step 2 and step 3
    gives us A-positions. 
    A-movements -- movements to A-positions -- usually happen before 
    A'-movements -- movements like topicalization.
    A large portion of this chapter therefore is about A-positions.
    When ``generalized semantic roles'' -- S, A, P labels -- are used syntactically, 
    they represent A-positions.
    ...
    Conflating step 1 and step 2 also makes sense in most cases, 
    because of UATH: 
\end{theorybox}


\subsection{Step 1: purely semantic classification of verbs}
...
\subsection{The second step: semantic roles in the eye of syntax}\label{sec:valency.overview.semantic-roles}
...
as in \form{the knife cut the lace}.
...
For the verb \form{hear}, for example,
the canonical object is a stimulus rather than a patient,
but we have passive constructions like \form{my opinions are never heard},
in which the stimulus \form{my opinion} is just like 
...
in \form{she ran home}, \form{she} is the theme, and \form{home} the goal.
Another property, metaphorically related to the location, is (change of) possession.
Thus \form{the key} in \form{he gave me the key}, and \form{the key} in \form{the key is mine}
...
In this way, 
\form{she} in \form{she went mad}, 
\form{her} in \form{it made her angry},
\form{Kim's} and \form{Pat's} in \form{Kim's writing resembles Pat's}
are all themes.
...
The \concept{recipient} is a role related to verbs like \form{give}.
The \concept{beneficiary} is a role that something is done for,
as in \form{I've bought [you] a present}.
...
\subsection{The third step: clausal dependents that are analyzable at the surface}\label{sec:valency.overview.dependents}
...
\subsubsection{The passive \form{by}-phrase}\label{sec:valency.overview.by-phrase}
...
for example the \form{be said to} construction
\citep[\citepage{245}]{dixon2005semantic}.

TODO: \citep[\citepages{249, 1433}]{cgel}

TODO: is \form{the dog bites} antipassive?

TODO: What about Dixon's promotion to subject, like \form{the CD sells well}?
...
\subsubsection{The \category{affect} verbs}
...
\subsection{Prototypical ditransitive verbs}\label{sec:valence.ditransitive}
...
In English there are two major verb frames 
for \category{giving} verbs:
...
A binary-branching analysis of the event structure and the (deep) argument structure of 
(\prettyref{ex:valence.give.1}) would be the follows:
\begin{enumerate}
    \item The action is \translate{to give}.
    \item The result of the action is \translate{the film star possesses a new nose}.
    \item The \acs{np} \form{the film star} is then promoted to the patient position 
        because it's the film star that undergoes the plastic surgery.
    \item The action is initiated by the agent -- the plastic surgeon.
\end{enumerate}
The most trivial linear realization of this structure gives precisely the constituent order in 
(\ref{ex:valence.give.1}).
A similar analysis -- which also directly predicts the constituent order -- 
of (\ref{ex:valence.give.2}) is the follows: 
\begin{enumerate}
    \item The action is \translate{to give}.
    \item The result of the action is \translate{a new nose comes to the film star};
        this result complement may be seen as a \acs{pp}
        with a subject \form{a new nose}.
    \item The \acs{np} \form{the new nose} is then promoted to the patient position 
        because it's the new nose that undergoes the transferring process.
    \item The action is initiated by the agent -- the plastic surgeon.
\end{enumerate}
The fact that in (\ref{ex:valence.give.2}),
we have an embedded ``the new nose to the film star'' structure 
leaves the listener an impression 
that the new nose is somehow an independent object, 
not necessarily possessed by someone, 
so the meaning of the sentence is like 
\translate{the plastic surgeon has a pocket of new noses 
and just gave the film star a new nose 
...
\subsection{Verbs with prepositions and particles}
...
\subsection{Small clause constructions}
...
\section{Peripheral arguments}\label{sec:peripheral-arguments}

Peripheral arguments, or simply 

\section{Negation} 

Negation in English clauses is usually realized by the negator \form{not}
or negative forms of auxiliaries (\prettyref{sec:verb-inflection.negation}). 
Negative pronouns like \form{nobody} can also be used to express a negative idea (TODO: ref).
...
\begin{theorybox}{Negation in Cartography}{negation-position}
    ...
    (and also CP -- think about the Latin negative complementizer \form{ne})
    ...
    \paragraph*{Implication to the structure of syntax} 
    The aforementioned second approach 
    may give rise to the theoretical problem 
    ...
    So we can safely assume that the scope changing of negation 
    is due to movement of SpecNegP.
    ...
    \paragraph*{Structure-meaning mismatch due to pragmatics}
    ...
    For example, \form{I don't think he will come} 
    ...
    But it's highly likely that the interpretation just comes from a quick logical inference:
    if I don't think he will come to tonight's activity, 
    then the \emph{only} reason seems to be 
    I already know he is likely to be absent to the party. 
    The construction \form{I don't think \dots} 
    may then be semi-fossilized as an idiomatic way to say 
    \translate{I think \dots is not the case}, 
    but its original semantics is clearly analyzable 
    without stipulating any syntactic operation.
\end{theorybox}
...
but it may also be about manner or frustrative expressions like \form{in a stupid way}:
...
It is similar to \prettyref{sec:valency.overview.steps},
