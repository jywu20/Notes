...
A more established word is \form{winterized}.
...
We can provide one more piece of evidence: the word \form{whorification}.
...
Another piece of evidence for a dephrasal analysis of proper names 
is occasionally observed fossilized grammatical structures that typically should not appear in a nominal.
For instance, we have \form{The Hague} as a calque of the original Dutch name.
The reference of the phrase however is not determined by the presence of \form{the}:
proper names without \form{the} have definite, unique references too.
What happens here is more likely that the full noun phrase \form{the Hague} has 
undergone dephrasalization.
...
Also consider the form \form{Orange Countians} \translate{residents of Orange County},
which clearly has the structure \form{[Orange County]-ian},
or the more common \form{New Yorkers} or \form{East Coasters}.
Also consider less formal \form{do-nothingists} or \form{Old Calenderists}.
These forms seem to further suggest that proper names are dephrasal compounds now.
It is also an illustration of the fact that there is no strict boundary between
word structures and phrasal structures.
...
Another instance of the \category{location}-\category{axial part} structure is 
that for certain speakers, the form \form{in under 30 minutes} is acceptable,
as in \href{https://www.youtube.com/watch?v=ry5Du60WPGU}{this video, 7:04}.
...
containing all the core arguments (\prettyref{sec:valency.overview})
...
(\prettyref{sec:simple-clause.voice}).
...
-- can be classified as in \prettyref{tbl:argument-classification-general}.
...
\begin{table}[H]
    ...
    \begin{itemize}
        ...
        may be complicated (\prettyref{sec:np.pp}),
...
\prettyref{tbl:argument-classification-general} is purely syntactic;
...
according to \prettyref{tbl:argument-classification-general}.
...
from peripheral semantic roles to syntactic positions in \prettyref{tbl:clausal-dependent}.
...
we get constructions in \prettyref{sec:clause.ditransitive-particle},
...
(\prettyref{sec:vp.valence.verb-particle}). 
...
and whether the particle can be altered.
