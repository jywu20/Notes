...
\chapter{Overview of clause structure}\label{chap:simple-clause}
...
no adverbials can be inserted between the main verb and, say, an object.
After these prototypical internal arguments, 
...
This auxiliary-main verb-argument-adverbial constituent in \prettyref{fig:clause-template} 
...
\begin{itemize}
    ...
    However, we can't find any \emph{structural} difference 
...
\begin{itemize}
    ...
    everything else that may appear in the post-verbal region,  
...
we have a three-fold classification
...
\footnote{
    Note that the \acs{np} after a preposition can be freely topicalized etc. 
    and leaves a stranded preposition behind; 
    this is much more widely available than prepositional passivization:
    \form{the thing I came across this morning\dots} 
    Here we are talking about what happens when a constituent is moved 
    from one position that's related with a case to another position that's related with a case 
    (from accusative to nominative, in passivization), 
    not what happens after the argument structure is finally finished.
    TODO: This is mentioned on page 14 of The Processing of Preposition-Stranding Constructions in English; 
    one viable explanation is to stipulate that only the accusative case can be stacked with the nominative case \citep{richards2017multiple}, while an inherent case usually can't.
}
...
the other is an oblique argument construction (see below).
...
\subsubsection{Pied-piping or not: mobility of preposition in \category{wh}-movements, etc.}

We do note that whether the preposition is somehow defunct is not relevant to 
whether the prepositional phrase can be \category{wh}-fronted and topicalized.
TODO: this explains the passivization parameter, but what about the position of the preposition, 
like why it can't be fronted sometimes?
The truly weird case is when the preposition can't be fronted but passivization also can't happen: 
the latter means the \acs{np} is an oblique argument, 
while the former means the preposition is somehow defunct? 
So there are two ways for the preposition to be defunct: 
one is losing the ability to assign case, 
the second is being fossilized to the verb. 
...
a phrasal verb,
keeping the name for verbal idioms in \citet{cgel}.
