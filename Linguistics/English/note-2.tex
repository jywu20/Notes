...
A grammatical word is just a mini-constituent,
...
Even when we constrain ourselves to content words,
the standard of the grammatical word is still blurred.
The first reason involves the syntactic wordhood: 
...
The second reason is realizational: 
whether a function item is firmly attached to the root it modifies is sometimes hard to say 
...
Here is a list of parts of speech recognizable in English:
\begin{itemize}
    \item Open content categories with clear part of speech labels: noun, verb or adjective
    (\prettyref{sec:pos.open}).
    \item Unlike Japanese, no close content categories with clear part of speech labels 
    ...
    is sometimes hard to make (\prettyref{sec:theory.pos}). %Enumerating: all blurred case
    \item Close grammatical categories like particles or affixes that don't really need part of speech labels.
    Examples of this kind include prepositions (\prettyref{sec:pos.prep}) TODO: list
\end{itemize}
...
\section{Open content categories}\label{sec:pos.open}
...
\subsection{Derivation-inflection distinction in English}
...
Roots don't necessarily have categories,
but of course, most of them have preferred part of speech,
...
a finite clause is able to be a sentence i.e. a clause that is an utterance on its own,
while a nonfinite clause is usually embedded,
and when it does appear as an utterance,
it behaves more like an \acs{np} that's used as an utterance.
By this standard, 
English finite clauses may be in the imperative mood and non-imperative moods.
...
The difference between non-imperative finite moods is small.
