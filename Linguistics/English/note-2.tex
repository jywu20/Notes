...
\begin{itemize}
    \item The generative approach, 
        ...
        of a functional hierarchy lies 
        a noun or verb root; 
...
is the complement-adjunct distinction.
...
the nominal, and the noun phrase. 
...
\begin{exe}
    \ex management of products, product management 
    \ex Quality controlling 
\end{exe}

\subsubsection{Internal complexity}\label{sec:np.nominal.nominal-attributive.complexity}

A nominal attributive indeed is able to contain another nominal attributive 
(\ref{ex:np.nominal-attributive.1},
\ref{ex:np.nominal-attributive.2},
\ref{ex:np.nominal-attributive.3}),
or adjectives (\ref{ex:np.nominal-attributive.4}, \ref{ex:np.nominal-attributive.5}),
and in principle the nominal attributive 
can be arbitrarily complex (\ref{ex:np.nominal-attributive.5}),
...
The status of \form{Ministry of Defense} and \form{Red River} as nominals 
may be questioned (\prettyref{sec:np.nominal.nominal-attributive.noun-compound-compare}). 
...
\begin{exe}
    ...
    \ex\label{ex:np.nominal-attributive.4} Just remember [the [[Red]_{\text{adjective attributive}} River]_{\text{nominal attributive}} Valley],
    and the cowboy that has loved you so true.

    \ex\label{ex:np.nominal-attributive.5} 
    The [[[theoretical]_{\text{adjective}} [condensed matter]_{\text{nominal attributive}}]_{\text{nominal attributive}} physics]_{\text{nominal attributive}} community 
...
It should be noted that the nominal attributive construction 
...
\begin{itemize}
    \item The most salient feature is 
    noun compounding can never be coordinated: 
    coordination is possible for prefixes, 
    but never a base in nominal compounding
    (\prettyref{sec:np.nominal.coordination}).
    \item The bases involved in noun compounding 
...
It should be noted that semantic criteria 
can't be invoked as decisive evidences 
to tell noun compounding from nominal modification:
a compound noun can be semantically compositional,
while a nominal attributive construction 
frequently has established meanings
\citep[\citepage{450}]{cgel}.
 
Some forms fall on the boundary between 
compound nouns and complex nominals.
The constituents of \form{Ministry of Defense} (\ref{ex:np.nominal-attributive.1})
and \form{Red River} (\ref{ex:np.nominal-attributive.4})
are almost never modified or coordinated, 
indicating the possibility for them to be arising from the noun compounding construction,
as opposed to the nominal attributive construction.
But the syntactic inactive status of the forms 
may also be explained by semantic reasons, 
...
Coordination can be applied on the level of the noun, 
the nominal, and the noun phrase. 
The first is much less frequent but is still possible
