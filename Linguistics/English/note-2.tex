...
There seems to be no disagreements between 
phonological wordhood and morphological or ``realizational'' wordhood 
...
others, however, can take modifiers (\prettyref{ex:np.pp.ex-1}),
...
the bare nominal
can still have limited distribution as a pre-head \acs{np} dependent,
...
under which we have complements and modifiers 
(modifiers in the \acs{np} are known as attributives), 
which may appear before and after the head noun. 
There exists a relatively fixed order of the constituents (\prettyref{sec:np.nominal.order}). 
Also, the cardinal numeral, modifiers and post-head dependents seem to be only licensed 
...
thus, in the nominal region, 
a core nominal subregion can be recognized, 
which only contains the complements.
...
sometimes can be confused with the nominal attributive construction. 
...
the nominal appears below the determiner and 
contains the numeral, the modifier(s) and the complement(s).%
\footnote{
...
\begin{exe}
    \ex the three exotic chess boarders 
...
TODO: some ``nominal attributives'' are complements
...
\subsubsection{Internal complexity}\label{sec:np.nominal.nominal-attributive.complexity}

A nominal attributive is able to contain another nominal attributive 
(\ref{ex:np.nominal-attributive.2},
\ref{ex:np.nominal-attributive.3}),
or adjectives (\ref{ex:np.nominal-attributive.8}).
Several layers of nominal attributive constructions are possible 
(\ref{ex:np.nominal-attributive.5}),
although nominal attributives within a nominal attributive 
are much more common than 
adjectives in a nominal attributive,
the former in turn is less frequent than 
one-word nominal attributives in everyday speech.
Coordination of nominal attributives is also possible
(\prettyref{sec:np.nominal.coordination}, \ref{ex:np.nominal.coordination.6}). 

On the other hand, 
it seems when the nominal attributive contains 
...
post-main noun dependents in the nominal attributive
...
\begin{exe}
    \ex\label{ex:np.nominal-attributive.2} We planted [an [apple]_{\text{nominal attributive}} tree]_{\text{\acs{np}}} yesterday.

    \ex\label{ex:np.nominal-attributive.3} {} [The [[Fuji apple]_{\text{nominal attributive}} tree]_{\text{nominal attributive}} variety]_{\text{\acs{np}}} has a reddish-green color.

    \ex\label{ex:np.nominal-attributive.8} We are facing a [medical doctor]_{\text{nominal attributive}} shortage.  

    \ex\label{ex:np.nominal-attributive.5} 
    The [[[[theoretical]_{\text{adjective}} [condensed matter]_{\text{nominal attributive}}]_{\text{nominal attributive}} physics]_{\text{nominal attributive}} community]_{\text{NP}} 
    doesn't seem to be willing to write textbooks. 

    \ex\label{ex:np.nominal-attributive.6} *a [[good apple]_{\text{nominal attributive}} tree]_{\text{nominal}}
...
at least in nominal attributive constructions.

\begin{exe}
    \ex\label{ex:np.nominal-attributive.9} This state-of-the-art model 

    \ex\label{ex:np.nominal-attributive.1} {} [The [Ministry of Defense]_{\text{nominal attributive}} officials]_{\text{\acs{np}}} are having a secret meeting in that room.

    \ex\label{ex:np.nominal-attributive.4} Just remember 
    [the [[Red]_{\text{adjective attributive}} River]_{\text{nominal attributive}} Valley]_{\text{NP}},
...
The nominal attributive construction 
is not the same as noun compounding,
although the former frequently has an established meaning
and semantically may be regarded ``as a word'': 
a compound noun can also be semantically compositional, 
and established meanings are quite frequent for nominal attributive constructions as well
...
\begin{itemize}
    ...
    while this is possible for nominal attributives 
    (\prettyref{sec:np.nominal.nominal-attributive.complexity}). 
    ...
    \item The first base in noun compounding 
        ...
        but in a nominal attributive construction, 
        the nominal attributive is also very close to the head noun, 
        and therefore sometimes this test can't be run.

    \item Coordination is much less frequent in 
    the noun compounding than in the nominal attributive construction 
...
\begin{itemize}
    ...
    \item Often, the first element in a compound noun receives the stress.  
        ...
        and most nominal attributive constructions do have the primary stress 
...
at least in the nominal attributive construction
(\prettyref{sec:np.nominal.nominal-attributive.complexity}, 
\ref{ex:np.nominal-attributive.1}, \ref{ex:np.nominal-attributive.4}). 
...
which are summarized as \prettyref{tbl:clausal-dependent}.
...
The first layer in \prettyref{fig:clause-template} 
...
which are discussed in detail in \prettyref{sec:valency.overview},
...
colored as green in \prettyref{tbl:clausal-dependent}.
