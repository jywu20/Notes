...
结论是，提出macroparameter的本意是寻找一组不多的parameter，它们的排列组合可以形成数目较多但是仍然有限的一组语法，这组语法可以完全解释各种语言现象，但是仔细一看其实并非如此。
...
- wh-movement相关的macroparameter：有关的讨论不太好找，不过简单地想一想，wh-movement能否发生可以化归到island effect，phase theory这些问题上，以及一些realizational effect上面（例如可能某些关系从句结构的C只有周围没有宾格标记时才能正常语音实现等）
