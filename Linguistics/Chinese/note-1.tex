...
The English form \corpus{explore-ed} is recognized as a word and therefore a constituent, 
while \corpus{have explored} is sometimes recognized 
as a phrase and therefore a constituent
and sometimes not (\prettyref{sec:theory}).
The structures of the two are basically parallel: 
they both need to take an object, 
...
If \corpus{have explored} is recognized as a constituent, 
it makes sense 
because the items inside have strong interlink in dependency relations; 
but of course we can stick to the analysis in \citet{cgel}
and insist that it's better to include the object into the verb phrase.
In principle, \corpus{explore-ed} faces the same problem,
but we all agree it's a word in everyday use, 
because it's a phonological word
(\prettyref{sec:pos.word.phonological}).
Of course, there is no strict boundary between the two cases, 
