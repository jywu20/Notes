...
with verb valence, 
and therefore it's a good idea 
to describe the \acs{vp} 
...
\section{Verb frames and lexical aspect}

There is a strong correlation between the argument structure 
and the lexical aspect 
in Mandarin \acs{vp}s.
\acs{vp}s (not verbs: a verb may project into several different \acs{vp}s) 
involving zero or one objects 
(not including so-called semi-objects in \prettyref{TODO: semi-object, and terminology})
may be divided into the following classes 
\citep[\citechap{6}]{deng2010formal}: 
\begin{itemize}
    \item The \category{be} type, describing a state, with one argument.
    \item The \category{do} type, describing an atelic event, with one agentive argument 
    ...
    \item The \category{become} type, describing a telic event, 
    ...
    \item The \category{cause-become} type, a telic event, 
...
\begin{infobox}{Unaccusativity}{unaccusative}
    ...
    In this note, I will avoid using these terms.
\end{infobox}
...
Anyway there are significance differences between the short bei-construction and the long bei-construction.
...
\chapter{Valency and valency changing}
