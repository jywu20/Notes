...
\begin{infobox}{Are phrasal prosodic words?}{feng-wording-word}
    ...
    (\prettyref{box:internal-object-xbar}),
...
\form{[-ed [explore \emph{object}]_{\text{argument structure}}]_{\text{\acs{tame} marking}}};
...
念佛 as a word is something like \form{Buddha-praying}
or the lexicalized \form{pray-to-Buddha},
while 念佛 as a verb is something like \form{pray to Buddha}.
...
The relation between the subject and the verb phrase 
is partly captured by the argument structure, 
...
and therefore some discussions about the subject 
are delayed to \prettyref{chap:simple-clause}.
...
The verb valency can be further split into two systems:  
the system of verbal complements and ``internal objects'', 
and the system that eventually gives rise to more ``traditional'' subjects and objects.
The structure of the core \acs{vp} -- as well as what is the subject -- depends
strongly on the argument structure.
...
the potential complement (\prettyref{sec:verbal-complement}),
...
between (\prettyref{ex:verb-phrase.overview.1}) 
and (\prettyref{ex:verb-phrase.overview.2}):
...
for the \category{do}-\category{affect} structure,
...
is at best problematic (\prettyref{ex:verb-phrase.affectee.1-ba}).

\begin{exe}
    \ex\label{ex:verb-phrase.affect.1} 
    \gll [他]_{\text{subject, agent}} 抢 了 [我]_{\text{\category{affectee}}} [十 块 钱]_{\text{patient}} \\ 
...
A \category{become} verb may or may not appear with the causer argument.
...
These facts mean the \category{experience}-\category{become} structure 
...
\subsubsection{The \category{cause}-\category{experience} structure}\label{sec:verb-phrase.dative}
...
It's only seen in the 
The position of the dative indirect object 
is higher than internal objects and verbal complements, 
...
it's impossible to recast (\prettyref{ex:verb-phrase.cause.experience.1})
into a \form{bǎ}-construction
by pretending that the receiver argument is in the \category{become} structure
(\prettyref{ex:verb-phrase.cause.experience.1-ba}).
The theme argument is able to appear after 把 
in the \form{bǎ}-construction
(\prettyref{ex:verb-phrase.cause.experience.1-ba-correct}).
This however is likely linked to 
(\prettyref{ex:verb-phrase.cause.experience.1-become},
\prettyref{ex:verb-phrase.cause.experience.1-cause-become}),
...
\begin{exe}
    ...
    \gll [这 本 书]_{\text{subject,\category{become}}} [送 给 了]_{\text{verbal complex}} [我]_{\text{internal}} \\
    \category{dem} \category{cl} book give.gift give.toward \category{asp} 1 \\ 
...
in \prettyref{sec:verb-phrase.bei.passive-alternation}
or \prettyref{sec:verb-phrase.experience}.
...
The generalization that only the \category{cause}-\category{become} structure 
appears in the \category{bǎ}-construction
can be reached by observing 
verbs that can't appear in the \form{bǎ}-construction:
all of them face structural or semantic obstruction 
...
\prettyref{sec:verb-phrase.cause-become.ordinary}).
This generalization can also be justified by the fact that 
...
One interesting observation is the long \form{bèi}-construction
...
\subsection{\category{giving} verb with resultative complement}
