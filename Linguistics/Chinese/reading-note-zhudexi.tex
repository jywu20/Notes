...
Several definitions of grammatical words are discussed (\prettyref{sec:grammatical-word}),
and the puzzle of split verbs (\prettyref{sec:splitting})
...
Finally, morphological devices (\prettyref{sec:morphological-device})
are introduced.
...
It is also possible that a prosodic word has morphosyntactic significance, 
i.e. it is a morphosyntactic constituent.
A large portion of prosodic words,
...
and as is said in \prettyref{sec:blur-line-word-phrase},
they can be interpreted as grammatical words and phrases in different syntactic contexts
-- this even works for those without analyzable internal structures (\prettyref{sec:splitting}).
These prosodic words are therefore also building blocks of the language,
...
we are left in the situation described in \prettyref{sec:blur-line-word-phrase},
...
word splitting is sometimes also possible (\prettyref{sec:splitting}).
Thus the morphosyntactic prosodic word is an intermediate level 
both between morphemes and words and between words and phrases.
...
\subsubsection{Relation between grammatical words and phrases}

Not all grammatical words constitute phrases themselves.
Monosyllabic location words like 前 are definitely words 
because they can be used with an arbitrary \ac{np} 
...
like those discussed at the end of \prettyref{sec:blur-line-word-phrase},
are unable to constitute phrases -- regardless of their types -- by themselves.
Examples are given here:
\begin{exe}
    \ex Transitive disyllabic verbs as one-word phrases
    \begin{xlist}
...
Like non-prosodic simple words,
...
It can be seen in Chinese the level of grammatical words 
is of relatively minor interest in the grammar:
most of grammatical words are themselves phrases,
major exceptions including only verb-result or verb-direction constructions like 看出 and 走上,
monosyllabic location words like 上 and 前,
and coverbs.
The latter two, however,
are clearly function words
...
On the other hand,
prosodic words have an established status in the grammar:
they are subject to richer surface-oriented syntactic devices 
compared with other grammatical words,
and they are the most frequently used bridges to build phrases from morphemes.
This is radically different from English:
...
while in Chinese, the inner structure of the grammatical word 
is highly complicated,
and the word-phrase boundary and the morpheme-word boundary are not always so clear.
...
Nouns (\prettyref{sec:reduplication-noun}), 
