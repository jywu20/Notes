...
between the binary branching definition of \emph{verb phrase} as Aux plus V plus NP_{\text{object}} 
...
What tears linguists apart is what they expect a head to do. % TODO
...
but it fails to see that what this is all about.
...
It can be expected to be rare, because what generative linguists do is actually 
...
(read: ``syntactic structures, be them constituency or dependency, 
...
but in the same section I will show this poses no conflict with generative syntax.
The motto of ``describing languages without prejudice'' therefore makes sense more rhetorically:
...
there is bias not because linguists support Eurocentrism, but because languages themselves have bias.
The problem now is the bias is in the \emph{implicit} knowledge 
passed in the typology word, 
orally or subconsciously by reading previous reference grammars,
...
is far from the layers that define the inner structure of the constituent (the \emph{form} of the constituent).
...
which is also an important classification criterion of the head (the famous \concept{subcategorization}),
...
% TODO: 因此还是需要按照功能区间区分不同的dependent
% TODO: 有无像modifier的functional word?
