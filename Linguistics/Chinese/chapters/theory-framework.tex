...
talk about this in \prettyref{sec:routine}. % TODO: BLT句法：先给出一个layer中的constituent，然后讨论它们彼此之间的依存关系；由不可见的较高位置的functional head导致的语法关系通过phrase label给出（如句子中的A, S, O, predicate这些标签），似乎有lexical realization的functional head导致的语法关系通过依存关系给出
...
All the descriptive theories work on more % TODO: 总结常见的表示构式的方式
...
are probably construction grammars, % TODO: 语序可以直接传递信息，等等
...
are the \emph{only} kind of structure-building primitives. % TODO: TAG
...
Let us start from \prettyref{fig:hurt-dm-routine}. It still contains many invisible functional heads and is not 
...
relations -- see \prettyref{sec:minimalist-dependency}) are supposed to have something to do with Merge, 
and only Merge (not Adjoin or something else), and hence the invisible functional head hierarchy.
To throw away functional heads while keeping their functions, what we will get is \prettyref{fig:hurt-constituent-tree}. In \prettyref{fig:hurt-constituent-tree}, (a) is the syntactic tree in which 
...
in \prettyref{sec:sub-cat}) and hints nothing about whether a constituent is a maximal projection or not. 
...
movements occur. Let us consider the Spec$v$P-to-SpecTP A-movement of the subject in \prettyref{fig:coarse-grained-hurt-the-dog-badly}. We see the notion of this movement is just reflection of the fact 
...
\prettyref{fig:coarse-grained-hurt-the-dog-badly} contains ad adverb, traditionally considered 
...
case (or so-called \concept{$\phi$-features} in generative terms). Some say it is still kind of nominal, 
...
which are licensed by the verb, commonly called 补语 in Chinese School Grammars (\prettyref{sec:school-grammar}).
