...
The notion of \term{words} do not have a primitive status in our descriptive framework.
...
which usually is a (extended) verb phrase.
...
\subsubsection{Compounding, dephrasalization, affixation}

Frequently, the main verb itself has a synchronically analyzable internal structure.
In the simplest case, the argument structure of the main verb can't
be regularly predicted from the internal structure of the verb stem
(on why we can postulate a stem, see \prettyref{sec:verbal-complex.linear-order}).
In this case the verb stem looks not too different from a root without internal structures.
...
and therefore 关心 can only be analyzed as a dephrasalized verb phrase,
which is re-categorized into a verb with its own subcategorization frame.

\begin{exe}
    \ex\label{ex:grammatical.clause.core-vp.compounding.1} 张三的女朋友很关心他
\end{exe}

Other more or less cross-linguistically common derivational devices include verbalization suffixes 
(\prettyref{sec:verbal-complex.v-c-a}),
and also prototypical full fossilized compounding which gives rise to words like 驱逐,
which can't be analyzed in the synchronic syntax of Mandarin
(\prettyref{sec:verbal-complex.derivation.compound}).
...
\begin{exe}
    ...
    \begin{xlist}
        ...
        \ex\label{ex:grammatical.clause.core-vp.compounding.2.3} 一些人念佛的时候不知道自己在[念哪一尊佛]_{\text{VP2}}
    \end{xlist}
\end{exe}
...
The historical origin of 白大褂 likely dephrasalization of the regular adjective modification phrase [[白]_{\text{color adj}} [大褂]_{\text{NP}}]_{\text{NP}},
...
The dephrasal nouns 大褂 and 白大褂 in \prettyref{fig:grammatical.np.adjective.2}
...
The compound 计算, for instance, means both \translate{to compute} and \translate{computation
(both the abstract concept of computation and one round of computation)}.
We have no evidence supporting that any part of speech categorization has happened to 计算:
it's possible that this compound is stored without any part of speech tag attached to it.
...
\subsection{Incorporation of personal pronouns}
