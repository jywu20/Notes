...
\begin{theorybox}{Terminology: \term{dephrasal} and \term{compound}}{dephrasal-compound}
    ...
    %In the latter case, the structure should be comparable to English \form{grammaticality judgment},
    %in which we can argue that the nominalizer \form{-ment} has scope over the whole form
    %(\translate{the action or result of judging grammaticality}),
    %and hence the form can be represented, in the spirit of \prettyref{fig:grammatical.clause.core-vp.%verbal-complex.1.1},
    %as something like \form{[[judge [[[grammatic] -al]_{\text{A}} -ity]_{\text{N}}]-ment]_{\text{N}}},
    %in which \form{judge} at first appears as a root,
    %and then \form{-ment} falls it because of pure morphology.
    This subtle difference is related to whether we should label 关 in \prettyref{fig:grammatical.clause.core-vp.compounding.1} as a root or as a verb.
    Since \prettyref{fig:grammatical.clause.core-vp.compounding.1} is an analysis based on the surface form,
    we represent 关 as a verb for now.%, in the same way we say \form{grammaticality judgment} is a noun-noun compound.
    TODO: forms like \form{hard worker} makes this labeling strategy problematic 
    only when the category is shifted: in this case the 
    
    In Mandarin, being a compound in the sense above does mean one thing:
    the object cannot be referential.
    This is not surprising or otherwise semantic problems occur?
\end{theorybox}

The structure of some verbs reminds us of verb phrase-level coordination (\prettyref{sec:grammatical.clause.subject.clause}).
...
\prettyref{fig:grammatical.clause.core-vp.compounding.1} is not truly unusual;
...
In a glued-up verb-object form,
the object being a bare noun, without any modification or complementation:
see e.g. 念佛 in (\ref{ex:grammatical.clause.core-vp.compounding.2.3}).

\begin{exe}
    \ex\label{ex:grammatical.clause.core-vp.compounding.2.3} 一些人[念佛]_{\text{glued-up}}的时候不知道自己在[念哪一尊佛]_{\text{VP}}
\end{exe}

At the first glance, there is nothing special with the object being a bare noun,
as if 念哪一尊佛 in (\ref{ex:grammatical.clause.core-vp.compounding.2.3}) is acceptable,
then so is 念佛, in which the \category{wh}-pronoun, the numeral and the classifier
are all removed from the object,
which is permitted by the syntax of the \ac{np} in Mandarin.
There are however properties of glued-up verb-object forms that are not shared by
...
念佛 (a glued-up verb-object structure) can be used as a nominal in \prettyref{sec:grammatical.np.complementation}
while 念阿弥陀佛, which is a prototypical verb phrase, or in the terminology of \citet[\citepages{128-9}]{zhudexigrammar},
a \concept{compositional verb-object structure} (组合式述宾结构), can't 
(cf. \ref{ex:grammatical.np.complementation.2}).
...
This grammar tentatively assumes that what is commonly known as a glued-up verb-object form
has two coexisting analyses:
...
念佛 in the first analysis can appear either as a noun (and thus we have \ref{ex:grammatical.clause.core-vp.compounding.2.1}) or as an intransitive verb.
The second analysis is the only possibility when topicalization etc. happens,
as in (\ref{ex:grammatical.clause.subject.no-topic}),
where 吃饭 clearly is a prototypical verb phrase or otherwise the frontings are not possible.

But do we really need to assume the existence of a dephrasal process?
In \prettyref{sec:grammatical.np.complementation}, for instance,
the nominals, despite looking like compounds, are not truly compounds in the proper sense
as they are formed by regular argument structures.
In this way we can assume that 念佛 and so are 

A question is how the dephrasal compound version of a glued-up verb-object structure
compares with prototypical dephrasal compounds like 
关心 in \prettyref{fig:grammatical.clause.core-vp.compounding.1}.
We note that a glued-up verb-object form regularly takes no other object.
This can be seen by contrasting (\ref{ex:grammatical.clause.core-vp.compounding.2.3})
with (\ref{ex:grammatical.clause.core-vp.compounding.2.4}):
once 念 and 佛 are placed together, it is no longer possible for a full fledged \ac{np}.


\begin{exe}
    \ex\label{ex:grammatical.clause.core-vp.compounding.2.4} *念佛哪一尊佛
\end{exe}

A glued-up verb-object form often -- but not necessarily -- is disyllabic,
...
\begin{exe}
    \ex\label{ex:grammatical.np.complementation.numerical-calculation-machine} *数值计算的机
\end{exe}

Furthermore, the complexity of the ``argument'' seems to be severely constrained.
This cannot be explained by lexicalization,
