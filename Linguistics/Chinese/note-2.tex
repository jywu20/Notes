...
\newtcbtheorem[number within=chapter, use counter from=infobox]{theorybox}{Box}{
    ...
    breakable,
    before upper={\parindent15pt\noindent}}{box}
\definecolor{my-green}{HTML}{5FA219}
...
\begin{itemize}
    ...
    are invoked to explain their differences in their syntactic behaviors.
    ...
    Grammatical properties explainable by the vP-TP-CP and the nP-NumP-DP hierarchies in Cartography 
    are reinterpreted as specifier/complement distinctions etc. in X-bar scheme.
    We can push this further and from X-bar schemes with different types of heads,
    we can further derive 
    subject-predicate structures, predicator-object structures, coordination structures, and so on,
    which is exactly what is done in \citet{deng2010formal},
    and results in a descriptive formalism quite similar to that in \citet{cgel}:
    ...
    to refer to full \emph{morphological words} that have the n or v categorizers
    (\prettyref{sec:intro.theory.word}),
    or subparts of these morphological words  
    whose form are lexicalized (\prettyref{sec:intro.theory.lexicon}) and may be known as the \concept{lexeme} in descriptive linguistics
    (but this term is not always clearly defined: see \prettyref{sec:intro.theory.lexicon} and \prettyref{box:lexeme-meaning} for possible confusions).
...
because the tense T node is above the predicator-object vP,
and since T is considered as a part of the verb,
then why shouldn't the object be considered as a part of the verb?),
...
In this way in the main text we will argue that 大褂 in 白大褂 is a word,
because the violation of the adjective order means 大褂 can only be a nP.
A nP is not necessarily a word (as we do not rely on syntactic smallness)
but the fact that 大褂 has no difficulty being a nP (just like a noun root) is lexically specified,
and this behavior means the form has to be a word.
...
(i.e. a person who works for salary).
...
(a) root-functional head combinations or ``stems'' with fixed preferences of syntactic environments (e.g. valency)
...
When large, phrasal idioms are excluded from our consideration,
...
Inconsistency between the two types of lexicalization still exists,
for instance in the case of \prettyref{box:lexeme-meaning},
in which forms with different lexicalized meanings 
share a common subpart, which dictates the morphological and syntactic profiles.
In many languages, typically this can be resolved by introducing concepts like \concept{word family}:
grammatical words with different idiomized meanings,
whose morphological realizational details and syntactic environment preferences are however determined by a single shared subpart (as in \prettyref{box:lexeme-meaning}),
are said to form a word family.

The convergence of two types of lexicalization
is strongly related to the purported universal notion of \concept{grammatical wordhood},
as it involves both syntax and morphological realization:
it is strongly related to morphological wordhood
because lexicalization of form dictates morphological realization,
and syntactic properties commonly attributed to words, like subcategorization,
can be captured in the framework of morphological realization as well 
(see discussions above).
The way stems are mapped to morphological words is commonly known as \emph{inflectional} morphology,
which, in certain cases (like auxiliary verb constructions),
may create multiple morphological words for a single lexeme.

Once we have a language-specific definition of lexemes based on lexicalization,
parts of speech can be defined.
...
Given that lexicalization of form and lexicalization of meaning are highly correlated,
this implies that the meanings of root-affix tuples are also not deterministic:
\wordroot{eat}-\category{nominal}, for instance,
would be understood as the action of eating or something to eat, 
depending on the context.

\subsection{Secondary concepts}

The notion of lexemes 
inevitably prompts us to make the derivation/inflection distinction,
which is closely related to the size of lexemes.
The position of this book is that 
the derivation/inflection distinction
(or the argument/adjunct distinction, and the like), are epiphenomena in our theoretical framework,
and in this grammar we do not attempt to do clear demarcations 
and merely focus on the relevant grammatical phenomena related to these distinctions.
Here, we argue why a clear derivation/inflection distinction is not possible.

A clear derivation/inflection distinction based on universally attested morphosyntactic categories is always problematic.
...
if understood as a universal phenomenon in natural languages,
reflects parameters like how close a grammatical category is to the root in one construction
(template morphology is more likely to develop for
relatively external categories in the TP projections)
or the morphological profiles of a language,
but how internal a grammatical category should be to be considered derivational is not clear,
and nor is how layered its morphological realization should be clear.
...
Here, we define a lexeme as what is lexicalized in its form (\prettyref{sec:intro.theory.lexicon}),
but this means the paradigm of a lexeme should also include some of its regular derivations.%
\footnote{
    And a Latin learner may find our opinion quite reasonable.
}


Now we turn to another distinction, the argument/adjunct distinction.
The standard of being an \concept{argument} can be defined according to criteria listed in \prettyref{sec:grammatical.clause.peripheral}.
...
\begin{theorybox}{Shared syntactic properties of words and phrases in English}{english-word-phrase}
    For instance, regular syntactic processes happen to both phrases and what are common considered words.
    Some syntactic tests aimed at distinguishing the two are given in \citet[\citepages{448-451}]{cgel},
    but a quick google search reveals that \form{tooth- and back-ache}
    or \form{pre- and post-revolutionary France} are both considered acceptable by many.
    I've even seen \form{Sun- and holidays} on Long Island.
...
forming one morphological word,
which at the same time decides (by its lexical properties)
structural possibilities (e.g. valency) of the construction it heads.
The core of this head morphological word is known as the \concept{lexeme},
...
\begin{theorybox}{On how large a \term{lexeme} should be}{lexeme-meaning}
    ...
    This is an instance of mismatch between lexicalization of form
    and lexicalization of meaning.
...
\begin{theorybox}{Terminology: \term{dephrasal} and \term{compound}}{dephrasal-compound}
    The terms \term{dephrasal} and \term{compound} here 
    do not imply categorical distinction between prototypical phrasal syntax and grammar of word
    (see \prettyref{box:english-word-phrase}; \citealt{scher2014unifying}).
    In this grammar, grammatical wordhood is defined solely on the criteria in \prettyref{sec:grammatical.wordhood.grammatical-word},
    and we \emph{define} a verbal lexeme (which is stored in the lexicon with its argument structure specified)
    made of roots as a \term{compound},
    and if its structure is comparable to that of a prototypical phrase,
    then it is a \term{dephrasal compound}.
    This does not deny that syntactic processes apply to compounds too,
    or that the lexical entry that determines the valency and very often the morphological realization
    and the lexical entry that determines the meaning are not always identical
    (\prettyref{box:lexeme-meaning}).

    Whether actually the internal syntax of a dephrasal compound is the \emph{same} as prototypical phrasal syntax is not always clear.
    It could just be \concept{recategorization}, i.e. a verb phrase as a whole wears a verb frame outside of it,
    ...
    A form I saw on Twitter, \form{I always this-is-the-pride-month it},
    is a clear example of the former.
    The latter case can be illustrated by \form{sickbed},
    which can be regularly interpreted as \translate{bed related to sickness}, 
    ...
    Mixing of the two is also possible, as in \form{[killing 682]-ology} from \href{https://scp-wiki.wikidot.com/forum/t-410418/scp-1066}{this forum discussion}.
    %As usual, syntactic criteria hardly provide clear demarcation of wordhood without considering lexicalization
    ...
    and so is the verb in (\ref{ex:grammatical.clause.core-vp.compounding.coordination.2}).
    The distinction is also related to whether we should label 关 in \prettyref{fig:grammatical.clause.core-vp.compounding.1} as a predicator (which only makes sense when we have a ``real'' verb phrase structure).
    Our labeling here is tentative. 
    The problem of labeling of constituents of compounds becomes even more problematic 
    ...
    so any labeling treating \form{worker} as a whole is problematic.
...
``real'' compounding,
different from (\ref{ex:grammatical.clause.core-vp.compounding.coordination.1}).
...
\subsubsection{Non-dephrasal compounds}\label{sec:grammatical.np.derivation.compounding}
...
The semantics of the three nominalization devices has subtle differences,
which can be illustrated in \ref{ex:nouns.complex-noun.nominalization.1}.
性 refers to an absolute property and/or the ``essence'' of something having some properties.
Thus 甜性 refers to the property of something being sweet,
which likely appears only in philosophical contexts.
On the other hand, zero conversion is a nominalization construction converting the whole verbal argument structure (?)

\begin{exe}
    \ex\label{ex:nouns.complex-noun.nominalization.1} \begin{xlist}
        \ex 甜头
        \ex 甜性
        \ex 苹果的甜和我心里的苦放在一起对比鲜明啊
    \end{xlist}
\end{exe}

\subsection{Compounding and bound roots}\label{sec:nouns.complex-noun.roots}
...
\subsubsection{Hyponymic compounds}\label{sec:nouns.complex-noun.modification}

A large class of compounds in Mandarin are \concept{hyponymic compounds},
which consist of two parts, the first specifying the coverage of the second.
In Mandarin, in almost all hyponymic compounds,
the first branch modifies the second.

The root 工 \translate{worker}, for instance, often serves as the second part in hyponymic compounding.
Its meaning cannot be summarized as something like \translate{agent of an action},
so it cannot be a grammatical formative.
It is also productive (\ref{ex:nouns.complex-noun.roots.1}).

\begin{exe}
    \ex\label{ex:nouns.complex-noun.roots.1} \begin{xlist}
        \ex 劳工 \translate{laborer} 
        \ex 操作工 \translate{operator}
        \ex 纺织工 \translate{textile worker}
        \ex 车工 \translate{(lathe) turner}
        \ex 船工 \translate{boat worker}
        \ex 电工 \translate{electrician}
    \end{xlist}
\end{exe}

Another example is 计 (\ref{ex:nouns.complex-noun.roots.2}).
We note that two roots, 计 and 工, select different roots preceding them:

\begin{exe}
    \ex\label{ex:nouns.complex-noun.roots.2} \begin{xlist}
        \ex 水温计
        \ex 油温计
    \end{xlist}
\end{exe}
...
\subsubsection{Tautological compounds}\label{sec:nouns.complex-noun.the-same}

Another type of compounds in Mandarin are made of two parts with similar meanings.
This construction is probably a reflex of tautological rhetoric devices (cf. English \form{happiness and bliss}) in earlier stages of Chinese.
...
Tautological compounding is productive,
and not all compounds produced by the process are well established in form or meaning.
(\ref{ex:nouns.complex-noun.tautological.2.a})
is the standard term for \translate{effect, result},
while (\ref{ex:nouns.complex-noun.tautological.2.b}) is non-standard,
which may be interpreted in the same way by an ordinary Mandarin speaker.
Christians, on the other hand, use (\ref{ex:nouns.complex-noun.tautological.2.b})
to refer to spiritual results 
in contrast with (\ref{ex:nouns.complex-noun.tautological.2.a}),
which refers to secular results and consequences.

\begin{exe}
    \ex\label{ex:nouns.complex-noun.tautological.2} \begin{xlist}
        \ex\label{ex:nouns.complex-noun.tautological.2.a} 效果
        \ex\label{ex:nouns.complex-noun.tautological.2.b} 果效
    \end{xlist}
\end{exe}

\subsubsection{Coordinative compounds}\label{sec:nouns.complex-noun.coordinate}

Coordinative compounds are infrequent in Mandarin,
and mostly are calques for foreign terms (\ref{ex:nouns.complex-noun.coordinate.1}).

\begin{exe}
    \ex\label{ex:nouns.complex-noun.coordinate.1}
    [奥地利-匈牙利]帝国
\end{exe}

\subsubsection{Dephrasal compounds}

In \prettyref{sec:grammatical.np.adjectives} we have mentioned that 
compounds can be constructed using syntactic rules pertaining to prototypical phrases
(\prettyref{fig:grammatical.np.adjective.1} vs. \prettyref{fig:grammatical.np.adjective.2}).
In this section, we enumerate structures of common dephrasal compounds in Mandarin.

The boundary between ``ordinary'' compounds and dephrasal compounds is sometimes unclear.


On the other hand, (\ref{ex:nouns.complex-noun.dephrasal.2}) is good 

\begin{exe}
    \ex \label{ex:nouns.complex-noun.dephrasal.2}
    \begin{xlist}
        \ex 白大褂
        \ex 金笔
    \end{xlist}
...
\subsubsection{Classical Chinese terms}

The structures of examples in (\ref{ex:nouns.complex-noun.dephrasal.1})
cannot be explained by regular compounding constructions in
\prettyref{sec:nouns.complex-noun.modification}--\prettyref{sec:nouns.complex-noun.coordinate}.
They may be classified as dephrasal compounds.
We however note that certain forms, like (\ref{ex:nouns.complex-noun.dephrasal.1.b})
and (\ref{ex:nouns.complex-noun.dephrasal.1.c}),
are not acceptable as modern Modern phrases,
unlike examples in (\ref{ex:nouns.complex-noun.dephrasal.2}).
至, for instance, is never used as a verb now.
These forms \emph{were} Classical Chinese phrases,
which later became first idiomized and then found their way in modern Mandarin.

\begin{exe}
    \ex\label{ex:nouns.complex-noun.dephrasal.1} \begin{xlist}
        \ex 霜降
        \ex\label{ex:nouns.complex-noun.dephrasal.1.b} 主席
        \ex\label{ex:nouns.complex-noun.dephrasal.1.c} 冬至
    \end{xlist}
\end{exe}

In Mandarin, the internal structures of many of these forms stop being productive,
which means we have reasons to analyze these forms synchronically either as \emph{roots},
or as ad hoc compounds (TODO: ref).
...
\subsubsection{Comparison with affixes}

The distinction between bound roots and affixes is not always clear.
In English, for instance, the \form{-logy} formative almost always appears
as the second element in neo-classical compounds,
but this can be explained by \form{-logy} having to be complemented by \emph{what} is being studies.
The giant stock of English neo-classical final combining forms
and their diverse meanings mean that these formatives are highly unlikely to be suffixes.
...
By default, the condition clause always appears before the main clause.
