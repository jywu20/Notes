...
This grammar does not assume existence of a unit called ``word'' a priori in constituency relations in syntax,%
\footnote{
...
If we try to define wordhood according to a single, clearly defined standard, like verb valency --
...
What is truly non-trivial, then, is whether and how these three definitions of wordhood overlap.
...
does not depend on the derivation/inflection distinction.
Lexicalization, a concept often related to derivation
(``what is stored in the lexicon has to be formed by derivation'')
is treated on its own (\prettyref{sec:grammatical.lexicon}).
...
Therefore, 吃-in-transitive-verbal-environment -- that's to say, 吃 as a \term{verb} or more clearly as a \term{verb lexeme} (\prettyref{box:verb-definition}) -- 
...
By saying that 吃 is lexicalized as a verb,
we mean that 吃 appearing at the head position has been \emph{idiomized}.
Lexemes can be conceived as small idioms (\prettyref{sec:grammatical.lexicon.idiom}).

The main difference between lexemes and more prototypical idioms
is that the former are smaller in size --
which hints at a possible purely syntactic definition of wordhood.
We may want to run morphological and phonological wordhood tests on these lexemes
as well to see if a \emph{global} definition of wordhood exists in Mandarin
based on correlations between different wordhood definitions.

The verbal complex can be optionally made of one lexeme plus suffixes TODO
