...
Some written materials in Chinese were definitely \emph{not} written in 文言 i.e. Classical Chinese,
...
(TODO: specify which varieties were most influential.)
...
We consider texts before the Spring and Autumn period Pre-Classical,
despite their classic status in a Confucian context.
This periodization is consistent with traditional philological scholarship in China:
...
The \work{Book of Odes} includes poems spanning a long period of time from early Zhou to Spring and Autumn,
and does not have a single, coherent grammar.
Thus, these texts are excluded from the scope of this work.

This work attempts to analyze the shared grammatical traits of these admittedly highly diverse texts.
...
and can only be proven by demonstrating that most texts considered Classical are analyzable under the framework of this work.

The mental grammar of an author of post-Han 文言 prose almost certainly differed from any possible grammar of Classical writers.
...
The conjunction 而 is never seen in pre-Spring and Autumn texts
...
Comparison between the language of Classical prose and the language(s) of poetry
shows the relative homogeneity of the former,
while the latter both demonstrate divergence from the language of prose
and regional differences.

\begin{todobox}{Poetry and prose}{peotry-and-prose-before-han}
    This is presumably due to how the texts were transmitted.
    It is likely that they were passed by recitation,
    and regularization happened to prose when there was a predominant dialect,
...
has the following differences from the language of prose.
...
The language of the \work{Odes} also seems to differ slightly from prose Classical Chinese.
...
and also proliferation of disyllabic words.
...
太史 \translate{grand historian} was in charge of administering archives and making calendars.
...
but may have a variety of innovations.
...
In this chapter we summarize the syntax of the clause (\prettyref{sec:grammatical.clause}) and the syntax of the noun phrase (\prettyref{sec:grammatical.np}), along two intertwined lines:
...
Thus the Mandarin verb 关心, despite having a regular verb-object structure,
...
\begin{exe}
    ...
    \gll [此 人]_{\text{subject}} [力士]_{\text{predicate}} \\ 
    this person strength-man \\ 
    \glt\translate{This person is a man of great strength.} \hfill (史记·魏公子列传)
...
\begin{exe}
    ...
    \gll 且 蔺相如 [素]_{\text{TAM}} 贱人 \\ 
    and \category{name} always humble-person \\ 
    \glt\translate{\dots and Lin Xiangru has always been a person with a very low social status.}
\end{exe}
...
We use the standard S, A, P, etc. labels to refer to argument slots,
without any implication of unity (S roles famously split into A-like and O-like ones, for instance).
...
\begin{exe}
    ...
    \gll [三王]_{\text{subject}} [[既]_{\text{\ac{tam}}} [以 ---]_{\text{instrument}} [定]_{\text{main verb}} [法度]_{\text{object}}]_{\text{predicate:VP}} \\ 
    three-king already using {} determine law-system \\ 
    \glt\translate{The three great kings had already established social institutions by [it].}
\end{exe}
...
As an instance, it is hard to determine if the second verb had already been incorporated into the first verb in the sequence 问曰 in a text when it was written.
We do not find any instance in Classical texts that require a verbal morphological analysis 
(问曰 for instance at least originally came from argument structure-level coordination; cf. \ref{ex:grammatical.clause.verbal.argument.complement-clause.direct-quotation.v-say.1}).
...
\footnote{
    In traditional terms, 词类活用 \translate{word class flexibility} does not mean 一词多义. 
}
...
\begin{exe}
    ...
    [子]_{\text{subject,A}} [[奚]_{\text{reason (\prettyref{sec:grammatical.clause.verbal.argument.high-level})}} 不 [为]_{\text{verb},\category{do}} [政]_{\text{P}}]_{\text{predicate:VP}}？

    \ex\label{ex:grammatical.clause.verbal.argument.do-be-become.intransitive.1}
    \gll [君子]_{\text{S}} [不 [器]_{\text{verb,\category{be}}}]_{\text{predicate:VP}} \\ 
    lord-son \category{neg} instrument \\ 
    \glt\translate{A gentleman does not resemble an instrument/container (i.e. his skills are broad).}
\end{exe}
...
a state (\category{be}; \ref{ex:grammatical.clause.verbal.argument.do-be-become.intransitive.1}, \prettyref{sec:valency.simple.state-and-change}) or a change of the state (\category{become}; \prettyref{sec:valency.simple.state-and-change}).
...
\footnote{
    More accurately, \citet{li2004grammar} classifies subtypes of objects (受动宾语 vs. 致动宾语).
...
The verb 杀 is otherwise a \category{do} verb,
...
The sequence of 以 plus the argument following it is never fronted,
implying that the construction is not a prepositional argument construction, but a light verb construction. 
Other light verb constructions include the 为…所 construction and more.
...
(\ref{ex:grammatical.clause.verbal.argument.prepositional.fall.1}, \ref{ex:grammatical.clause.verbal.argument.prepositional.give.1}, \ref{ex:grammatical.clause.verbal.argument.prepositional.ask.1}) are examples of prepositional constituents, carrying a wide range of semantic roles.
...
Prepositional phrases can also be fronted (\prettyref{sec:grammatical.clause.verbal.argument.high-level}).
...
\begin{exe}
    ...
    \glt\translate{The duke was afraid and fell from the carriage.}

    \ex\label{ex:grammatical.clause.verbal.argument.prepositional.give.1}  
    \gll 故 赐 [之] [以 重 祭]_{\textbf{\text{theme}}} \\ 
    therefore give 3 \category{prep} solemn offering \\ 
    \glt\translate{\dots and therefore gave him solemn venerations.}

    \ex\label{ex:grammatical.clause.verbal.argument.prepositional.ask.1} 
    \gll 季康子 问 政 [于 孔子]_{\textbf{\text{addressee}}} \\ 
    \category{name} ask politics \category{prep} \category{name} \\ 
    \glt\translate{J. asks Confucius about politics.}
\end{exe}
...
though circumstantial constituents, like those in the 见…於 construction, are likely structurally more external \citep[\citepage{323}]{meiguang2018}. 

Some prepositional arguments can be replaced by non-prepositional objects (\ref{ex:grammatical.clause.verbal.argument.prepositional.no-prep.1}).

\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.prepositional.no-prep.1} 入秦何見
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.possession.1} 子路性鄙
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.complement-clause.1} 臣窃以为 [不便於君]_{\text{\textbf{complement clause}}}
    \ex\label{ex:grammatical.clause.verbal.argument.complement-clause.direct-quotation.say.1} 	
    子曰：[“不患人之不己知，患不知人也。”]_{\text{\textbf{direct quotation}}}
\end{exe}
...
\begin{exe}
    ...
    \gll [子禽]_{\text{subject: NP}} [[问]_{\text{verb}} [于 子贡]_{\text{target}}]_{\text{verb phrase 1}} [[曰：“…”]_{\text{direct quotation}}]_{\text{verb phrase 2}} \\
...
Raising-to-object, object control, and similar argument sharing constructions are collectively known as 兼语式 or the \term{pivot construction} \citep[\citepage{40}]{pulleyblank1995outline}.
...
\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.raising-control.raising.1} 令军勿敢犯
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.raising-control.control.2} 吾不欲觀之矣
\end{exe}
...
First, the object pronoun is often regularly moved before the verb in a negative clause (\ref{ex:grammatical.clause.verbal.argument.high-level.sov.neg.1}) without clear information structure motivation, leading to a SOV order.
An interrogative object pronoun can also be fronted (\ref{ex:grammatical.clause.verbal.argument.high-level.sov.interrogative.1}).
This may be analyzed as the pronoun clitic being attached to the negator or the main verb \citep{aldridge2010cliticization}.

\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.high-level.sov.neg.1}
    \gll 余  [不  女  忍  杀]_{\text{predicate}} \\
    1 \category{neg} 2 endure kill \\ 
    \glt\translate{I can't bear to kill you.} \hfill (左传·昭公元年)
...
\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.high-level.focus.1} 季康子 [以其妹]_{\text{VP-focus}_i} 妻 之 ---_{i}
\end{exe}
...
This is much rarer than fronting of object pronouns,
which again is consistent with verb phrase internal focalization. 

\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.high-level.focus.2} 室於怒而市於色
\end{exe}
...
That the adverb is before the anteriority adverb 已 demonstrates its high-scope, speaker-oriented nature.

\begin{exe}
    \ex\label{ex:grammatical.clause.tam.speaker-oriented.1} [固] [已] 多敗亡
\end{exe}

On the other hand, \citet[\citepage{359}]{li2004grammar} notes that clauses with certain sentence final particles,
which he calls 说明句, are more subjective.
The sentence initial particle 盖 also marks an inferential attitude towards the situation being reported.
...
\begin{exe}
    \ex\label{ex:grammatical.clause.tam.sentence-aspect.1} 此迫矣
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.clause.tam.tense.1} 夫子 [殆]_{\text{epistemic}} [將]_{\text{future}} 病也
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.clause.tam.ability.1} 我 [固]_{\text{objectivity}} [能]_{\text{possibility}} 治偏枯
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.clause.tam.progressive.1} 秦王方環柱走
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.clause.tam.lexical.1} 行二百餘里
    \ex\label{ex:grammatical.clause.tam.lexical.2} [常]_{\text{habitual}} 封三錢之府
\end{exe}
...
First, in reading traditions of Classical Chinese (as indicated by traditional 句读 punctuations), some pivotal constituents are followed by a pause,
...
Second, in tropative clauses, the pivotal constituent closest to the main verb 
...
\begin{exe}
    \ex\label{ex:grammatical.clause.subject.topic-comparison.tropative.1} 孟尝君客我 
\end{exe}

When the pre-verbal pivotal constituent closest to the main verb is semantically the patient,
it is often the case that it is obligatorily so (\ref{ex:grammatical.clause.subject.topic-comparison.passive.1}),
...
\begin{exe}
    \ex\label{ex:grammatical.clause.subject.argument-structure.giving.1} [诸侯]_{\text{G}}，赐 [弓矢]_{\text{T}} 然后征
\end{exe}
...
is much less frequent than the so-called 判断句 \translate{judgmental clause},
...
\begin{exe}
    ...
    \gll [城 北 徐-公]_{\text{topic: \ac{np}}}， [齐-国 之 美-丽 者]_{\text{comment: \prettyref{sec:grammatical.np.relative}}} 也 \\
    city north \category{name}-\category{gong} Qi-country \category{gen} beautiful-beautiful \category{rel} \category{sfp} \\
    \glt\translate{Mr. Xu from the north of the city is a handsome man in the country Qi.}
...
\begin{exe}
    \ex\label{ex:grammatical.clause.topic.verbal.1} 此二人者，实弑寡君
\end{exe}
...
\begin{exe}
    ...
    [仁之实]_{\text{topic:NP_i}}，[[事亲]_{\text{subject}} [是]_{\text{predicate_i}}]_{\text{comment}} 也 
\end{exe}
...
Some topic-like constituents are analyzed as \term{frames} in \citet[\citepage{129}]{meiguang2018}.
...
\begin{exe}
    ...
    \gll [[甚]_{\text{VP_i}} 矣]_{\text{focus}}， [[汝 之 不 惠]_{\text{subject:clause (\prettyref{sec:grammatical.clause.force.nonfinite})}} ---_{\text{predicate_i}}] \\
...
These categories are traditionally known as \term{clause types} or 句类, as opposed to 句型 i.e. the internal structure of clauses \citep[\citepage{58}]{li2004grammar}.
...
Interrogative clauses may completely lack formal marking except interrogative pro-forms (\ref{ex:grammatical.clause.verbal.argument.high-level.sov.interrogative.1}).
...
\begin{exe}
    ...
    \gll 王 如 知 此， 则 无 望 [民 之 多 于 邻 国]_{\text{object:\form{zhī}-construction}} 也 \\
    ...
    [父母之爱子]_{\text{condition:\form{zhī}-construction}}，则为之计深远
\end{exe}
...
The consequence can be marked by a particle (e.g. 则; \prettyref{ex:grammatical.clause.linking.conditional.1}).
...
\begin{exe}
    \ex\label{ex:grammatical.clause.linking.vp.1}
     吾尝 [跂]_{\text{modifier verb phrase}} 而 [望]_{\text{main verb phrase}} 矣
\end{exe}
...
\begin{exe}
    ...
    声伯…… [食使者]_{\text{verb phrase 1}}，而后 [食]_{\text{verb phrase 2}}
\end{exe}
...
\begin{exe}
    ...
    \gll [舟]_{\text{modifier}}[人]_{\text{head}} \\ 
    boat-person \\ 
    \glt\translate{boatman}
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.np.word.2} 社稷之臣
\end{exe}
...
坚 refers to \translate{what are hard (i.e. defensive gears)}
and 锐 refers to \translate{offensive weapons},
...
\begin{exe}
    \ex\label{ex:grammatical.np.nominalization.1} 被坚执锐
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.np.direct-modification.1} 大白狗 \hfill (汉书·五行志)
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.np.relative.modify.1} 晉之從政者
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.np.relative.long.1} 若[--- 至力农畜， --- 工虞商贾， --- 为权利以成富，[--- 大者倾郡]，[--- 中者倾县]，[--- 下者倾乡里]]者，不可胜数 
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.np.relative.pre.1} 得誌之君子
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.np.determiner.dem.1} 此三姦者 (荀子·非十二子)
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.np.determiner.gen.1} 君之三臣
\end{exe}
...
\begin{exe}
    \ex\label{ex:grammatical.np.preposition.1} 且貳於楚也
\end{exe}

It is possible to omit the complement of a preposition.

\begin{exe}
    \ex\label{ex:grammatical.np.preposition.omission.1} 愚不足與謀智
\end{exe}
...
An overview of these words (虚词 in the traditional terminology) can be found in \citet[\citechap{4}]{li2004grammar}.
...
\small{
    ...
    and if they are synchronic, whether they derive from human language capacity or from some other cognitive capacities.
    ...
    Regarding the first question, neurolinguistic experiments suggest that both holistic and sub-lexical processing exist in the human brain.
    ...
    while the existence of sub-lexical processing is supported by the fact that characters with valid semantic or phonetic components are processed more quickly
    ...
    and this suffices as an argument against the assumption that the ``grammar'' of Chinese characters and the grammars of spoken languages have the same neurological origin \citep[\citepages{209-210}]{myers2019grammar}.
    Still, this does not by itself settle the cognitive status of character-internal structure.
}
...
\begin{exe}
    \ex \gll [子禽]_{\text{subject: NP}} [[问]_{\text{verb}} [于 子贡]_{\text{target}}]_{\text{verb phrase 1}} [[曰：“…”]_{\text{direct quotation}}]_{\text{verb phrase 2}} \\
    \category{name} ask at \category{name} say \\
    \glt\translate{Ziqin asked Zigong: \dots}
