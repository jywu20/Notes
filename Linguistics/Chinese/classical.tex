...
Concepts like SpecTP and SpecvP, in a descriptive context, correspond to concepts like \term{clausal subject} and \term{pivot in the argument structure} (\prettyref{sec:grammatical.verbal.subject}).
...
a structure building process being structurally limited (e.g. impossibility of coordination) should not automatically qualify it as a derivation process (though in the existing literature, the derivation/phase grammar distinction is often dealt in this way).
...
(e.g. \prettyref{sec:grammatical.clause.verb}).
...
One strongly indication of wordhood in Classical Chinese is AABB reduplication:
...
Another largely uncontroversial criterion of wordhood is lexicalization:
an idiom (like \form{kick the bucket}) also has a single, coherent meaning, 
...
The internal makeup of clauses and noun phrases depends on lexical properties of the head verb/noun,
...
a verbal clause can have a predicate headed by a form only recorded as a noun in the lexicon (cf. \prettyref{sec:grammatical.clause.verb}).
...
a clause with verbal predication consists of a \term{subject} (\prettyref{sec:grammatical.verbal.subject}) and a verb phrase (VP), which is the predicate of the clause and contains \ac{tam} adverbs and core and peripheral arguments.
...
\label{sec:grammatical.clause.verb}
...
and should thus be considered verbalization derivation.
An important process of this kind is the tropative (\ref{ex:grammatical.clause.subject.causative.1}).

Whether certain multimorphemic forms should be considered outputs of verb derivation is not completely clear,
because of the blurry boundary between words and larger units (\prettyref{sec:grammatical.morphology}).
As an instance, it is hard to determine if the second verb is incorporated into the first verb in the sequence 问曰 (which comes from argument structure-level coordination; cf. \ref{ex:grammatical.clause.argument.verbal-complementation.direct-quotation.v-say.1}).
...
On the other hand, the direct quotation in (\ref{ex:grammatical.clause.argument.verbal-complementation.direct-quotation.say.1}) is sentential has a sentence final particle.
...
Other verbs of speech, which themselves do not license complement clauses (\ref{ex:grammatical.clause.argument.verbal-complementation.direct-quotation.v-say.1.2}),
...
\begin{exe}
    ...
    \ex\label{ex:grammatical.clause.argument.verbal-complementation.direct-quotation.v-say.1.2} 季康子患盜，問於孔子。
\end{exe}
...
\begin{exe}
    ...
    \gll 余  [不  女  忍  殺]_{\text{predicate}} \\
    1 \category{neg} 2 endure kill \\ 
    \glt\translate{I can't bear to kill you.} \hfill (左传)
...
\begin{exe}
    ...
    \gll [[甚]_{\text{VP_i}} 矣]_{\text{focus}}， [[汝 之 不 惠]_{\text{subject:clause (\prettyref{sec:grammatical.clause.verbal.gerundive})}} ---_{\text{predicate_i}}]_{\text{nucleus clause}} \\
...
Classical Chinese has one clausal construction that at the first glance resembles the noun phrase.
Instances of it appear as the object
...
The construction is sometimes known as nominalization \citep[\citepage{64}]{pulleyblank1995outline}.
This term cannot be taken literally, because the construction does not admit adjectival modification,
while there is nothing resembling an object (e.g. 子 in \ref{ex:grammatical.clause.verbal.gerundive.2}) in prototypical Classical Chinese noun phrases.
...
Relative clauses are discussed in TODO

\subsubsection{Sentential subordination}
...
An interesting phenomenon is that the subordinated clause can be a nonfinite 之 clause (\prettyref{ex:grammatical.clause.verbal.gerundive.2}).
...
which is only licensed in a clausal environment and cannot link to nominal constituents.
Note that the functions of 而 are not restricted to conjunction \citep[\citepage{183}]{meiguang2018}.
...
\subsection{Structure of the noun}
...
\subsection{Direct modification adjectives}
...
However, examples involving multiple pre-nominal attributives are scarce.
...
\begin{todobox}{What can be relativized, and possible external possession}{external-possession-or-relative-clause}
    ...
    then Classical Chinese should have an external possession construction:
...
Second, it seems that 矣 and 也 can be shared by two coordinated conjuncts with different subjects (\prettyref{sec:grammatical.clause.sfp}),
