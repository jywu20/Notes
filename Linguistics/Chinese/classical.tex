...
can be about an intentionally initiated event (\category{do}; \ref{ex:grammatical.clause.svo.interrogative.1}),
a state (\category{be}; \ref{ex:grammatical.clause.svo.intransitive.1}) or a change of the state (\category{become}; \prettyref{sec:valency.simple.state-and-change}).
...
(\ref{ex:grammatical.clause.verbal.argument.valency-alternation.1}) is an example of a causative clause based on the pseudo-passive construction.
...
implying that the construction is not a prepositional argument construction. 
...
(\prettyref{sec:grammatical.clause.subject.argument-structure}, \prettyref{sec:grammatical.clause.verbal.argument-structure.pseudo-passive.multiple-argument}),
which means it is somehow more ``external'' or ``subject-like'' (\prettyref{sec:grammatical.clause.subject.argument-structure}).
