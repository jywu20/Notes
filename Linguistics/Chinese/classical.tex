...
Grammatical markers can be distinguished from content roots and words 
by virtue of having rigid linear orders and limited choices.
...
they are related to extractional properties of arguments in valency alternation (\prettyref{sec:grammatical.verbal.subject.argument-structure.alternation}),
...
\footnote{
    ...
    The agent position thus becomes a subject position at least in the argument structure, consistent with modern practices (\prettyref{sec:grammatical.verbal.subject.argument-structure.alternation}).
...
(see also \prettyref{sec:introduction.coverage}).

\section{Coverage}\label{sec:introduction.coverage}
...
For one, we identify nominal and verbal \emph{grammatical environments}
...
These discourse-related devices typically only appear in sentential clauses and are absent in embedded clauses (apart from direct quotations; \prettyref{sec:grammatical.clause.argument.verbal-complementation.direct-quotation}). 
...
A clause with verbal predication consists of a \term{subject} (\prettyref{sec:grammatical.verbal.subject}) and a \ac{vp}, which is the predicate of the clause and contains \ac{tam} adverbs and core and peripheral arguments.
\Acp{vp} can be coordinated (\prettyref{sec:grammatical.clause.coordination}), demonstrating their constituent status.
...
The \ac{vp} therefore can be analyzed as the (extended) argument structure
...
in morphologically richer languages, the mapping from the two to surface morphological cases may also be heterogeneous \citep{maling2001dative}.
...
It is cross-linguistically hard to establish the \term{object} as a recurring grammatical function across different clausal constructions (cf. \citealt[\citepages{28-29}]{Pinkster1}).
...
A peculiarity of Classical Chinese is that roots typically appearing in non-verbal environments frequently undergo verbalization or even ad hoc recategorization.
...
(\ref{ex:grammatical.clause.svo.interrogative.1}).
...
can be about an intentionally initiated event (\category{do}; \prettyref{sec:valency.simple.do}),
...
Finally, \category{become} and \category{be} clauses are often inputs to the synthetic causative construction,
...
animacy and volition of the subject as well as the viability of argument structure alternation processes (\prettyref{sec:valency.simple.do.properties}): S=A labile valency alternation naturally exists for \category{do} clauses,
and S=P labile valency alternation naturally exists for \category{cause} clauses.
...
for instance, some causative verbs have no intransitive counterparts (\prettyref{sec:grammatical.clause.verbal.argument-structure.causative.fossilization}),
...
The synthetic causative is a productive valency increasing construction in Classical Chinese,
which produces transitive \category{cause} clauses
(\prettyref{sec:grammatical.clause.verbal.argument-structure.causative}).
On the other hand, Classical Chinese has a productive valency decreasing construction,
...
like the recipient (\ref{ex:grammatical.clause.verbal.argument.prepositional.give.1}),
...
\begin{exe}
    ...
    \gll 故 赐 [之]_{\text{goal}} [以 重 祭]_{\textbf{\text{theme}}} \\ 
...
\subsubsection{Complement clause constructions}
...
\subsubsection{Direct quotations}\label{sec:grammatical.clause.argument.verbal-complementation.direct-quotation}

Classical Chinese has direction quotation (\ref{ex:grammatical.clause.argument.verbal-complementation.direct-quotation.say.1}).
The quoted content is a \emph{sentence}, which includes a sentence final particle,
...
and is commonly known as \term{control} in general linguistics.
TODO: or raising?

(\ref{ex:grammatical.clause.argument.verbal-complementation.raising.1}) is an example of control in Classical Chinese
and also an example of the analytic causative construction (\prettyref{sec:valency.causative.analytic}).
...
(\prettyref{sec:grammatical.verbal.subject.argument-structure.alternation}),
...
what argument can be externalized into a pivotal constituent,
...
\subsubsection{Pivot in argument structure}\label{sec:grammatical.verbal.subject.argument-structure.alternation}

The existence of a pivot on the level of argument structure can be demonstrated by examining the shared argument in control constructions.
...
which then may be known as the second most external argument.
...
\begin{exe}
    \ex \gll [秦]_{\text{subject}}， [虎 狼 之 国]_{\text{predicate}} \\
...
Coordination of two \acp{vp} has two structural possibilities:
...
\subsection{Lexicalized synthetic causative construction}
