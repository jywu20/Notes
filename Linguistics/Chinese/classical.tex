...
Han Yu, renowned for reviving Classical proses, famously described the \work{Book of Documents} as 诘屈聱牙 (\translate{twisted, hard to pronounce}) in his 进学解 (\work{Analysis of academic advancement}). 
...
The \work{Book of Odes} includes poem spanning a long period of time from early Zhou to Spring and Autumn,
and do not have a single, coherent grammar.
Thus, these texts are not excluded from the scope of this work.
...
The mental grammar of an author of post-Han 文言 proses almost certainly differed from any possible grammar of Classical writers.
...
with the possessive marker 的 is replaced by 之,
...
\footnote{
    ...
    Generativism treats grammar as a semi-autonomous component of human's cognitive abilities.
...
We note that we do not consider Adjoin an independent structure building process that is different from Merge.
...
Yet the primacy of noun phrases and clauses in syntax is indeed acknowledged by generative syntax (for instance they are \term{phases})
...
they are related to extractional properties of arguments in valency alternation (\prettyref{sec:grammatical.clause.subject.argument-structure}),
...
ordering of tense and aspect auxiliary and adverbs à la Cinque, attested in languages completely unrelated to Germanic or Romance languages that have been thoroughly investigated in Cartography (e.g. \citealt[\citepages{166-167}]{grimm2021grammar}),
...
Therefore, the lexicon (understood as List B) is often said to contains \term{words} or \term{lexemes} (the latter term assumes a clear inflection/derivation distinction; see below)
...
However, Classical Chinese clearly possesses the level of word in its grammar.
The reduplicated form 沉沉 does not appear to be generated by any combinatorial syntactic process: 
...
A nucleus clause may be have nominal (\prettyref{sec:grammatical.clause.nominal}) or verbal predication (\prettyref{sec:grammatical.clause.verbal}).
...
A nominal predicate typically express an ``is-a'' relation between the subject (\prettyref{sec:grammatical.verbal.subject.clause-pivot}) and the predicate
(\ref{ex:grammatical.clause.nominal.isa.1}).
...
The term \term{object} is used to refer to any argument without an explicit preposition for convenience. 
...
Cross-linguistically, many phenomena commonly attributed to the distinction do not require a primitive core/peripheral distinction of clausal dependents to explain,
and subcategorization i.e. lexicalization of the argument structure,
arguably the golden standard of the distinction,
is uncontroversially demonstrable in some but not all situations
(as obligatoriness sometimes originates from pragmatic or semantic factors),
and diagnostics based on the structural closeness of the arguments to the main verb (e.g. pro-form substitution, extraction constraints) fail to show qualitative distinctions \citep[\citepages{112-117, 125, 128, 212}]{mcinnerney2022argument}.
Lexicalization of verb frame is indeed attested in Classical Chinese (e.g. \prettyref{sec:grammatical.clause.verbal.argument.do-be-become}).
...
it expresses a dynamic event and not just a state and has an modality auxiliary (illustrating that the predicate is not a noun phrase).
...
\subsubsection{Passive and causative constructions}
...
and a productive valency decreasing construction
which may be known as the (pseudo-)passive, which \emph{suppress} the subject 
and promote an internal argument (\prettyref{sec:grammatical.clause.subject.argument-structure}) to the subject position (\prettyref{sec:grammatical.clause.verbal.argument-structure.passive}).
...
The preposition of the prepositional argument can sometimes be omitted after the verb,
producing ``indirect objects''  \citep[\citepage{102}]{li2004grammar}.
...
(\citealt[\citepages{416-421}]{meiguang2018}; \ref{ex:grammatical.clause.verbal.argument.prepositional.ask.double-object.2}).
...
This is first demonstrated by the linear order (cf. \ref{ex:grammatical.clause.verbal.argument.prepositional.ask.1}),
and is also demonstrated in passivization.
In (\ref{ex:grammatical.clause.argument.applicative.pivot.1}),
it is the G argument that is passivized (\prettyref{sec:grammatical.clause.verbal.argument-structure.pseudo-passive.multiple-argument})
TODO: 赐之以重祭 vs 赐祭於之

This is expected with the analysis that the applicative introduces the second pivotal argument in the clause
(\prettyref{sec:grammatical.clause.subject.argument-structure}).

\begin{exe}
    \ex\label{ex:grammatical.clause.argument.applicative.pivot.1} 诸侯，赐弓矢然后征
\end{exe}
...
not all of which should be considered syntactic. 
...
and therefore there is always room to argue that grammaticalization did not happen.
...
(and never \translate{I considers Lord Mengchang as a guest}).
...
Interestingly, we are also able to define the second most pivotal argument:
in applicative constructions, which appear to cover all double object constructions in Classical Chinese
(\prettyref{sec:grammatical.verbal.argument.prepositional}),
promotion to subject always targets the argument introduced by the applicative process,
which then may be known as the second most pivotal argument.
Hierarchies of externality or ``pivotality'' of arguments is not uncommon in world languages
(\prettyref{box:multiple-external-arguments}).

\begin{infobox}{Hierarchy of arguments}{multiple-external-arguments}
    ...
    by first taking the two most internal arguments and decide which is more salient on the empathy hierarchy,
...
are clause pivots, around which everything else ``revolves around''.
...
is much less frequent than the so-called 判断句 \translate{judgemental clause},
...
The distribution of sentence final particles is strongly correlated to clause type:
they almost always appear in sentential clauses.
...
Classical Chinese has one clausal construction known as the \category{gerundive} in , which may appear as the object
...
Others are clearly not, including those in which the two verb phrases branches are separated by a pause \citep[\citepage{195}]{meiguang2018}.
...
(\ref{ex:grammatical.clause.verbal.stative.3}) can analyzed as a situation 
...
\subsection{Verbs governing a locative object}
...
but if the verb phrases is emphasized, topicalization is possible.
