...
List B dictates lexicalization of grammatical properties of forms
...
List C dictates idiomization, which may also be known as semantic lexicalization.
...
This often means that forms containing grammatical markers that seemingly semantically make sense are not acceptable (e.g. \prettyref{sec:grammatical.clause.verbal.tam}).
...
and are introduced in the rest of the sections of \prettyref{sec:grammatical.clause} and \prettyref{sec:grammatical.noun-phrase}.
...
In this chapter we summarize the syntax of the clause (\prettyref{sec:grammatical.clause}) and the syntax of the noun phrase (\prettyref{sec:grammatical.noun-phrase}), along two intertwined lines:
systems and constructions in clauses and noun phrases and grammatical functions therein, 
and lexical properties that determine how content roots appear in these environments.

Thus we identify nominal and verbal \emph{grammatical environments}
(i.e. the head of a noun phrase and the main verb of a clause),
and identify noun and verb \emph{form classes} in the lexicon according to language-specific properties of Classical Chinese at the same time,
totally aware that the mapping between the two may be non-trivial
(e.g. \prettyref{sec:grammatical.clause.verbal.word}).
...
Another largely uncontroversial criterion of wordhood is \emph{lexicalization}:
...
In this way, when the internal makeup of clauses and noun phrases depends on lexical properties of the head verb/noun,
and the heads are grammatical words.
...
These discourse-related devices typically only appear in sentential clauses and are absent in embedded clauses (except for direct quotations; \prettyref{sec:grammatical.clause.argument.verbal-complementation.direct-quotation}). 
...
We call it a \term{nominal clause}\footnote{We do not use the term to refer to a nominalized clause.} or a \term{verb clause}, respectively.
...
Classical Chinese lacks obligatory \ac{tam} marking but does have a \ac{tam} system marked by adverbs and possibly some sentence final particles (\prettyref{sec:grammatical.clause.verbal.tam}).
...
A nominal predicate typically expresses an ``is-a'' relation between the subject (\prettyref{sec:grammatical.verbal.subject.clause-pivot}) and the predicate
(\ref{ex:grammatical.clause.nominal.isa.1}).
...
Classical Chinese has nominative-accusative alignment and hence has a well-defined subject:
a clause with verbal predication consists of a \term{subject} (\prettyref{sec:grammatical.clause.subject}) and a verb phrase (VP), which is the predicate of the clause and contains \ac{tam} adverbs and core and peripheral arguments.
...
the order is always \ac{tam} \before peripheral argument (\ref{sec:grammatical.clause.order-of-modifier.1}).
...
\begin{exe}
    \ex \label{sec:grammatical.clause.order-of-modifier.1}
...
The structural closeness of arguments to the main verb is briefly discussed in \prettyref{sec:grammatical.verbal.argument.prepositional}.
...
(问曰 for instance at least originally came from argument structure-level coordination; cf. \ref{ex:grammatical.clause.argument.verbal-complementation.direct-quotation.v-say.1}).

A peculiarity of Classical Chinese is that a root typically appearing in non-verbal environments can undergo ad hoc categorization (traditionally known as 词类活用) and serve as the main verb (or more accurately, the \term{predicator} i.e. the core of the predicate).
(\ref{ex:grammatical.clause.nominal.noun-to-verb.1}) is a perfectly well-formed verbal clause:
...
(\ref{ex:grammatical.clause.svo.intransitive.1}) is a stative verbal clause involving ad hoc categorization.
...
(and thus it is not possible to generalize from the pattern in \ref{ex:grammatical.clause.nominal.noun-to-verb.1} to a generic process),
and therefore in the terminology of \citet[\citepages{1640-1641}]{cgel} is conversion as opposed to derivation.
Unlike the case in formal English,%
\footnote{
    Ad hoc conversion however is possible in informal English,
    as in \form{I might [guinea pig] it for you}
    and even the dephrasal \form{``Sappho and her friend'''d}.
}
in Classical Chinese conversion to forms not lexically well established is idiomatic:
that 水 appears in (\ref{ex:grammatical.clause.nominal.noun-to-verb.1})
does not necessarily mean creation of a new verb \emph{lexical entry}.
In traditional terms, 词类活用 does not mean 一词多义. 

In other clauses, the meaning of the root filling the main verb position of the clause can be regularly inferred,
and should thus be considered verbalizing zero derivation.
An important process of this kind is the tropative (\ref{ex:grammatical.clause.subject.causative.1}).
...
(\ref{ex:grammatical.clause.svo.interrogative.1}; but see \prettyref{sec:grammatical.clause.argument.high-level}).
...
\begin{exe}
    ...
    [子]_{\text{subject,A}} [[奚]_{\text{reason (\prettyref{sec:grammatical.clause.argument.high-level})}} 不 [为]_{\text{verb},\category{do}} [政]_{\text{P}}]_{\text{predicate:VP}}？
...
can be about an intentionally initiated event (\category{do}; \ref{ex:grammatical.clause.svo.interrogative.1}, \prettyref{sec:valency.intransitive.action}),
a state (\category{be}; \ref{ex:grammatical.clause.svo.intransitive.1}, \prettyref{sec:valency.simple.state-and-change}) or a change of the state (\category{become}; \prettyref{sec:valency.simple.state-and-change}).
...
and are ``transitive'' superficially with a structure different from that of (\ref{ex:grammatical.clause.svo.interrogative.1}).

\subsubsection{Passive and causative constructions}\label{sec:grammatical.clause.argument.valency-change}
...
which may be known as the passive, which suppress the subject 
...
(\ref{ex:grammatical.clause.verbal.argument.valency-alternation.1}) is an example of the causative applied on top of the passive.

\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.valency-alternation.1} 
    \gll 杀 御叔 \\ 
    kill \category{name} \\ 
    \glt\translate{\dots caused Yushu to be killed.} 
    \hfill (=\ref{ex:grammatical.clause.verbal.argument.causative.synthetic.1} in \prettyref{sec:grammatical.clause.verbal.argument-structure.causative.synthetic})
\end{exe}
...
\subsubsection{Prepositional arguments}\label{sec:grammatical.verbal.argument.prepositional}

(\ref{ex:grammatical.clause.verbal.argument.prepositional.1}, \ref{ex:grammatical.clause.verbal.argument.prepositional.give.1}, \ref{ex:grammatical.clause.verbal.argument.prepositional.ask.1}) are examples of prepositional constituents, carrying a wide range of semantic roles.
...
Prepositional phrases can also be fronted (\prettyref{sec:grammatical.clause.argument.high-level}).
...
\begin{exe}
    ...
    \gll 季康子 问 政 [于 孔子]_{\textbf{\text{location}}} \\ 
...
This is not possible for every verb: no non-prepositional counterpart of (\ref{ex:grammatical.clause.verbal.argument.prepositional.1}) is attested.

\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.prepositional.no-prep.1} 入秦何見
\end{exe}

\subsubsection{Applicative constructions}\label{sec:grammatical.verbal.argument.applicative}
...
(\ref{ex:grammatical.clause.verbal.argument.prepositional.ask.double-object.2}) illustrates an applicative clause where the target of the question becomes an object
...
\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.prepositional.ask.double-object.2} 
    上 问 上林尉 诸禽兽簿
\end{exe}
...
(\ref{ex:grammatical.clause.argument.possession.1}). 

\begin{exe}
    \ex\label{ex:grammatical.clause.argument.possession.1} 子路性鄙
\end{exe}

(\ref{ex:grammatical.clause.argument.possession.1}) should not be analyzed as topicalization, because typically a pause cannot be inserted after 子路 (\prettyref{sec:grammatical.verbal.subject.topic-comparison}).

\subsubsection{Complement clause constructions}\label{sec:grammatical.clause.argument.verbal-complementation.direct-quotation}
...
it is not possible for the prototypical complement clause in (\ref{ex:grammatical.clause.argument.complement-clause.1}) to have sentence final particles;
on the other hand, the direct quotation (see \prettyref{sec:valency.direct-quotation}) in (\ref{ex:grammatical.clause.argument.verbal-complementation.direct-quotation.say.1}) is sentential and has a sentence final particle.

\begin{exe}
    \ex\label{ex:grammatical.clause.argument.complement-clause.1} 臣窃以为 [不便於君]_{\text{\textbf{complement clause}}}
...
(\prettyref{sec:grammatical.clause.coordination}; \ref{ex:grammatical.clause.argument.verbal-complementation.direct-quotation.v-say.1}).
...
\subsubsection{Raising and control}\label{sec:grammatical.clause.argument.raising-control}
...
(\ref{ex:grammatical.clause.argument.verbal-complementation.raising.1}) is an example of raising-to-object in Classical Chinese,
...
\subsubsection{Verb phrase structure}\label{sec:grammatical.clause.argument.high-level}

All arguments \citep[\citepages{3-5}]{he2005shiji}, include prepositional ones (\ref{ex:grammatical.noun-phrase.preposition.omission.1}), can be omitted when they are identifiable from the context. 
...
First, the object pronoun is often regularly moved before the verb in a negative clause (\ref{ex:grammatical.clause.sov.neg.1}) without clear information structure motivation, leading to a SOV order.
...
\begin{exe}
    \ex\label{ex:grammatical.clause.verbal.argument.focus.1} 季康子 [以其妹]_{\text{VP-focus}_i} 妻 之 ---_{i}
\end{exe}

The fronted prepositional phrase may further undergo preposition-object inversion (\ref{ex:grammatical.clause.verbal.argument.focus.2}).
...
\subsection{Tense, aspect, and modality}\label{sec:grammatical.clause.verbal.tam}
...
The most wide-scope \ac{tam} categories are usually categories related to the speaker's evaluation of the situation being talked about.
\ac{tam} categories falling under this type include evaluative adverbs (like \form{frankly} at the start of a sentence), evidentiality, and epistemic modality.
...
marked by the sentence final particle 矣 (\ref{ex:grammatical.clause.tam.sentence-aspect.1}).
...
We discuss the issue in \prettyref{sec:grammatical.verbal.subject.topic-comparison}.

\subsubsection{Subject vs. topic}\label{sec:grammatical.verbal.subject.topic-comparison}
...
(\ref{ex:grammatical.clause.subject.causative.1}),
...
it is often the case that it is obligatorily so (\ref{ex:grammatical.clause.subject.passive.1}),
which again is unusual for topicalization
(but not unusual if interpreted as valency alternation).

\begin{exe}
    \ex\label{ex:grammatical.clause.subject.passive.1} 
    国定而天下定
\end{exe}
...
The existence of a pivot at the level of argument structure, which is often the A-like argument in a clause, can be demonstrated by examining the shared argument in control constructions.
The reflexive pronoun binding test is not available in Classical Chinese,
as no unambiguous reflexive is attested.
...
In applicative clauses, the promoted argument undergoes a leftward movement,
which can be interpreted as it being promoted to a structurally more pivotal position
(\prettyref{sec:grammatical.verbal.argument.applicative}, \ref{ex:grammatical.clause.verbal.argument.prepositional.ask.double-object.2}).
In double object \translate{giving} clauses,
it is always the G argument being passivized (\ref{ex:grammatical.clause.argument.applicative.pivot.1}),
which again can be interpreted as the G argument being structurally more pivotal.

\begin{exe}
    \ex\label{ex:grammatical.clause.argument.applicative.pivot.1} [诸侯]_{\text{G}}，赐 [弓矢]_{\text{T}} 然后征
\end{exe}
...
\subsubsection{Clausal pivot}\label{sec:grammatical.verbal.subject.clause-pivot}

We then proceed to prove that the A-like, most pivotal argument in a verbal clause 
and the initial constituent in a nominal clause 
are clause pivots, around which everything else ``revolves''.
This proves that Classical Chinese has nominative-accusative alignment and justifies the notion of subject.
...
which is usually the argument structure pivot (\prettyref{sec:grammatical.clause.coordination},
...
in which the subject is topicalized, often with the sentence final particle 也 (\ref{ex:grammatical.clause.nominal.isa-topic.1}),
or the topic particle 者 (\ref{ex:grammatical.clause.nominal.isa-topic.2}), or both.
...
The topic marker 者 is not restricted to the nominal predication construction
(\ref{ex:grammatical.clause.topic.1}).
...
\begin{exe}
    ...
    \gll [[甚]_{\text{VP_i}} 矣]_{\text{focus}}， [[汝 之 不 惠]_{\text{subject:clause (\prettyref{sec:grammatical.clause.verbal.gerundive})}} ---_{\text{predicate_i}}] \\
...
Interrogative clauses may completely lack formal marking except interrogative pro-forms (\ref{ex:grammatical.clause.sov.interrogative.1}).
...
\subsubsection{Nonfinite constructions}\label{sec:grammatical.clause.verbal.gerundive}
...
in (\ref{ex:grammatical.clause.verbal.gerundive.2}), the ``nominalized'' clause does not admit adjectival modification
and takes an object, both unusual for a noun phrase.
The example therefore has a structure comparable to the English gerundive non-finite clause \form{his playing national anthem}
...
Complement clauses are discussed in \prettyref{sec:grammatical.clause.argument.verbal-complementation.direct-quotation} and \prettyref{sec:grammatical.clause.argument.raising-control}.
Relative clauses are discussed in \prettyref{sec:grammatical.noun-phrase.determinative-relative}.
...
An interesting phenomenon is that the subordinated clause can be a \form{zhī}-construction clause (\prettyref{ex:grammatical.clause.verbal.gerundive.2}).
...
In (\ref{ex:grammatical.clause.coordination.subordination.1}),
...
\begin{exe}
    \ex\label{ex:grammatical.clause.coordination.subordination.1}
...
In Classical Chinese, certain examples are clearly argument structure-level coordination (\ref{ex:grammatical.clause.argument.verbal-complementation.direct-quotation.v-say.1}).
...
which only links verbal constituents and cannot link to nominal constituents.
Note that the functions of 而 are not restricted to conjunction (\citealt[\citepage{183}]{meiguang2018}; \ref{ex:grammatical.clause.coordination.subordination.1}).
...
(\citealt[\citepage{61}]{pulleyblank1995outline}; \prettyref{sec:grammatical.noun-phrase.determiner}).

\subsection{Structure of the noun}\label{sec:grammatical.noun-phrase.word}
...
they have meanings regularly inferrable from their nominal uses (as in the tropative example \ref{ex:grammatical.clause.subject.causative.1}).
They thus should be regarded as grammatical words, due to lexicalization of their grammatical behaviors.

This however does not say anything about wordhood of composite forms like (\ref{ex:grammatical.noun-phrase.word.1}).
...
In (\ref{ex:grammatical.noun-phrase.word.1}), the modifier usually cannot be modified by adjectives.
Structural complexity constraints nonetheless appear in structures that are clearly not words (cf. \prettyref{sec:grammatical.clause.argument.verbal-complementation.direct-quotation}).

\begin{exe}
    \ex \label{ex:grammatical.noun-phrase.word.1}
...
There is clear semantic lexicalization: (\ref{ex:grammatical.noun-phrase.word.2}) obligatorily means \translate{a minister serving the sovereign}, never, say, \translate{a minister who happens to reside in the sovereign}.
...
\footnote{
    ...
    as in \prettyref{sec:grammatical.clause.argument.verbal-complementation.direct-quotation}.
...
\begin{exe}
    \ex\label{ex:grammatical.noun-phrase.word.2} 社稷之臣
\end{exe}
...
and parts in a form like (\ref{ex:grammatical.noun-phrase.word.1}) are often though not always aggregated into a word,
...
The 之-insertion (\prettyref{sec:grammatical.clause.verbal.gerundive}) is sometimes considered nominalization.
Interestingly, relativization in Classical Chinese may indeed be nominalization (\prettyref{sec:grammatical.noun-phrase.determinative-relative}).

Classical Chinese does have nominalization in the sense of recategorization.
In (\ref{ex:grammatical.noun-phrase.nominalization.1}),
坚 refers to \translate{what are hard (i.e. offensive weapons)}
and 锐 refers to \translate{defensive weapons},
semantically comparable to relative clauses and analyzable as zero derivation (cf. discussion in \prettyref{sec:grammatical.clause.verbal.word}).

\begin{exe}
    \ex\label{ex:grammatical.noun-phrase.nominalization.1} 被坚执锐
\end{exe}
...
(\ref{ex:grammatical.noun-phrase.word.2}).

Because it is hard to distinguish lexicalized argument structures for Classical Chinese nouns (\prettyref{sec:grammatical.noun-phrase.word}),
...
(\ref{ex:grammatical.noun-phrase.direct-modification.1}; cf. English \form{a big gray bear}).

\begin{exe}
    \ex\label{ex:grammatical.noun-phrase.direct-modification.1} 大白狗 \hfill (汉书·五行志)
\end{exe}

It should be noted that examples involving multiple pre-nominal attributives are scarce.
Complex attributives are much more idiomatically constructed using the relative clause construction (\ref{ex:grammatical.np.nominal.relative.long-1}).

\subsection{Relative clause constructions}
\label{sec:grammatical.noun-phrase.determinative-relative}

Classical Chinese relative clauses are typically marked by 者.
A fused relative clause can take a determiner
(\ref{ex:grammatical.noun-phrase.relative.modify.1}),
and hence may be analyzed as a nominal \citep{aldridge2009old}.

\begin{exe}
    \ex\label{ex:grammatical.noun-phrase.relative.modify.1} 晉之從政者
\end{exe}

(\ref{ex:grammatical.np.nominal.relative.long-1}) is an instance of a long, complex relative clause in Classical Chinese.
...
\begin{exe}
    \ex\label{ex:grammatical.np.nominal.relative.long-1} 若[--- 至力农畜， --- 工虞商贾， --- 为权利以成富，[--- 大者倾郡]，[--- 中者倾县]，[--- 下者倾乡里]]者，不可胜数 
\end{exe}
...
Classical Chinese has pre-nominal relative clauses too (\ref{ex:grammatical.noun-phrase.relative.pre.1}).

\begin{exe}
    \ex\label{ex:grammatical.noun-phrase.relative.pre.1} 得誌之君子
\end{exe}

\subsection{The determiner system}\label{sec:grammatical.noun-phrase.determiner}

A demonstrative-before-numeral order can be observed in the determiner system
(\ref{ex:grammatical.np.template.dem.1}).

\begin{exe}
    \ex\label{ex:grammatical.np.template.dem.1} 此三姦者 (荀子·非十二子)
\end{exe}

The ``possessor'' (which does not necessarily has possessive meaning), when present, is separated by the particle 之 from the nominal region
(\ref{ex:grammatical.np.template.gen.1}).
It appears that the possessor, the demonstrative, and the pre-nominal relative clause (\ref{ex:grammatical.noun-phrase.relative.pre.1}) are in competition 
(while they coexist with the numeral).

\begin{exe}
    \ex\label{ex:grammatical.np.template.gen.1} 君之三臣
\end{exe}
...
Prepositions in Classical Chinese all have diachronic verbal etymology,
but at least some of them have already fully grammaticalized.
The meaning of 於 in (\ref{ex:grammatical.noun-phrase.preposition.1}) cannot be reasonably analyzed as verbal.

\begin{exe}
    \ex\label{ex:grammatical.noun-phrase.preposition.1} 且貳於楚也
\end{exe}

It is possible to omit the complement of a preposition.

\begin{exe}
    \ex\label{ex:grammatical.noun-phrase.preposition.omission.1} 孔子 [因 ---]_{\text{reason}} 叹
\end{exe}
...
in their distribution in various syntactic environments,
...
A noun-verb distinction is supported by carefully examining the traditionally recognized noun-used-as-verb phenomena
...
The long and complicated historical evolution of Classical Chinese
...
\paragraph*{之} (TODO: add a discussion of the particle 之.)
