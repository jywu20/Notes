...
as is shown by comparison between the \work{Analects} and the \work{Odes}.
...
It is attested that the word 楚 may mean \translate{the Chu state} or \translate{do what Chu people do},
and the word 床 may mean \translate{bed} or \translate{settle down your bed or sleep on a bed}.
Therefore, if the word 城 \translate{city, castle} is completely not categorized in Classical Chinese lexicon, 
when used in a verbal way,
it would mean \translate{do city-related things} (c.f. 楚), 
\translate{build a city} (c.f. 床), and much more.
However, in reality, only the \translate{build a city} meaning is attested.
This suggests that what is stored in the lexicon is not the bare, non-categorized root 城,
but one noun lexeme 城 \translate{city} and one verb lexeme \translate{build a city},
and other seemingly possible ways to categorize the root, although attested elsewhere,
do not work for the root 城.

In the above examples, the relations between the nominal and verbal usages do not seem particularly regular.
Although tropative or causative derivations in Classical Chinese
...
In these examples the verbal meaning are regularly derived from the nominal meaning.
Even though there is no formal marking,
it seems the verbal usages are nothing different from noun-to-verb derivation observed in other languages.
These derivations are not completely regular and not for every word:
this is another piece of evidence suggesting 
that the lexicon specifies how a root should be categorized even in Classical Chinese. 

Therefore, the phenomenon in Classical Chinese that a root usually appearing 
in a nominal environment appears in a verbal environment 
is not different from similar phenomena in other languages.
In some cases, like the case of 臣 or 王, it seems a root is first categorized as a noun 
and then undergoes something similar to English \form{-ize} (albeit without any explicit marking),
and while the meaning of the nominal usage has to be recorded in the lexicon,
the meaning of the verbal usage can be automatically decided from the derivation rule.
We note that whether a derivational rule applies is also attached as a lexical property of the root.
In other cases, like the case of 城 or 楚,
both the nominal and verbal usages are to be recorded in the lexicon,
as they cannot be inferred regularly from each other.
The boundary between the two situations is somehow blurry,
...
But we can at least conclude that both multiple functions and zero derivation exist in Classical Chinese
\citep[\citesec{11.3}]{dixon2010basic2}, 
and in either cases, how a root is possibly categorized is stored in the lexicon,
...
A similar pattern can be observed for words commonly seen in verbal environments receiving a nominal usage.
...
\begin{todobox}{Deverbalization derivation}{deverbal}
    Summarize deverbal derivations. Also, it seems this section is too long.
    Add another chapter about derivation?
\end{todobox}

We have two final caveats for the noun-verb distinction.
First, there are sporadic verbal usages of nouns that are almost never attested elsewhere,
like 军 in 沛公军霸上.
...
This is also possible in English but usually not accepted in formal texts.
...
Second, what appears as an argument is not necessarily a \ac{np}:
...
the constituent order of core constituents is almost always SVO
...
\subsection{Discussions on subjecthood}
...
Traditionally, a pause is needed after some clause-initial \ac{np}s but not others.
