...
refrained from analyzing some sort of ``underlying'' system behind the surface forms
...
which may be either a verbal clause 
with possible \ac{tam} modifications
or a nominal predicate clause,
...
In this case we may say ``the noun is used as a verb'',
although in conventional terminology this would be a confusion between
grammatical functions (``head of \ac{np}/\ac{vp}'') 
and lexical properties (``usually appear as head of \ac{np}; 
when phonologically realized a certain morphological template applies'').

\begin{exe}
    \ex 大楚兴，陈胜王
\end{exe}

\subsection{Verbal nucleus clauses}
...
The rest of a verbal clause, besides the subject,
