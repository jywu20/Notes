...
It is often worthwhile to package a class into a module,
especially when objects of that class are to be shared by different threads.
In software engineering for instance we have producer-consumer model,
which translates to FIFO in hardware designing,
and just as an object can be shared by multiple threads in software engineering,
a FIFO or other port can be shared by multiple clock domains in RTL.
When demarcating module boundaries when translating software algorithms to RTL,
one choice is to put the logic of `stream.read` invocations into the logic of consumer threads,
and to put the logic of `stream.write` invocations into the logic of producer threads,
but now we find we have storage units shared by the two threads which do not clearly belong to either clock domain.
A more intuitive way is to make the FIFO, the logic of `read`  and `write` into a single module,
and circuits corresponding to the producer and the consumer can send signals to the 
`start_read` or `start_write` ports of the FIFO module.
(Note, however, that to write an asynchronous FIFO,
we expect the `read` and `write` methods to be able to have different clock signals,
which need additional pragmas or can't be done in HLS altogether,
...
# Cheat sheet: comparison between HLS and RTL
...
- Variables plus [event listeners](#event-listener) is a module in RTL.
  In [HLS](#hls-synthesizes-functions), a function that may launches threads is roughly (see below) equivalent to a module:
  the function, when run, runs as a thread, which links the input and output variables in a certain way,
  ...
  The features of the function in question decide what kind of module it corresponds to.
  ...
  That's to say, a module in RTL with control flow transferring supports = a non-thread function that can be called by others in HLS.
  - Note that calling the function in a main function alternates the state machine design of the main function.
  - An never-ending `while (1)` thread in HLS corresponds to a module by default without handshake signals.
  Handshake signals and additional states in the finite state machine structure of the main function
  however appear when we choose to use writing to and reading streams
  to share data with the thread.
  - It's of course in theory also possible to have threads that end, although tools do not necessarily support that, as parallelism pragmas used with ordinary functions are enough to provide this functionality.
...
- Streaming in HLS correspond to FIFO and the like in RTL (also see below)
- [Object-oriented programming](#object-oriented-programming) in HLS is typically synthesized by treating methods as functions.
  A class, with possibly multiple ``methods'' inside and their shared memory is another type of modules in RTL.

So if we want to put things in a more concise way,
a module corresponds to one or more threads with possible shared memory,
and as function invocation can be understood as a thread going into an idle state
and another thread is "activated",
a function can also be conceived as a module 
with necessary handshake mechanisms to ensure the correct control flow.
A module containing multiple functions is an object.
We can also have thread bundles (with shared resources, like what people do in web server)
