...
（将非函数式的语言嵌入函数式语言是一条编程语言理论中的暗线。与之有关的学术出版可见The End of History? Using a Proof Assistant to Replace Language Design with Library Design。所谓monad as programmable semicolon。这种嵌入有一个好处，就是做形式化验证的时候只需要一个能用来verify typed lambda calculus的逻辑就可以了。例如HOL只是自带一个typed lambda calculus，但是靠monad支持了命令式编程。
另，嵌入或者说embed这个说法是PLT学界有在用的，如见The Linearity Monad一文，下面会讲到。）
...
例如，“Lean和ZFC+可数个不可达基数可以互相诠释对方”就是一个作为后者的类型论的成果。

（注：可以互相诠释对方，或者说equi-interpretable, 不能理解为两个理论可以互相构造对方的模型，否则这两个理论可以互相证明对方的自洽性。理论1诠释理论2可以通过在理论1中构造理论2的模型做到，但也可以通过将理论2解释为理论1的“全体”来做到）

如前所述，（作为一个具体的形式系统的）类型论有语义。一种常见的语义是把类型理解成集合。
...
实际上如果我们想做一个基于集合论的proof assistant，高层设计也可以这么做，为了避免“空集是不是一个平面的元素”之类无谓的讨论。

全体允许的tag以及这个集合的结构可以称为**动态类型系统**，用面向对象编程的说法讲大概就是object model。

在研究用此编程语言开发的程序的行为时，我们用（比如说）small step semantics来定义这个编程语言的operational semantics。
这种情况下，一个程序$f$的类型安全性等价于如下的命题：设$x_1 : T_1, x_2 : T_2, \ldots$，则$f(x_1, x_2, \ldots)$的运行过程中不会出现类型错误（或者，如果操作语义中没有设定类型错误，类型安全性就是说一段程序不会因为找不到能进行的下一步操作而卡死）。
...
一个自然的想法是向编程语言内引入一个**静态类型系统**，即不仅给值贴类型标签，也给表达式（和值合称term）贴上类型标签，然后据此开发一个natural deduction或者sequent calculus来在运行前就自动地检查一段程序是不是会有类型问题。

这个类型系统是**对象语言，即待分析的编程语言**中的类型系统，
不过它可以理解成元语言中关于值的类型和表达式的结果的类型的理论（元语言本身可以是有类型的或者无类型的）的一个切片。
对象语言中的类型系统在多大程度上能够“自动”地保证一个程序有某些（元理论中的）性质是下面所谓soundness研究的课题。
...
另外也注意此处作者其实已经定义了一个operational semantics，因为有谓词`eval e e'`，但这个operational semantics仍然带有很强的“语法”色彩：这是说，`e`可能代表一个值（as in `value e`）但是$e : T$还是被形式化成了`typing E e T`，而不是`e : something`。
...
有若干种不同的soundness的定义。
最强的定义是这样的：“一个程序的输出*真的*在它的输出类型中”——这就是说，程序真的有输出而不报类型错误（从而输出的值的类型标签和这个程序的函数签名中的输出类型一致），而且输出的值的二进制编码（如前所述，一个值是编码-类型标签对）也确实在这个类型对应的集合里面（也即没有data corruption，比如说一个字符串的二进制表示被强行贴上了`ArrayList`的标签）。
...
## Intrinsic and extrinsic type systems

我们这里采取的现有操作语义、再定义（静态）类型系统以保证一段程序拿去跑不会报类型错误的思路让人想到intrinsic typing和extrinsic typing的区别：人们经常说前者是先有类型系统再有操作语义而后者反过来。
但这个说法是有误导性的。
这两种类型系统的真正区别在于类型标签是怎么贴到term上面的，intrinsic typing中标签始终是由于语法要求而跟着term走的，而后者中可以证明一个term有某个type，也就是说编程语言还带有一个（不可运行的）逻辑，用来检查代码会不会报类型错误。
事实上编程语言设计者经常从操作语义出发去motivate一个intrinsic type system的设计。

又有说法说intrinsic typing允许让语义依赖于类型；但这只是trivially true。
考虑intrinsic type中用类型标签区分结构相同的Tuple的做法。
在一个extrinsic type system中，可以往tuple中加一个常量的标签，然后将含有标签`T`的tuple的类型都指定为$T$，这样就获得了和intrinsic type system类似的行为。
所谓intrinsic typing是先有类型系统再有操作语义这个说法的源头是类型系统是语法的一部分，而extrinsic typing有一个语法以外的证明语言，仅此而已。

实际上intrinsic type和extrinsic type的边界某种意义上是模糊的。
考虑标准版本的HOL，它里面有一个intrinsic type system，然后它支持通过`typedef`定义refinement type，然后通过`lift_definition`证明，比如说，一个从自然数到自然数的函数在输入是偶数的情况下其实是从偶数到偶数的，从而证明一段作用在偶数上的程序确实是偶数到偶数的。
这算intrinsic type还是extrinsic type？听着感觉是intrinsic simple type加上extrinsic refinement。
但是实际上所谓的intrinsic simple type也是靠`typedef`定义出来的（靠无穷公理加分离来定义归纳类型；例如自然数就是靠对`ind`做分离得到），然后类似于两个自然数加起来得到另一个自然数这样的函数签名实际上都是可以也需要证明的命题。
所以实际上可以认为HOL的类型系统是a minimal intrinsic core consisting of `ind`加上一堆extrinsic的定义和推理搞出来的。
所以HOL的类型系统仍然有intrinsic的核心（and hence is still type a la Church）。

主流编程语言采用intrinsic typing的主要原因可能还是因为这样足够方便，因为如果动态类型系统有特定的结构，那么为了给它们写specification，静态类型系统也要有，那当某些构造——如归纳类型——比较复杂的时候，我们是把一个复杂的东西重复做一遍，没有必要。
当然，这也没有妨碍我们在脑子里面设想某种给intrinsically typed语言加入一个extrinsic的证明语言的做法，实际上大部分讲算法的书就是在做这种事情。

而且实际上当静态类型系统足够强的时候，并没有必要要有一个extrinsic的证明用的语言。
我们前面提到到类型系统可以看成Hoare logic trimmed down，即它可以看成是语义上的不要报类型错误的要求的语法化。
而实际上在类型系统足够强大的时候，Hoare logic也可以看成类型系统的一个特例：例如如果类型系统中有Prop，则前置条件和输入可以打包在一起，而后置条件和输出可以打包在一起（实际上这就是refinement type）。
事实上如果一个类型论被拿去做数学基础，Hoare logic就可以这么定义。

（语义的语法化是数理逻辑中很常用的套路，如集合论中$V=L$是一个元语言中关于模型的叙述，但是可以将其语法化成一个公理。）

intrinsic type还有另外一个好处：纯粹的extrinsic type有底层依赖（如它依赖于无类型的加法如何运作，显然Python上的无类型加法、JavaScript上的无类型加法不一样，而汇编里面并没有什么无类型加法），而intrinsic type编程语言的抽象只要做得足够好，是可以one size fits all的。
例如ML风格的ADT数学上就很干净（无非多项式不动点），而在常规计算机上实现也很容易（结构体加有标签的共用体）。
Isabelle/HOL的代码抽取也主要target `datatype`定义出来的类型上的函数。

从intrinsic type到extrinsic type的翻译，或者说softening，已有一些理论讨论，如见arXiv 2107.07665。
...
通用编程语言中的函数不该总是停机。
停机问题和类型系统的复杂关系（normalization）一般认为是类型系统的研究（程序语言理论）而非类型论的研究（元数学）中讨论的问题。

第二个不同点是soundness。soundness对作为数学基础的类型论来说是一定要满足的。
理想情况下对通用编程语言也该是这样，但实操上数量可观的语言的类型系统其实都不sound。
“一个不sound的类型系统加入何种限制以后就sound”是程序语言理论中的而不是数学基础中的话题。

soundness的问题还涉及一些设计上的取舍。“检查一段程序是否报类型错误”不是一个图灵可判定的问题，因此一个通用编程语言的类型系统如果能排除所有的运行时类型错误的话，是图灵不可判定的，也就是说对每个能被实际写出来的类型检查器，都存在一些程序实际上是well typed的但是没法通过检查。
...
第三个不同点是类型推导。类型推导是一个主要见于程序语言设计——开发proof assistant时显而易见很有好处但不是核心calculus的一部分——的议题。
类型推导有自己的decidability问题。

第四个不同点是，通用编程语言中的类型系统需要处理一些元数学中的类型论不会处理的约束。

例如，Rust的借用检查经常被看成其类型系统的一部分。作为数学基础的类型论很少关心这些。（生命周期甚至不是一个函数式编程中经常出现的东西）当然我们有linear type，但实际上Rust的calculus也很难说就是linear type。

借用检查是如下更宽泛的问题的一部分：实用的类型系统的开发还是要考虑命令式编程。
设想一个函数式编程语言$L_F$有类型系统$T$。将$T$移植到一个命令式编程语言$L_I$中。
显然，$L_F$可以看成$L_I$的子集，因为后者额外引入了副作用。
现在的问题是，能否严格地将后者（通过monad之类的办法）再嵌入前者？换而言之命令式编程会不会导非平凡地扩充类型系统？

对较为简单的类型系统而言回答应该是肯定的。例如ML语言一般都是不纯的，但是这并没有对其类型系统造成任何非平庸的影响
（例如设想将`ST a`和`a`都映射为`a`）。
但是假设我们在dependent type的编程语言中引入副作用，那就很麻烦了。
假设类型系统中有dependent type而某个类型依赖于某个term而后者又有副作用，我们要问：类型检查应该被允许做IO吗？
...
但哪怕不考虑dependent type，其实还是会有subtlety。
设想有一个返回值类型为`T`但可能中间报错（Java中的`throws`关键字）的命令式程序，我们要问此命令式程序的“行为”（包括类型检查）能否完整地编码到此函数式语言中。
这里有很多subtlety，例如如果一个程序抛出多种异常，而且我们有`try... catch...`语法，则这段程序嵌入回函数式编程语言时大概要看成结构大致为`result T | error1 | error2`的monad，而`try catch`对应模式匹配。
如果$T$不支持untagged union type那$L_I$的程序就无法嵌入$L_F$。
...
我们可以追问dependent type theory或者别的很强的typed lambda calculus是否足以容纳由于命令式编程的引入而产生的额外需求，或者反过来，也可以质问是不是命令式编程里面某些常见做法其实不对头而应该采用能从typed lambda calculus自然地导出的概念取代。

另一方面，如果$T$已经被充分地扩展以容纳$L_I$了（异常以外，仍需要Types and Programming Languages一书12.3节这一类的编码如何读取变量和赋值的类型系统扩展，但这些可以自然地在将命令式程序翻译成monad时做完；可惜的是这一节，包括第19章对Java的形式化，都没有讨论类型系统如何被命令式编程的求值语义影响），是否可以说$L_I$中的程序是well-typed，当且仅当它在$L_F$中的对应物是well-typed，以及两个语言中的soundness是否可以互相翻译？


## 命令式编程与内存安全

在命令式编程中除了类型安全以外尚有内存安全。数据损坏的问题——即一个变量确实获得了一个值，但是这个值和这个变量的类型不符合——前面已经讨论了，在primitive操作是内存安全的情况下可以看成类型安全的一部分。
...
当然，和类型安全性类似，一个保证decidability的类型系统会误伤实际上内存安全但是在前者中无法证明的程序，而一个保证decidability的内存安全性calculus也会拒绝一些其实内存安全的程序。
...
类型系统发明出来就是避免这一点的，一个好的关于内存安全性的calculus也类似的应该要足够简单，最好还要可判定（会误杀一些好的程序，如双向链表lol）。
...
这些看起来都是很对……然后你看它的typing judgment的格式：Γ | E;L | K;T ⊢ F。
以及" It is important to stress that the typing context is substructural"这句话。
...
格式和sequent calculus的一般形式不一样的类型系统要嵌入譬如说Lean的类型系统就不好做了。
所以可想而知ownership要嵌入没有直接计算一个变量出现几次的能力的类型论是困难的。
...
暂时先不考虑Rust，The Linearity Monad by Paykin and Zdancewic倒是成功地将一个linear type system嵌入了Haskell。下面简单讨论它对我们的启示。

先考虑一个所谓的deep embedding，其中linear typed program（或者别的不好表示的类型系统中的term）被直接编码成ADT中的term。
然后我们想办法把线性类型系统的规则编码成Haskell的类型系统的规则。
如Haskell中的`x :: exp γ σ`说的其实是（线性类型系统中）$\gamma \vdash x : \sigma$。在这里看到，typing context显式地出现在了类型表达式中。
σ ⊸ τ定义为`Lolli σ τ`。
linearity的保证靠的是`HasLolli`中用到的若干个type class（用于判定$\gamma$中含有几个元素、是否相交等等）：
这几乎就是在写线性类型系统的定义：线性性就是依靠定义线性函数时写`λ $ \s → s ⊗ e`得到维持的。
这种定义下，一个linear的程序只是一个term而已，尚无monad结构。

这种embedding感觉没啥意思，更多是证明了靠Haskell的parametric polymorphism可以编码很复杂的命题。
前述type class其实就是可计算的谓词，无非为了绕过Haskell没有dependent type的约束，靠类型体操把wellformedness定义和证明挪到了类型层面而已。
（System F也是这么在做）
要在Lean里面这么搞就更容易了因为Lean有Prop，可以直接把线性类型系统的定义写下来——但也更加没意思。

我们在这里做的事情看起来和在Lean中定义一个任意的编程语言的类型系统和操作语义没有本质区别。
当然，这个嵌入的成功至少是说明将内存安全放到类型系统中并没有真的增强类型系统，那也可以说是一个不错的理论结果了。
但是我们想知道的是linear type在多大程度上可以用“普通”的、ML风格的类型编码出来。

也许出于这个动机，这篇文章又提到了一个shallow embedding，其中线性函数就是Haskell函数，而没有被表示成ADT。TODO


两种嵌入都比前述的“将命令式编程中的赋值符号替换成`<-`这样的操作，然后命令式编程的类型系统可以直接看成Haskell的类型系统作用在ST monad上”更加复杂。
无论如何，看来要把线性类型系直接插入进已有的monad生态还是做不到的。
...
面向对象编程的三个要素是封装、继承、多态。
所谓继承，就是说如果一些行为在子类上没有定义，则自动fall back to父类。
所谓多态，就是说可以写`a.method()`而不论这个method实现在哪一级类上面。


## 复制粘贴代码

经典的面向对象编程是基于继承的。继承这个事情和subtyping不完全一样。
subtyping实际上只管到继承和多态这两个事情，这些都是类型系统上的问题。
然而继承还涉及另外一个行为，就是复制粘贴代码：
所谓类`A`继承类`B`有的时候就是说把类`B`的代码复制粘贴到类`A`里面。
这么一搞以后，数据类型转换总是安全的，从而子类型关系似乎成立。

但反过来是不是成立呢？是不是子类型关系一定要来自代码的复制粘贴？我们是否真的想要给编译器一个对应的`Coe`实例呢？没有理由认为一定要这么做。

## 基于数据类型转换的方法fall back

另一方面，如果认可基于数据类型转换的subtyping，则可以自然地获得方法或者说行为的fall back，通常也称为继承。
...
（这个将行为和数据分开，然后让继承发生在描述行为的typeclass上的技巧不是单纯的奇技淫巧：它实际上可以看成所谓的finally tagless设计模式的一种特殊情况，后者一开始设计出来的目的是为了解决函数式编程语言容易扩展函数而不容易扩展数据结构的困境。事实上，finally tagless是Church encoding的一个版本：一个抽象类型的子类型的构造器同样可以用type class中的成员来模拟。
由于这个原因，finally tagless可以部分取代归纳类型，但由于它是在模拟“开放、用户可任意扩展”的父类，这样定义出来的抽象类型均没有归纳原理，后者对应面向对象编程中所谓的sealed class。）
