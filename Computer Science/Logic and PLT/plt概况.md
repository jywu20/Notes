...
这个类型系统可以理解成元语言中关于值的类型和表达式的结果的类型的理论元语言本身可以是有类型的或者无类型的）的一个切片。这个类型系统是**对象语言，即待分析的编程语言**中的类型系统。
...
现在的问题是，能否严格地将后者再嵌入前者？换而言之命令式编程会不会导非平凡地扩充类型系统？
...
另一方面，如果$T$已经被充分地扩展以容纳$L_I$了（异常以外，仍需要Types and Programming Languages一书12.3节这一类的编码如何读取变量和赋值的类型系统扩展，但这些可以自然地在将命令式程序翻译成monad时做完；请注意这一节，包括第19章对Java的形式化，都没有讨论类型系统如何被命令式编程的求值语义影响），是否可以说$L_I$中的程序是well-typed，当且仅当它在$L_F$中的对应物是well-typed，以及两个语言中的soundness是否可以互相翻译？
