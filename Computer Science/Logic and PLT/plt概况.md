...
弱一些的定义——weak soundness——可以通过通常称为syntactic soundness的方式证明，即证明progress和preservation两个性质。
...
作为数学基础的类型论中的函数一般会要求要是总是停机的（否则可以拿着一个其实算不出来的死循环的term招摇撞骗说构造出了inhabit某个类型的term）。
通用编程语言中的函数不该总是停机。停机问题和类型系统的复杂关系（normalization）是一个主要集中在程序语言理论而非元数学中讨论的问题。
