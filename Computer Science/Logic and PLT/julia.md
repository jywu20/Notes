...
`where`表达式和数学中的习惯记号是一样的。A where B在结构体和函数定义中给出了一个函数关系，而在其他地方给出了全体满足B的变量取值下A构成的集合；
两者使用同一个关键字，因为generic的并集运算形如$\cup_{i \in I} A_i$，其中$I$是所谓指标集，也即，定义generic的并集运算需要先有函数关系。
...
但是arrow type在Julia中是可以自定义的，[方法如下](https://discourse.julialang.org/t/specific-function-type/117256)：
...
foo2(5)
```
这个定义的一个问题是，无法在函数派发的时候就检查输入类型是否正确。要在函数派发的时候就检查输入类型是否正确可以采用如下的写法：
```julia
...
# Example usage: 
# Define a simple function to be wrapped in an arrow
foo = FunctionArrow{Int, Int}(x -> x + 1)
foo(5) # This will return 6
foo("std") # This will throw a MethodError
```
这个写法的问题在于如果有多个输入值需要用tuple或者struct包装一下。
...
这意味着Julia确确实实是动态语言，因为它保留了动态类型检查。静态类型检查可以看成对动态类型检查在运行前的一种模仿，即尽可能排除必然导致类型错误的代码。（见[此处](plt概况.md)）
...
以上讨论实际上说明了一件事：实用的语言的类型系统是两面的，它一方面是语言的operational semantics的一部分，此时不需要使用类型论的那种（语法的）方式理解，一方面包含一个静态的（最好用语法的方式理解的）演算，用来排除掉铁定在运行时会报类型错误的一部分程序。
；实际上，前者的数学性质也[常常并不好](#函数无法定typed-calculus中的类型)。
所谓弱的类型系统允许没有类型标注的值，一个强的类型系统不允许。
类型标注在operational semantics中的主要目的是函数派发；例如，尽管一个字符串也可以看成一个数，可是如果`a`和`b`被标记为了字符串，那我们就希望`a * b`是字符串拼接，如果`a`和`b`被标记为了数，则`a * b`就是乘法。
...
子类型在纯粹的类型论研究中是比较讨厌的一个东西，因为它不能像其它常见feature一样，通过一个引入规则和一个消去规则非常干脆利落、和其它feature确定正交地被引入，而且还经常不decidable。
