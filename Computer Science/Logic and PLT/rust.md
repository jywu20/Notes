...
这些机制不仅依赖于变量的类型，也依赖于程序的结构。原则上它们可以在一个足够强大的类型系统中用dependent type定义出来（平凡的，因为可以用Hoare triple来描述“如果某个步骤之前没报内存错误，那么它之后也不报”，而Hoare logic和refinement type是一回事），因此可以看成类型系统的一部分。
但是刨去它们，Rust的类型系统已然自成体系，并且是不是能够通过类型检查和程序的结构无关，那么borrow checker, lifetime这些概念被当成类型系统的一部分又会让类型系统看起来无端的别扭。
