...
\begin{itemize}
    ...
    (which are \emph{not} based on e.g. $\grad \grad$).
...
When $\epsilon$ is gaussian, $\log p(y | x, \theta)$ leads to the \concept{mean squared error} loss function.
...
which in turn means a sigmoid function as the last layer.
...
\begin{equation}
    \hat{\theta} = \argmax_\theta \big( \log p(\theta) + \sum_{(x, y) \in \mathcal{D}} y \log  + (1 - y) \log (1 - \hat{p}) \big),
\end{equation}
where $\hat{p} \coloneqq p(y = 1 | x, \theta)$.
This is often known as the \concept{binary cross-entropy loss}.
...
\end{equation}
Now the output of the model is supposed to be $[p(y_k | x, \theta)]_k$.
The constraints brought by this interpretation of the output of the model are 
\begin{equation}
    0 \leq p(y_k | x, \theta) \leq 1, \quad \sum_k p(y_k | x, \theta) = 1.
\end{equation}
This is equivalent to adding a softmax layer to the end of the model.
